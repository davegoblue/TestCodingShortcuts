...
Predictions are made for day/night-season for Houston, once using Houston's data and once using the existing model:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

# Using own data, predictions returned
houPredsTODS <- runFullRF(dfTrain=houTempTrain %>% filter(year(date) < 2022), 
                          yVar="todSeason", 
                          xVars=varsTrain, 
                          dfTest=houTempTest %>% filter(year(date)==2022), 
                          useLabel=keyLabel, 
...
# Using previous city data
newPredsTODSHOU <- newCityPredict(rfTODS, 
                                  dfTest=houTempTest %>% filter(year(date)==2022), 
                                  trueCol="todSeason", 
                                  isContVar=FALSE, 
                                  useSub="Previous city percentile model applied to make new city predictions"
...
Day/night and season are predicted reasonably when applying the old city model to the new city data, with ~75% accuracy. Predictions are stronger when using new city data as inputs to the model, with ~80% accuracy

Frequency of prediction errors is explored:  
```{r, fig.height=9, fig.width=9}

tmpAccTODSHOU <- newPredsTODSHOU %>% 
    count(todSeason, pred) %>% 
    mutate(dnPred=ifelse(str_detect(pred, pattern="Night"), "Night", "Day"), 
           dnAct=ifelse(str_detect(todSeason, pattern="Night"), "Night", "Day"), 
           seasPred=str_extract(pred, pattern="^[A-Za-z]+"), 
...
There are some meaningful misses in predicting season, particularly Summer predicted as Fall and Spring predicted as Winter or Summer

Training data is updated to include four cities:  
```{r, fig.height=9, fig.width=9}

cVec <- c("1"="LA", "2"="NYC", "3"="Chicago", "4"="Houston")

allCityTrain <- laxTempTrain %>%
    select(-starts_with("pct")) %>%
    bind_rows(nycTempTrain, chiTempTrain, houTempTrain, .id="src") %>%
    mutate(src=cVec[src])
allCityTrain
allCityTrain %>% count(year, src) %>% pivot_wider(id_cols="year", names_from="src", values_from="n")

allCityTest <- laxTempTest %>%
    select(-starts_with("pct")) %>%
    bind_rows(nycTempTest, chiTempTest, houTempTest, .id="src") %>%
    mutate(src=cVec[src])
allCityTest
allCityTest %>% count(year, src) %>% pivot_wider(id_cols="year", names_from="src", values_from="n")
...
Soil moisture alone strongly differentiates LA, Chicago, and Houston/NYC. Other variables are needed for pulling apart Houston/NYC:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

runFullRF(allCityTrain %>% 
              mutate(fct_src=factor(src)) %>% 
              filter(year<2022, fct_src %in% c("NYC", "Houston")) %>%
              mutate(fct_src=fct_drop(fct_src)), 
          yVar="fct_src", 
          xVars=varsTrain %>% str_replace(pattern="pct_", replacement=""), 
          dfTest=allCityTest %>%
              mutate(fct_src=factor(src)) %>% 
...
Patterns of deep soil temperature and deep soil moisture appear sufficient to differentiate NYC and Houston:  
```{r, fig.height=9, fig.width=9}

p1DF <- allCityTrain %>% 
    filter(year<2022) %>%
    bind_rows(filter(allCityTest, year==2022), .id="df") %>% 
    mutate(soil_temperature_100_to_255cm=round(soil_temperature_100_to_255cm, 1), 
           df=c("1"="1. Train (pre-2022)", "2"="2. Test (2022)")[df]
           ) %>% 
    count(df, src, soil_moisture_100_to_255cm, soil_temperature_100_to_255cm) %>%
    group_by(df) %>%
    mutate(wtdf=n/sum(n)) %>%
    ungroup()
...
# Create base plot
p1DF %>%
    ggplot(aes(x=soil_moisture_100_to_255cm, y=soil_temperature_100_to_255cm)) +
    geom_point(aes(size=n, color=src), alpha=0.2) + 
    facet_wrap(~df)
...
The model is re-run using only deep soil moisture. As expected, this drives 100% accuracy with predicting Chicago and LA in the test data, while having meaningful inaccuracies in predicting NYC vs. Houston:  
```{r, fig.height=9, fig.width=9, cache=TRUE}
...
Including deep soil temperature with deep soil moisture helps split Houston and NYC, driving overall accuracy above 95%:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

runFullRF(allCityTrain %>% mutate(fct_src=factor(src)) %>% filter(year<2022), 
          yVar="fct_src", 
          xVars=c("soil_moisture_100_to_255cm", "soil_temperature_100_to_255cm"), 
          dfTest=allCityTest %>% mutate(fct_src=factor(src)) %>% filter(year==2022), 
          isContVar=FALSE, 
          returnData=FALSE
          )
...
The model is re-run excluding soil moisture and soil temperature, which maintains high accuracy with test data:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

runFullRF(allCityTrain %>% mutate(fct_src=factor(src)) %>% filter(year<2022), 
          yVar="fct_src", 
          xVars=varsTrain[!grepl(pattern="moist|soil_temp", x=varsTrain)] %>% str_remove(., pattern="pct_"), 
          dfTest=allCityTest %>% mutate(fct_src=factor(src)) %>% filter(year==2022), 
          isContVar=FALSE, 
          returnData=FALSE
          )
...
There is generally strong separation of cities by pressure, though Houston and NYC are very similar:  
```{r, fig.height=9, fig.width=9}
...
The possible combinations of 2 variables are explored on a smaller dataset containing only Houston and NYC:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

# Train and test data
dfTrainCity <- allCityTrain %>% filter(src %in% c("Houston", "NYC"), year<2022) %>% mutate(fct_src=factor(src))
dfTestCity <- allCityTest %>% filter(src %in% c("Houston", "NYC"), year==2022) %>% mutate(fct_src=factor(src))
...
# Train and test data (all cities)
dfTrainCity <- allCityTrain %>% mutate(fct_src=factor(src)) %>% filter(year<2022)
dfTestCity <- allCityTest %>% mutate(fct_src=factor(src)) %>% filter(year==2022)

possLargeVars <- c("pressure_msl", 
                   "surface_pressure", 
                   "soil_temperature_100_to_255cm", 
                   "soil_moisture_100_to_255cm", 
                   "month"
