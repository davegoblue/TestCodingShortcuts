...
Location for final hand probabilities is updated:  
```{r, fig.height=9, fig.width=9}

# Mapping of data label to plot label
cmapHigh <- c("allHigh"="3. New high", 
              "tieHigh"="2. Ties previous high", 
              "none"="1. Below previous high"
              )
nmapMetric <- c("ovBonus"="3. Bonus", "ovCum"="1. Overall", "ovMain"="2. Main")
...
tcpSimAllTime %>%
    ...
    geom_text(data=~filter(., hn==100), 
              aes(label=round(value, 3), 
                  vjust=ifelse(name=="tieHigh", 1, 0), 
                  color=cmapHigh[name]
                  ), 
              hjust=1, 
              size=2.5
              ) +
    geom_line(aes(group=name, color=cmapHigh[name])) + 
    labs(x="Hand Number", 
         y="Proportion of simulations", 
         title="Likelihood of new overall maximum by hand number", 
         caption="Hand 1 not plotted (always a new high)"
    ...
    scale_color_discrete(NULL) + 
    theme(legend.position = "right") + 
    facet_wrap(~nmapMetric[metric]) + 
    theme(legend.position="bottom")

```

Probability for being at an all-time low for all three metrics is plotted:  
```{r, fig.height=9, fig.width=9}

# Mapping of data label to plot label
cmapLow <- c("allLow"="3. New low", 
             "tieLow"="2. Ties previous low", 
             "none"="1. Above previous low"
             )
nmapMetric <- c("ovBonus"="3. Bonus", "ovCum"="1. Overall", "ovMain"="2. Main")

# Probability for being at all-time low
tcpSimAllTime %>%
    count(name, hn, newmin, tiemin) %>% 
    group_by(name, hn) %>% 
    mutate(pct=n/sum(n), 
           type=case_when(newmin~"allLow", tiemin~"tieLow", TRUE~"none")
           ) %>% 
    ...
    geom_text(data=~filter(., hn==100), 
              aes(label=round(value, 3), 
                  vjust=ifelse(name=="tieLow", 1, 0), 
                  color=cmapLow[name]
                  ), 
              hjust=1, 
              size=2.5
              ) +
    geom_line(aes(group=name, color=cmapLow[name])) + 
    labs(x="Hand Number", 
         y="Proportion of simulations", 
         title="Likelihood of new overall minimum by hand number", 
         caption="Hand 1 not plotted (always a new low)"
...
The likelihood for a given value to be an all-time minimum is explored:  
```{r, fig.height=9, fig.width=9}
...
tcpSimAllTime %>%
    filter(hn==100) %>%
    count(name, value, newmin, tiemin) %>% 
    group_by(name, value) %>% 
    mutate(pct=n/sum(n), 
           type=case_when(newmin~"allLow", tiemin~"tieLow", TRUE~"none")
           ) %>% 
    ungroup() %>% 
    rename("metric"="name", "cumvalue"="value") %>%
    pivot_wider(id_cols=c("metric", "cumvalue"), 
                names_from="type", 
                values_from="pct", 
                values_fill=0
                ) %>%
    pivot_longer(cols=-c(metric, cumvalue)) %>% 
    ggplot(aes(x=cumvalue, y=value)) +
    geom_line(aes(group=name, color=cmapLow[name])) + 
    labs(x="Cumulative value (hand number 100)", 
         y="Proportion of simulations", 
         title="Likelihood of new overall minimum by cumulative value (hand 100)", 
         caption="Every simulation is 100 hands (Overall = 2*Main + Bonus)"
         ) + 
    scale_color_discrete(NULL) + 
    theme(legend.position = "right") + 
    facet_wrap(~nmapMetric[metric], scales="free_x") + 
    theme(legend.position="bottom")

```

Boxplots for cumulative minimum and maximum value at each hand number are created:  
```{r, fig.height=9, fig.width=9}

# Boxplot for cumulative value
tcpSimAllTime %>%
    select(metric=name, hn, cmax, cmin) %>%
    pivot_longer(cols=-c(metric, hn)) %>%
    group_by(metric, name, hn, value) %>% 
    summarize(n=n(), .groups="drop") %>%
    filter(metric=="ovMain", hn %in% c(1:10, 10*(2:10))) %>%
    ggplot(aes(x=factor(hn), y=value)) +
    geom_boxplot(aes(weight=n, fill=name)) + 
    labs(x="Hand number", 
         y="Cumulative max/min for Main", 
         title="Cumulative max/min distribution for Main by hand number", 
         subtitle="Plots first 10 hands, then every tenth hand",
         caption="Every simulation is 100 hands (Overall = 2*Main + Bonus)"
         ) +  
    facet_wrap(~c("cmax"="2. Cumulative Max (Main)", 
                  "cmin"="1. Cumulative Min (Main)"
                  )[name]) + 
