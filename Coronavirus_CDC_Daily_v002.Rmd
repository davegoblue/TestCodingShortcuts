...
cdc_daily_210801$plotDataList$dfFull %>% 
    filter(date==max(date)) %>% 
    mutate(estPop=100*vxc/vxcpoppct, 
           estPopgte18=100*vxcgte18/vxcgte18pct, 
           estPopgte65=100*vxcgte65/vxcgte65pct, 
           vxc1864=vxcgte18-vxcgte65, 
           vxc0017=vxc-vxcgte18, 
           vxc1864pct=100*vxc1864/(estPopgte18-estPopgte65), 
           vxc0017pct=100*vxc0017/(estPop-estPopgte18)
           ) %>% 
    select(state, date, ends_with("pct"), cluster, -vxcgte18pct) %>% 
    pivot_longer(-c(state, date, cluster)) %>% 
    ggplot(aes(x=fct_reorder(state, value, max), y=value)) + 
    geom_col(aes(fill=cluster)) + 
    geom_text(aes(y=value/2, label=paste0(round(value), "%")), size=4) +
    coord_flip() + 
    facet_wrap(~c("vxcpoppct"="4. All", "vxcgte65pct"="1. 65+", "vxc1864pct"="2. 18-64", "vxc0017pct"="3. 0-17")[name],
               nrow=1
               ) + 
    ...
    labs(x=NULL, 
         y="% fully vaccinated", 
         title="Fully vaccinated by state and age cohort (July 31, 2021)", 
         subtitle="Dashed lines are 35/70 percent"
         ) + 
    geom_hline(yintercept=c(35, 70), lty=2)
