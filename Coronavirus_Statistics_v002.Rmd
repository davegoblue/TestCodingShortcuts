...
The rules-based segmentation approach is updated to allow for more user control (defining the number of segments by dimension and setting a seed):  
```{r}
...
# Updateds to the clustering function
clusterStates <- function(df, 
                          caseVar="cpm", 
                          deathVar="dpm",
                          shapeFunc=lubridate::month, 
...
Further, the assessClusters() and helperMakeStateClusterData() functions are updated so they can order based on EITHER cpm or dpm based on function arguments:  
```{r}
...
The updated segments can be run using 3x3, and with a seed set:  
```{r}

# Create 3x3 clusters for hml and shape
popLoc <- "./RInputFiles/Coronavirus/covid_county_population_usafacts.csv"
cty_20201015_rule9_caseshvy <- readRunUSAFacts(maxDate="2020-10-13", 
                                               popLoc=popLoc, 
                                               caseLoc=caseLoc, 
                                               deathLoc=deathLoc, 
                                               dlCaseDeath=!(file.exists(caseLoc) & file.exists(deathLoc)),
...
The shapes of the segments are intriguing, with generally:  
  
1.  Early (Apr-Jun) disease in a classic pandemic curve  
2.  Middle (Jul-Sep) disease in a classic pandemic curve  
3.  Late disease that does not yet show the downslope of the pandemic curve  
  
Additionally, the "lower burden" version of each of the shapes shows more signs of rebounding than the "higher burdens" of the same shape.  Further exploration is a next step.
  
The overlap of death clusters and case cluster can again be explored:  
```{r}

# Run comparisons for cases and deaths
temp_caseDeath_003 <- examineCasevDeathClusters(caseList=cty_20201015_rule9_caseshvy, 
                                                deathList=cty_20201015_rule6, 
                                                ctyMin=5, 
                                                popMin=500000
                                                )
...
Overlaps are as expected, with good diferentiation of death volumes and shapes by death cluster and good differentiation of case volumes and shapes by case cluster.  And, the overlaps between the death cluster and case cluster are closely tied to the case fatality rate (CFR), at least in overlap buckets where there is "sufficient" data volume.

Suppose that the objective is to look for counties that have had significant increases in case volumes after September (the final data used for the shape portion of the segmentation):  
```{r}

octCases <- cty_20201015_rule9_caseshvy$clusterStateData %>%
    filter(!is.na(date)) %>%
    mutate(dateType = factor(ifelse(date >= "2020-10-01", "Oct", "Jan-Sep"), levels=c("Jan-Sep", "Oct"))) %>%
    group_by(fipsCounty, cluster, dateType) %>%
    summarize(cpm=sum(cpm), dpm=sum(dpm), pop=mean(pop))

octCases %>%
    pivot_wider(c(fipsCounty, cluster, pop), names_from="dateType", values_from=cpm) %>%
    ggplot(aes(x=`Jan-Sep`/1000, y=Oct/1000)) + 
    geom_point(aes(color=cluster, size=pop), alpha=0.25) + 
    geom_smooth(aes(group=cluster, color=cluster), se=FALSE, method="lm") + 
    labs(x="Cases per million (Jan-Sep), in thousands", 
         y="Cases per million (Oct), in thousands", 
         title="Cases per million by cluster and time period (000s)"
         ) +
...
octCases %>%
    ...
    labs(x="Cases per million (Jan-Sep), in thousands", 
         y="Cases per million (Oct), in thousands", 
         title="Cases per million by cluster and time period (000s)", 
         subtitle="Excludes counties < 25k population"
         ) +
    facet_wrap(~cluster)
...
octCases %>%
    ...
    ggplot(aes(x=`Jan-Sep`/1000, y=Oct/1000)) + 
    geom_point(aes(color=cluster, size=pop), alpha=0.25) + 
    geom_smooth(aes(group=cluster, color=cluster, weight=pop), se=FALSE, method="lm") +
    geom_smooth(aes(weight=pop), se=FALSE, method="lm", color="black", lty=2) + 
    labs(x="Cases per million (Jan-Sep), in thousands", 
         y="Cases per million (Oct), in thousands", 
         title="Cases per million by cluster and time period (000s)", 
         subtitle="Excludes counties < 25k population", 
         caption="Smooths driven by lm, with points weighted by population"
         )

```
  
In general, higher impacted elements in Jan-Sep within a given cluster tend to be higher impacted in October also.  Suppose that counties with at least 10,000 cases per million through mid-October are of interest:  
```{r}

octCases %>%
    filter(dateType=="Oct", cpm >= 10000) %>%
    group_by(cluster) %>%
    summarize(n=n(), cpm=sum(cpm*pop)/sum(pop), dpm=sum(dpm*pop)/sum(pop), mean_pop=mean(pop))

```
  
By and large, counties with large "spikes" in early October tend to be the small counties where it is easy to get a spike with just a few cases.  There are a handful of larger counties that also meet this threshold.

One common threshold used is 150 cases per million.  Since the October data extends through October 13, this would be roughly 2000 cases per million.  Suppose that a "spike" is defined as a pace that is at least double this threshold:  
```{r}

octCases %>%
    filter(dateType=="Oct") %>%
    group_by(cluster) %>%
    summarize(nTotal=n(), n_gt4k=sum(cpm>=4000), pct_gt4k=n_gt4k/nTotal,
              cpm_gt4k=sum(cpm*pop*(cpm>=4000))/sum(pop*(cpm>=4000)), 
              dpm_gt4k=sum(dpm*pop*(cpm>=4000))/sum(pop*(cpm>=4000)), 
              mean_pop_total=mean(pop), mean_pop_gt4k=sum(pop*(cpm>=4000))/sum(cpm>=4000)
              ) %>%
    arrange(-pct_gt4k)

```
  
There is significant variation in the likelihood of a county experiencing ~300 cases per million per day during early October:  
  
* A majority of the counties experiencing heavy and late disease  
* Roughly half of the counties experiencing moderate and late case burden  
* Roughly a quarter of small counties (<25k population) and counties that experienced early and heavy burden  
* Roughly 15% of counties experience light and late case burden and counties that experienced heavy and mid-period case burden  
* Few counties that experienced mild-moderate burden either early or mod-term  
  
In general, population for counties experiencing high disease burden in early October is less than average population for counties in the cluster.  
  
These counties can be plotted on the US map:  
```{r}

octCases %>%
    filter(dateType=="Oct", cpm >= 4000) %>%
    select(fips=fipsCounty, cluster) %>%
    usmap::plot_usmap(regions="counties", data=., values="cluster") + 
    scale_fill_discrete("Cluster", na.value="lightgrey") + 
    labs(title="Counties averaging 300+ cases per million per day (October 1-13)")

```
  
There is a strong geographic clustering of the high-case counties which is suggestive of prevalence increase rather than spurious spikes based solely on small denominators.  Next steps are to refresh that data and see how burden by segment has evolved through mid-late October.

The three main data downloads are run again:  
```{r cache=TRUE}

# Use existing segments with updated data
locDownload <- "./RInputFiles/Coronavirus/CV_downloaded_201024.csv"
test_old_201023 <- readRunCOVIDTrackingProject(thruLabel="Oct 23, 2020", 
                                               downloadTo=if (file.exists(locDownload)) NULL else locDownload,
                                               readFrom=locDownload, 
                                               compareFile=test_old_201014$dfRaw,
                                               useClusters=readFromRDS("test_hier5_201001")$useClusters
                                               )

```
  
Updated USA Facts (county-level) data:  
```{r cache=TRUE}

# Data downloaded 24-OCT-2020
caseLoc <- "./RInputFiles/Coronavirus/covid_confirmed_usafacts_downloaded_20201024.csv"
deathLoc <- "./RInputFiles/Coronavirus/covid_deaths_usafacts_downloaded_20201024.csv"

# Use existing segments with different data (already downloaded)
old_20201024 <- readRunUSAFacts(maxDate="2020-10-22", 
                                popLoc="./RInputFiles/Coronavirus/covid_county_population_usafacts.csv", 
                                caseLoc=caseLoc, 
                                deathLoc=deathLoc, 
                                dlPop=!file.exists(popLoc),
                                dlCaseDeath=!file.exists(caseLoc) & !file.exists(deathLoc),
                                oldFile=cty_20201015_rule9_caseshvy$burdenData, 
                                existingStateClusters=test_old_201023$useClusters,
                                existingCountyClusters=cty_20201015_rule9_caseshvy$clustVec,
                                createClusters=FALSE, 
                                orderCluster="cpm"
                                )
...
Updated CDC all-cause deaths data:  
```{r cache=TRUE}

# Use data that have previously been downloaded
cdcLoc <- "Weekly_counts_of_deaths_by_jurisdiction_and_age_group_downloaded_20201024.csv"
cdcList_20201024 <- readRunCDCAllCause(loc=cdcLoc, 
                                       startYear=2015, 
                                       curYear=2020,
                                       weekThru=35, 
                                       startWeek=9, 
...
At a glance, findings are in line with those from earlier in the month.  Next steps are to create a new version that includes the cleaned versions of the functions and appropriately saved analysis files.
