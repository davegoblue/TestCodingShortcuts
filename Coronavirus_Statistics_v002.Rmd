...
Next, the USA Facts process is updated to be run using a single function, including the capability to download new data:  
```{r}
...
readRunUSAFacts <- function(maxDate, 
    ...
    # STEP 1: Read in the relevant data files
    rawUSAFacts <- readPivotUSAFacts(popFile=popLoc, 
                                     caseFile=caseLoc, 
                                     deathFile=deathLoc, 
    ...
    # STEP 2: Read case and death data (redundant), combine, and add population totals and existing clusters
    ...
    # STEP 6: Run state-level summary if requested
    if (!is.null(keyStates)) {
        stateCountySummary(states=keyStates,
                           df=changeOrderLabel(clusterStateData, grpVars="fipsCounty"),
                           keyDate=maxDate,
                           showQuadrants=TRUE, 
                           showCumulative=TRUE, 
                           facetCumulativeByState=TRUE, 
    ...
    # STEP 7: Assess lags for largest counties and counties with highest death rates
    # STEP 7a: Save data for top-100 counties by death rate (filtered to only counties of 100k+ population)
    top100_dpm <- exploreTopCounties(helperACC_county, 
                                     minDate="2020-03-01", 
                                     maxDate=maxDate, 
                                     minPop=100000,
                                     nVar="dpm", 
    ...
    # STEP 7b: Top-3 by population by cluster with plots
    exploreTopCounties(helperACC_county, 
                   minDate="2020-03-01", 
                   maxDate=maxDate, 
                   topNBy="cluster",
                   nVar="pop", 
...
identical(tmpCty$top100_dpm, top100_dpm_20201003)
