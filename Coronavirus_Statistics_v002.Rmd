...
helperKeyStateExcessPlots <- function(df, 
    ...
    # STEP 0: Create a general plotting database
    plotData <- df %>%
        filter(year==curYear) %>%
        mutate(quarter=factor(paste0("Q", ceiling(week/13), "-", curYear))) %>%
...
helperKeyStateExcessPlots(stateAgg_20200923, subT="CDC data through July 2020 (Q3 incomplete)")

```
  
Next steps are to continue working through the excess deaths by state plots, then convert the excess deaths by age and age/state to functional form:  
```{r eval=FALSE}
