...
readRunCDCDaily <- function(thruLabel, 
    ...
    dfRawList <- vector("list", length=length(elemUsed)) %>% 
        purrr::set_names(elemUsed)
    dfProcessList <- vector("list", length=length(elemUsed)) %>%
        purrr::set_names(elemUsed)
    ...
    if (is.null(dfPerCapita)) {
        ...
        # Step 4: Integrate in to a dfPerCapita file
        dfPerCapita <- createPerCapita(dfProcessList, 
                                       uqBy=c("state", "date"), 
                                       popData=stateData
                                       )
        
        # Glimpse the per capita data file
        if (is.null(writeLog)) glimpseFile(dfPerCapita, txt="\nPer capita data file:\n")
        else capture.output(glimpseFile(dfPerCapita, txt="\nPer capita data file:\n"), 
                            file=writeLog, 
                            append=TRUE
