...
checkUniqueRows <- function(df, 
    ...
    # uniqueVars: combination of variables for checking uniqueness (NULL means use all)
...
The function is then run for totals by date and state:  
```{r, fig.height=9, fig.width=9}
...
The data sources appear to be well aligned.  Next steps are to add the capability to print records that differ by more than a specified tolerance.
