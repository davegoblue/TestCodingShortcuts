...
specNAweight <- function(fn=weighted.mean, wt) {
    function(x, ...) if (any(!is.na(x))) fn(x, w=get(wt), ..., na.rm=TRUE) else NA
}
...
createGroupAgg <- function(df, 
    ...
    # vecSelect: the elements of df to be kept (NULL means keep all)
    ...
    # If a list for vecSelect has been passed, then:
    # 1. limit aggVars to elements in vecSelect
    # 2. keep only requested variables (vecSelect and aggTo)
    if (!is.null(vecSelect)) {
        aggVars <- base::intersect(aggVars, vecSelect)
        df <- colSelector(df, vecSelect=reduce(list(aggTo, 
                                                    vecSelect, 
                                                    if(is.null(wtVar)) c() else wtVar
                                                    ), 
                                               .f=base::union
                                               )
                          )
    ...
    # If the function has no weighting variable, summarize using aggFunc
    # If no weighting is requested, group by aggTo then summarize aggVars using function aggFunc
    if (is.null(wtVar)) dfGroup <- dfGroup %>% 
        summarize(across(.cols=all_of(aggVars), .fns=aggFunc), .groups="drop")
    else dfGroup <- dfGroup %>%
        summarize(across(.cols=all_of(aggVars), 
                         .fns=~weighted.mean(.x, w=get(wtVar), na.rm=TRUE)
                         ), 
                  .groups="drop"
                  )
...
dfAgg <- vecToTibble(cdc_daily_test_v2$useClusters, colNameData="cluster", colNameName="state") %>%
    ...
    createGroupAgg(aggTo=c("cluster", "date"), 
                   ...
                   vecSelect=c("tcpm7", "tdpm7", "cpm7", "dpm7", "hpm7")
                   )
...
dfAgg_wm <- vecToTibble(cdc_daily_test_v2$useClusters, colNameData="cluster", colNameName="state") %>%
    joinFrames(cdc_daily_test_v2$stateData, cdc_daily_test_v2$dfPerCapita, keyJoin="state") %>%
    createGroupAgg(aggTo=c("cluster", "date"), 
                   aggFunc=NA, # not relevant since wtVar will drive weighted.mean
                   vecSelect=c("tcpm7", "tdpm7", "cpm7", "dpm7", "hpm7"), 
                   wtVar="pop"
                   )
