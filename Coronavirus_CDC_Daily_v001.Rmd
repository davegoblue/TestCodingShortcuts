...
helperClusterSummaryPlots <- function(dfState, 
    ...
    print(c("cluster", "date", unname(p4Vars)))
    print(c("cluster", "date", names(p4Vars)))
    p4 <- dfPlot %>%
...
readRunCOVIDTrackingProject <- function(thruLabel, 
    ...
    # STEP 6: Create the cluster assessments
    # p4Def <- formals(assessClusters)$p4Vars  # Need to fix, pulls wrong formals
    p4Def <- c(cases="cases", deaths="deaths")
    plotData <- assessClusters(useClusters, 
                               dfState=stateData, 
                               dfBurden=dfPerCapita,
                               thruLabel=thruLabel,
                               ...
                               p4Fun=if(isCDCDaily) max else formals(assessClusters)$p4Fun, 
                               isCDC=isCDCDaily
                               )
    ...
    if (makeCumulativePlots) {
        consPos <- consolidatedPlotData %>%
            ...
            pivot_longer(-c(state, cluster, date), values_to="vpm7") %>%
            filter(!is.na(vpm7))
        typeKeep <- formals(makeCumulative)$typeKeep
        ...
        plotCumulativeData(clCum, keyMetricp2=a[1], flagsp2=findFlagStates(clCDCCum, keyMetricVal=a[1]))
        plotCumulativeData(clCum, keyMetricp2=a[2], flagsp2=findFlagStates(clCDCCum, keyMetricVal=a[2]))
        if(!isTRUE(isCDCDaily)) {
...
Next steps are to update the process so that it works with the old CTP data (defaults and formals are not quite aligned).
```{r eval=FALSE}
