...
helperReadRunCTP_steps02_04 <- function(downloadTo, 
    ...
    dfFiltered <- processCVData(dfRaw, varsKeep=varsFilter, varsRename=varsRename)
...
readRunCOVIDTrackingProject <- function(thruLabel, 
    ...
    # File processing stops at this point for now, continue steps 5-8 later
    return(dfPerCapita)
...
The function is then run, stopping after step 4 based on an artificial return statement:  
```{r}
...
# Test steps 1-4 for CDC daily data (processing prior to segmentation)
readRunCOVIDTrackingProject(thruLabel="Apr 14, 2021", 
                            readFrom="./RInputFiles/Coronavirus/CDC_dc_downloaded_210414.csv", 
                            compareFile=readFromRDS("cdcDaily_hier7_210414")$dfRaw %>%
                                mutate(date=lubridate::mdy(submission_date)), 
                            isCDCDaily=TRUE
                            )

# Confirm that process still works for final CTP data
readRunCOVIDTrackingProject(thruLabel="Mar 31, 2021", 
...
The process can still work with the old CTP data and also appears to work with the CDC data.
