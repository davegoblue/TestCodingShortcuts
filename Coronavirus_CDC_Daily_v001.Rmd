...
compareAggregate <- function(df, 
    ...
    dfAgg <- df %>%
        checkControl(groupBy=grpVar, useVars=numVars, printControls=FALSE)
    refAgg <- ref %>%
        checkControl(groupBy=grpVar, useVars=numVars, printControls=FALSE) %>%
...
checkControlMapper <- list("cdcDaily"=list("group"="date", "numVars"=c("new_cases", "new_deaths")), 
                           "cdcHosp"=list("group"="date", "numVars"=c("inp", "hosp_adult", "hosp_ped"))
                           )
...
keyAggMapper <- list("l1"=list("grpVar"="date", 
                               "numVars"=c("new_cases", "new_deaths", "tot_cases", "tot_deaths"), 
                               "sameUniverse"=NA,
                               "plotData"=TRUE, 
                               "isLine"=TRUE,
                               "returnDelta"=TRUE, 
...
readQCRawFile <- function(fileName, 
    ...
    # ovrWriteFilename: boolean, if fileName already exists, should it be overwritten?
    # vecRename: vector for renaming c('existing name'='new name'), can be any length from 0 to ncol(df)
    ...
    # Step 0: If url is null, set value using urlMapper[urlType]
    if (is.null(url) & !is.null(urlType)) url <- unname(urlMapper[urlType])
    ...
    # Step 2a: Get vecRename, selfList, and uniqueBy if NULL
    if (is.null(vecRename)) vecRename <- if (urlType %in% names(renMapper)) renMapper[[urlType]] else c()
    if (is.null(selfList)) 
        selfList <- if(urlType %in% names(selfListMapper)) selfListMapper[[urlType]] else list()
    if (is.null(fullList)) 
        fullList <- if(urlType %in% names(fullListMapper)) fullListMapper[[urlType]] else list()
    if (is.null(uniqueBy)) uniqueBy <- if(urlType %in% names(uqMapper)) uqMapper[[urlType]] else NULL
    ...
    if (is.null(step3Group)) step3Group <- checkControlMapper[[urlType]][["group"]]
    if (is.null(step3Vals)) step3Vals <- checkControlMapper[[urlType]][["numVars"]]
    dfRaw %>%
    ...
    # Step 4a: Get the key variable mapper
    if (is.null(step4KeyVars)) step4KeyVars <- checkSimilarityMapper[[urlType]]
    # Step 4b: Check similarity of existing and reference file
    diffRaw <- checkSimilarity(df=dfRaw, ref=dfRef, keyVars=step4KeyVars)

    # Step 5: Plot the similarity checks
    if (is.null(step5PlotItems)) step5PlotItems <- plotSimilarityMapper[[urlType]]
    ...
    lapply(keyAggMapper, FUN=helperAggMap)
