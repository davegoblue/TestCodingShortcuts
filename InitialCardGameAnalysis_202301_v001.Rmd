...
A function is written to output hand types by cards excluded:  
```{r, fig.height=9, fig.width=9}

handTypesByExclusion <- function(exclCards=c(), df=sortedHandRanks, pivotWider=FALSE, keyVars="wType") {
...
# Example for no exclusions
handTypesByExclusion(exclCards=c())

# Example for joker exclusions
handTypesByExclusion(exclCards=c(53), pivotWider=TRUE)

# Example for AKQJ9 as 4-1-0-0 (9 as separate suit)
handTypesByExclusion(exclCards=c(1, 13, 12, 11, 22), pivotWider=TRUE)

# Example for AKQJ9 as 2-1-1-1 (A and 9 as same suit)
handTypesByExclusion(exclCards=c(1, 26, 38, 50, 9), pivotWider=TRUE)

# Example for stacking hands
handTypesByExclusion(exclCards=c(1, 13, 12, 11, 22)) %>%
    bind_rows(handTypesByExclusion(exclCards=c(1, 26, 38, 50, 9)), .id="src") %>%
    mutate(src=c("1"="4-1-0-0", "2"="2-1-1-1")[src]) %>%
    filter(isOut=="avail") %>%
    pivot_wider(id_cols=c(wType, isOut), names_from="src", values_from="n") %>%
    mutate(delta=`2-1-1-1`-`4-1-0-0`)

# Example for stacking hands with wilds tracked
handTypesByExclusion(exclCards=c(1, 13, 12, 11, 22), keyVars=c("wType", "nWild")) %>%
    bind_rows(handTypesByExclusion(exclCards=c(1, 26, 38, 50, 9), keyVars=c("wType", "nWild")), .id="src") %>%
    mutate(src=c("1"="4-1-0-0", "2"="2-1-1-1")[src]) %>%
    filter(isOut=="avail") %>%
    mutate(hasWild=(nWild>0)) %>%
    group_by(src, wType, isOut, hasWild) %>%
    summarize(n=sum(n), .groups="drop") %>%
    pivot_wider(id_cols=c(wType, hasWild, isOut), names_from="src", values_from="n", values_fill=0) %>%
    mutate(delta=`2-1-1-1`-`4-1-0-0`) %>%
    arrange(desc(hasWild), wType)

```
  
The difference between AKQ93 and AKQ87, each of type 2-1-1-1 with the A and K same-suited, is explored:  
```{r, fig.height=9, fig.width=9}

# Example for AKQ93 and AKQ87
handTypesByExclusion(exclCards=c(1, 13, 25, 35, 42), keyVars=c("wType", "nWild")) %>%
    bind_rows(handTypesByExclusion(exclCards=c(1, 13, 25, 34, 46), keyVars=c("wType", "nWild")), .id="src") %>%
    mutate(src=c("1"="AKQ93", "2"="AKQ87")[src]) %>%
    filter(isOut=="avail") %>%
    mutate(hasWild=(nWild>0)) %>%
    group_by(src, wType, isOut, hasWild) %>%
    summarize(n=sum(n), .groups="drop") %>%
    pivot_wider(id_cols=c(wType, hasWild, isOut), names_from="src", values_from="n", values_fill=0) %>%
    mutate(delta=`AKQ87`-`AKQ93`) %>%
    arrange(desc(hasWild), wType)

```
  
Because either hand loses to anything with a wild, the change in available non-wild hand types is key. There are two changes in hand availability for AKQ87 vs. AKQ93:  
  
* 4 straight flushes become flushes (zero-pair hand loses to either)  
* 876 straights become zero-pair hands (zero-pair hand can potentially win against these)  
  
Even though AKQ93 outranks AKQ87, the latter's ability to block additional straights makes it slightly more likely to win. The impact is modest, with under 1000 straights blocked out of around 1 million possible zero-wild hands (excluding the 5 wilds and 5 cards in the relevant hand)
  
The zero-pair hand with the greatest range of EV by suit distribution is identified and explored:  
```{r, fig.height=9, fig.width=9}

# Rank with largest range based on suit distribution
maxRng <- noPairEVSuits %>% 
    filter(rngEV==max(rngEV))
maxRng

# Hands driving this difference
maxMinHands <- fullHandProbsData %>% 
    filter(rank==4707) %>% 
    filter(pWNoTie %in% c(max(pWNoTie), min(pWNoTie)))
maxMinHands

# Cards in the maximum and minimum hands
maxMinCards <- tblHands %>% 
    semi_join(select(maxMinHands, "handNum"), by="handNum")
maxMinCards

# Hand types blocked by (3457)sAh and (A7)s5h4d3c
handTypesByExclusion(exclCards=maxMinCards[1,1:5] %>% as.matrix() %>% as.vector(), 
                     keyVars=c("wType", "nWild")
                     ) %>%
    bind_rows(handTypesByExclusion(maxMinCards[2,1:5] %>% as.matrix() %>% as.vector(), 
                                   keyVars=c("wType", "nWild")), 
              .id="src"
              ) %>%
    mutate(src=c("1"="2-1-1-1", "2"="4-1-0-0")[src]) %>%
    filter(isOut=="avail") %>%
    mutate(hasWild=(nWild>0)) %>%
    group_by(src, wType, isOut, hasWild) %>%
    summarize(n=sum(n), .groups="drop") %>%
...
A main difference as before is the conversion of 458 non-wild flushes to non-pairing hands. The likelihood of winning with type 99 is explored next. The function to output hand types by cards excluded is updated:  
```{r, fig.height=9, fig.width=9}
...
# Example for stacking hands with wilds tracked
handTypesByExclusion(exclCards=c(1, 13, 12, 11, 22), keyVars=c("wType", "nWild"), checkRank=4380) %>%
    bind_rows(handTypesByExclusion(exclCards=c(1, 26, 38, 50, 9), keyVars=c("wType", "nWild"), checkRank=4380), 
              .id="src"
              ) %>%
    mutate(src=c("1"="4-1-0-0", "2"="2-1-1-1")[src]) %>%
    filter(isOut=="avail") %>%
    mutate(hasWild=(nWild>0)) %>%
    group_by(src, wType, isOut, hasWild, rankResult) %>%
    summarize(n=sum(n), .groups="drop") %>%
    pivot_wider(id_cols=c(wType, hasWild, isOut, rankResult), names_from="src", values_from="n", values_fill=0) %>%
    mutate(delta=`2-1-1-1`-`4-1-0-0`) %>%
    arrange(desc(hasWild), wType) %>%
    print(n=30)

# Example for stacking hands with wilds tracked and win/loss tracked
handTypesByExclusion(exclCards=c(1, 13, 12, 11, 22), keyVars=c("wType", "nWild"), checkRank=4380) %>%
    bind_rows(handTypesByExclusion(exclCards=c(1, 26, 38, 50, 9), keyVars=c("wType", "nWild"), checkRank=4380), 
              .id="src"
              ) %>%
...
In the case of hand AKQJ9, converting from (AKQJ)s9h to AsKhQdJc9s results in 458 fewer flushes, with 2 of these becoming ties and 456 of these becoming wins. With 4-1-0-0, only 2 of the four possible AKQJ9 flush are blocked, while with 2-1-1-1, all 4 of four AKQJ9 flush are blocked. Every other conversion from flush to non-pair convertes AKQJ9 (strongest non-pair hand) to a winner
  
The zero-pair hand with the greatest range of EV by suit distribution is explored again:  
```{r, fig.height=9, fig.width=9}
...
As before, two of the four A7543 hands are converted from flush to tie as the suit distribution goes from 4-1-0-0 (blocks two of four flushes by cards of same rank) to 2-1-1-1 (blocks all four flushes by cards of same rank). The reduction of 458 flushes is less valuable to the player, since 61 of the unpaired hands beat the player's A7543 while 395 of the unpaired hands lose to the player's A7543. As the player's hand gets weaker, the benefit of blocking straights and flushes is reduced, since it is less likely the player can beat the spot cards.

A function is written to automate the process including the win/loss/tie component:  
```{r, fig.height=9, fig.width=9}
...
# Checking for A7543
compareHandExclusion(c1=c(1, 7, 18, 30, 42), 
                     c2=c(3, 4, 5, 7, 14), 
                     checkRank=maxRng$rank, 
                     labs=c("2-1-1-1", "4-1-0-0")
                     ) %>%
    print(n=30)

# Checking for AKQJTs vs 5-wild
compareHandExclusion(c1=c(1, 10, 11, 12, 13), 
                     c2=c(2, 15, 28, 41, 53), 
                     checkRank=NULL, 
                     labs=c("RF", "5W")
                     ) %>%
...
The function appears to work as intended. The two forms of JJ543 are explored:  
```{r, fig.height=9, fig.width=9}

exmpl_jj543 <- fullHandProbsData %>% 
    filter(wType==11, tb1==11, tb3==5) %>% 
    group_by(nWild) %>%
    filter(pWNoTie==max(pWNoTie)) %>%
    filter(handNum==max(handNum)) %>%
    ungroup() %>%
    select(handNum, type, rank, nWild, everything())
exmpl_jj543

# Checking for JJ543 vs 2J543
tmpOut <- compareHandExclusion(c1=mtxHands[exmpl_jj543 %>% filter(nWild==0) %>% pull(handNum), , drop=TRUE], 
                               c2=mtxHands[exmpl_jj543 %>% filter(nWild==1) %>% pull(handNum), , drop=TRUE], 
                               checkRank=exmpl_jj543 %>% slice(1) %>% pull(rank), 
                               labs=c("JJ543", "2J543")
                               )
tmpOut %>% print(n=30)

# Summaries of shifts in hands
tmpOut %>% select(-wType) %>% group_by(rankResult) %>% summarize(across(where(is.numeric), sum))
tmpOut %>% 
    mutate(deltaType=case_when(delta<0~"neg", delta==0~"nil", delta>0~"pos")) %>%
    select(-wType) %>% 
    group_by(rankResult, deltaType) %>% 
    summarize(across(where(is.numeric), sum))

```
  
The non-wild JJKQ3 and JJAT9 are explored:  
```{r, fig.height=9, fig.width=9}

exmpl_jjhigh <- fullHandProbsData %>% 
    filter(wType==11, tb1==11, (tb3==13 & tb4==12 & tb5==3) | (tb3==14 & tb4==10 & tb5==9)) %>% 
    group_by(rank) %>%
    filter(pWNoTie==max(pWNoTie)) %>%
    filter(handNum==max(handNum)) %>%
    ungroup() %>%
    select(handNum, type, tb3, rank, nWild, everything())
exmpl_jjhigh

# Checking for JJAT9 vs JJKQ3
tmpOut <- compareHandExclusion(c1=mtxHands[exmpl_jjhigh %>% filter(tb3==14) %>% pull(handNum), , drop=TRUE], 
                               c2=mtxHands[exmpl_jjhigh %>% filter(tb3==13) %>% pull(handNum), , drop=TRUE], 
                               checkRank=exmpl_jjhigh %>% arrange(desc(tb3)) %>% pull(rank), 
                               labs=c("JJAT9", "JJKQ3")
                               )
tmpOut %>% print(n=30)
...
The highest ranked ten-high hand with and without wild (2T985 and TTAKQ) are explored:  
```{r, fig.height=9, fig.width=9}

exmpl_tt <- fullHandProbsData %>% 
    filter(wType==11, tb1==10, (tb5==12 | (nWild==1 & tb5==5))) %>% 
    group_by(nWild) %>%
    filter(pWNoTie==max(pWNoTie)) %>%
    filter(handNum==max(handNum)) %>%
    ungroup() %>%
...
# Checking for TTAKQ vs 2T985
tmpOut <- compareHandExclusion(c1=mtxHands[exmpl_tt %>% filter(nWild==0) %>% pull(handNum), , drop=TRUE], 
                               c2=mtxHands[exmpl_tt %>% filter(nWild==1) %>% pull(handNum), , drop=TRUE], 
                               checkRank=exmpl_tt %>% slice(1) %>% pull(rank), 
                               labs=c("TTAKQ", "2T985")
                               )
tmpOut %>% print(n=30)
...
Every one-pair hand of type TT is more likely to lose than to win. Hands of type AKJ7x are explored:  
```{r, fig.height=9, fig.width=9}

exmpl_akj <- fullHandProbsData %>% 
    filter(wType==99, tb1==14, tb2==13, tb3==11, tb4==7, (tb5==4 | tb5==3)) %>% 
    group_by(tb5) %>%
    filter(pWNoTie==max(pWNoTie)) %>%
    filter(handNum==max(handNum)) %>%
    ungroup() %>%
...
# Checking for AKJ74 vs AKJ73
tmpOut <- compareHandExclusion(c1=mtxHands[exmpl_akj %>% filter(tb5==3) %>% pull(handNum), , drop=TRUE], 
                               c2=mtxHands[exmpl_akj %>% filter(tb5==4) %>% pull(handNum), , drop=TRUE], 
                               checkRank=exmpl_akj %>% arrange(tb5) %>% pull(rank), 
                               labs=c("AKJ73", "AKJ74")
)
tmpOut %>% print(n=30)
...
Hands of type QQ543 and 2Q543 are explored are explored:  
```{r, fig.height=9, fig.width=9}

exmpl_qq <- fullHandProbsData %>% 
    filter(wType==11, tb1==12, tb3==5) %>% 
    group_by(nWild) %>%
    filter(pWNoTie==max(pWNoTie)) %>%
    filter(handNum==max(handNum)) %>%
    ungroup() %>%
...
# Checking for QQ543 vs 2Q543
tmpOut <- compareHandExclusion(c1=mtxHands[exmpl_qq %>% filter(nWild==0) %>% pull(handNum), , drop=TRUE], 
                               c2=mtxHands[exmpl_qq %>% filter(nWild==1) %>% pull(handNum), , drop=TRUE], 
                               checkRank=exmpl_qq %>% arrange(nWild) %>% pull(rank), 
                               labs=c("QQ543", "2Q543")
)
tmpOut %>% print(n=30)
