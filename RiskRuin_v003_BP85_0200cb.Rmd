...
* Standard Bonus Poker 8/5 pay table  
...
The start time is calculated to understand overall performance.  The 8/5 Jacks or Better file is read in, using raw data from <http://wizardofodds.com/games/video-poker/strategy/bonus-poker/8-5/>.  Further, the cash back component is set to be 2.0%:  
```{r}
startTime <- proc.time()

## Step 1: Read in the outcomes file
baseOutcomes <- getBaseOutcomes(myFileName="BP_85_Probs.csv", forceEQ=TRUE)
...
####_Conclusions_  
The results suggest that assuming a 2.0% loss rate in a 8/5 BP game, it is unlikely that actual results will ever fall more than ~4k units behind that benchmark.  The simulations match up reasonably well with the theory, suggesting that the "fake" cash back as solved by the polynomial is a plausible representation of the real world.  
