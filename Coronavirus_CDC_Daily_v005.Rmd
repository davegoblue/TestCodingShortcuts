...
updatedFindPeaks <- function(x,
    ...
    xLeft <- x-zoo::rollapply(lag(x, 1), width=lookLeft, FUN=FUN, fill=NA, align="right")
    
    # Run the look to the right, unless lookRight is 0
    if(lookRight==0) xRight <- rep(0, length(x))
    else xRight <- x-zoo::rollapply(lead(x, 1), width=lookRight, FUN=FUN, fill=NA, align="left")
    ...
    # Return the boolean vector (only enabled for MAX for now)
    if(identical(FUN, max)) !is.na(xLeft) & !is.na(xRight) & (xLeft>0) & (xRight>=0)
...
The methodologies find the same peaks, so overplotting is not caused by findPeaks. The function should still be updated for better behavior with ties, with ability to find minima as well as maxima in the update. The overplotting issue needs further exploration as well
