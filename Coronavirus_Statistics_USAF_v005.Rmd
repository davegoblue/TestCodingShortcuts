...
An existing processed USA Facts list is loaded for use as the comparison set.  The countyFIPS used to be stored as a numeric and is treated as zeroPad5 in the new version.  The relevant comparison data files are updated to reflect this:  
```{r}

cty_old_20210104 <- readFromRDS("cty_old_20210104")

# Population data
cty_old_20210104$pop <- cty_old_20210104$pop %>% 
    colMutater(selfList=list("countyFIPS"=zeroPad5)) %>%
    checkUniqueRows(uniqueBy=c("countyFIPS", "state", "countyName"))

# Burden data
cty_old_20210104$burdenData <- cty_old_20210104$burdenData %>% 
    colMutater(selfList=list("countyFIPS"=zeroPad5)) %>%
    checkUniqueRows(uniqueBy=c("countyFIPS", "date"))

# Clustering output vector
names(cty_old_20210104$clustVec) <- zeroPad5(names(cty_old_20210104$clustVec))
cty_old_20210104$clustVec %>%
    vecToTibble(colNameName="cluster") %>%
    checkUniqueRows(uniqueBy=c("cluster"), returnDF=FALSE)

# Per capita data files
cty_old_20210104$clusterData$countyFiltered <- cty_old_20210104$clusterData$countyFiltered %>% 
    colMutater(selfList=list("state"=zeroPad5)) %>%
    checkUniqueRows(uniqueBy=c("state", "date"))
cty_old_20210104$clusterData$countyDailyPerCapita <- cty_old_20210104$clusterData$countyDailyPerCapita %>% 
    colMutater(selfList=list("state"=zeroPad5)) %>%
    checkUniqueRows(uniqueBy=c("state", "date"))
cty_old_20210104$clusterData$countyCumPerCapita <- cty_old_20210104$clusterData$countyCumPerCapita %>% 
    colMutater(selfList=list("state"=zeroPad5)) %>%
    checkUniqueRows(uniqueBy=c("state", "date"))
...
sparseCountyClusterMap(cty_old_20210104$clustVec, 
                       brewPalette="Paired", 
...
Next steps are to load new data, compare against previous, and create new segments.
