...
Mapss are created for burden by date:  
```{r fig.height=9, fig.width=9}

dfTempBurden <- as.Date("2022-02-28") %>% lubridate::add_with_rollback(months(c(0, -2, -4, -6))) %>%
    purrr::map_dfr(.f=~makeBurdenSummary(cty_newdata_20220308, keyDate=.x, dateRange=35))
dfTempBurden
...
dfTempBurden %>%
    rename(fips=countyFIPS) %>%
    mutate(burden=pmax(pmin(dpm35, 500), 0)) %>%
    select(fips, burden, asofDate) %>%
    usmap::plot_usmap(regions="counties", data=., values="burden") + 
    labs(title="5-week coronavirus deaths by county", 
         subtitle="Floors and ceilings applied", 
...
The mapping process is converted to functional form:  
```{r fig.height=9, fig.width=9}
...
identical(dfTempBurden, createBurdenCountyDate(cty_newdata_20220308, maxDate="2022-02-28"))
makeBurdenDatePlot(dfTempBurden, keyVar="cpm35", timeLabel="5-week", varCeiling=30000, varDivBy=1000)
makeBurdenDatePlot(dfTempBurden, keyVar="dpm35", timeLabel="5-week", varCeiling=500)
