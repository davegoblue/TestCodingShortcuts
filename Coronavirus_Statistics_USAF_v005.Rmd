...
countyCorr <- function(dfCaseDeath, 
                       dfVax,
    ...
    # FUNCTION ARGUMENTS:
    # dfCaseDeath: data frame for county-level cases and deaths
    # dfVax: data frame for county-level vaccines
    ...
    dfFull <- dfVax %>%
        ...
        colRenamer("vaxMetric" %>% purrr::set_names(all_of(vaxVar))) %>%
        select(fips=FIPS, countyName, pop, state, vaxMetric) %>%
        full_join(dfCaseDeath, by="fips") %>%
        filter(!is.na(vaxMetric)) %>%
        mutate(type=case_when(pop>=500000 ~ ">500k",
                              pop>=100000 ~ "100k-500k",
                              pop>=25000 ~ "25k-100k",
                              TRUE ~ "<25k"
    ...
    p1 <- dfFull %>%
        ggplot(aes(x=vaxMetric, y=get(burdenVar))) + 
        geom_point(aes(size=pop)) + 
        geom_smooth(aes(weight=pop), method="lm") +
    ...
    lm(get(burdenVar) ~ vaxMetric, data=dfFull) %>% summary() %>% print()
    lm(get(burdenVar) ~ vaxMetric*type + 0 - vaxMetric, data=dfFull) %>% summary() %>% print()
...
countyCorr(dfCaseDeath=cty_newdata_20220202$dfPerCapita, 
