...
# Pull only from the four aces, high hearts (9-K) and low diamonds (2-6) for speed
sampHands <- combn( sort(c(1, 14, 27, 40, 22:26, 28:32)), 5)
...
```{r}
## Grab a sample of 10 hands from jbHands
set.seed(1608161356)
myKeys <- sample(1:nrow(jbHands), 10, replace=FALSE)
...
The lookups still take FAR too long; a much-improved method of determining values of the re-draws will be needed.  
  
And, sample plots are created using ggplot2 (cached, since they take a while to plot off the large frame):  
```{r, cache=TRUE}
library(ggplot2)
