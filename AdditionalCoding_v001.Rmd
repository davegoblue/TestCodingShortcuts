...
startTime <- proc.time()
for (intCtr in 1:nrow(jbHands)) {
    thisHand <- jbHands[intCtr, ]
    thisValue <- array0[intCtr]
    
    # Populate array5 (draw all 5)
    # array5[1, thisValue] <- array5[1, thisValue] + 1
    
    # Populate array4 (draw 4 and keep 1)
    # for (thisCard in thisHand) {
    #     array4[thisCard, thisValue] <- array4[thisCard, thisValue] + 1
    # }
    
    # Populate array3 (draw 3 and keep 2)
    # a <- combn(thisHand, 2)
    # b <- idxKeep2[a[1, ], "posn"] + diff(a)
    # array3[b, thisValue] <- array3[b, thisValue] + 1
    
    # apply(combn(thisHand, 2), 2, 
    #       FUN=function(x) { idx=idxKeep2[x[1], "posn"] + x[2] - x[1]; 
    #                         array3[idx, thisValue] <- array3[idx, thisValue] + 1
    #                      } 
    #      )
    
    if (intCtr %% 100000 == 0) { print(paste0("On record: ", intCtr)) }
...
The next step is to vectorize the above for array3, as has been done for array4 and array5; absent that, run time is still FAR too long.  