...
Next, a routine is established to look at the potential outcomes for removing a single card from a single hand and filling it with all replacements:  
```{r}
...
## Grab a sample of 100 hands from jbHands
set.seed(1608161356)
myKeys <- sample(1:nrow(jbHands), 100, replace=FALSE)

# Create an indexed data.table containing the hands and their values
library(data.table)
jbDT <- as.data.table(cbind(jbHands, jbType))
...
for (thisKey in myKeys) {
    ...
    newVal <- integer(0)
    for (thisCard in canReplace) {
        # Place it in position one
        thisHand <- sort( c(keyHand[-1], thisCard) )
        # Next need to find the value -- need a VERY fast lookup for this
        # Good to practice with DT, but will need to also make the algorithm much more clever
        newVal <- c(newVal, 
                    jbDT[.(thisHand[1], thisHand[2], thisHand[3], thisHand[4], thisHand[5]), jbType]
                    )
    }
}
```
