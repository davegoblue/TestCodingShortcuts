...
lstComboMapper <- list("cdcDaily"=list("nyc"=list("comboVar"="state", 
                       ...
                       "cdcWeeklyBurden"=list("nyc"=list("comboVar"="state", 
                                                         "uqVars"="date", 
                                                         "vecCombo"=c("NY"="NY", "NYC"="NY"),
                                                         "fn"=specNA(sum)
                                                         )
...
urlMapper <- c("cdcDaily"="https://data.cdc.gov/api/views/9mfq-cb36/rows.csv?accessType=DOWNLOAD", 
               "cdcWeeklyBurden"="https://data.cdc.gov/api/views/pwn4-m3yp/rows.csv?accessType=DOWNLOAD",
...
renMapper <- list("cdcDaily"=c('submission_date'='date', 'new_case'='new_cases', 
                  ...
                  "cdcWeeklyBurden"=c('end_date'='date'),
...
selfListMapper <- list("cdcDaily"=list('date'=lubridate::mdy), 
                       "cdcWeeklyBurden"=list("date"=function(x) if("Date" %in% class(x)) x else lubridate::mdy(x)),
                       "cdcHosp"=list(), 
...
lstFilterMapper <- list("cdcDaily"=list("state"=c(state.abb, "DC")), 
                        "cdcWeeklyBurden"=list("state"=c(state.abb, "DC")),
...
vecSelectMapper <- list("cdcDaily"=c("date", "state", "tot_cases", "tot_deaths", "new_cases", "new_deaths"), 
                        "cdcWeeklyBurden"=c("date", "state", "tot_cases", "tot_deaths", "new_cases", "new_deaths"),
...
checkControlVarsMapper <- list("cdcDaily"=c("new_cases", "new_deaths"),
                               "cdcWeeklyBurden"=c("new_cases", "new_deaths"),
...
checkSimilarityMapper <- list("cdcDaily"=list(date=list(label='date', countOnly=TRUE, convChar=TRUE), 
                              ...
                              "cdcWeeklyBurden"=list(date=list(label='date', countOnly=TRUE, convChar=TRUE), 
                                                     state=list(label='state', countOnly=FALSE)
                                                     ),
...
# Assign cdcWeeklyBurden to use the same mapping as cdcDaily for keyAggMapper
...
# Mapping file for HHS hospital capacity numerical variables to be kept, with renaming
hhsMapper <- c("total_beds_7_day_avg"="total_beds", 
               ...
               "inpatient_beds_used_7_day_avg"="inpatient_beds_occupied", 
               "inpatient_beds_used_covid_7_day_avg"="inpatient_beds_covid"
               )
