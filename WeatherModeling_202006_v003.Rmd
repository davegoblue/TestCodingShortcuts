...
xgbRunModel <- function(tbl, 
    ...
    # Create a base testData file that is NULL (will be for xgb.cv)
    testData <- NULL
...
helperXGBOnevAll <- function(df, 
    ...
    # Extract a balanced sample of data matching keyLoc and all-other data
    subDF <- subDF %>%
        ...
        ungroup() %>%
        mutate(binKEY=ifelse(isKEY==descName, 1, 0))
