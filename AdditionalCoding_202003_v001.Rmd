...
runFullPrecipExtraction <- function(df, 
    ...
    testIntervals <- createPrecipInterval(testAllEnds, testAllBegins, endExclude=endExclude, 
                                          beginExclude=beginExclude, maxProb=maxProb, sState=sState)
...
intervalConsistency <- function(lst, pType){
    ...
    precipMETAR <- grepl(paste0(pType, ".*RMK"), metar)
    ...
    if (length(mism) == 0) {
        ...
        for (x in mism) {
            cat("\nMismatch at time", strftime(dtime[x], format="%Y-%m-%d %H:%M"), "UTC\n")
...
findPrecipTypes <- function(lst, precipRegex="(?<= )[+-]?[A-Z]+(?= )", priorTo="RMK", exclTypes=NULL) {
    ...
    if (!is.null(priorTo)) {
        ...
        metar <- str_sub(metar, start=1, end=rmkLoc[, 1]-1)
    }
    ...
    if (max(listLengths) > 2) { 
        stop("Hard-coded for at most 2 precipitation matches, please investigate")
    ...
    if (max(listLengths)==2) {
        ...
        sapply(precipMatches[listLengths==2], FUN=paste, collapse=" ") %>% 
            table() %>% 
            sort(decreasing=TRUE) %>%
            print()
    }
