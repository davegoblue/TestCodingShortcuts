...
The model is over 75% accurate in classifying Chicago and Lincoln.  The most important variables continue to be Temperature and Dew Point, though it is notable that prediction accuracy is worse with calm or variable winds and best with strong winds (not many data points here).
...
Accuracy falls to just under 60%, and the model becomes overly confident in its predictions.  The model also very slightly prefers mtry=1 and a large node size, so it self-selects parameters less likely to lead to overconfidence.  
...
Wind direction is much more helpful than wind speed, with accuracy increasing to 68%.  Is this more valuable than knowing the month (much of the wind speed difference is Chicago having predominantly W winds rather than the N/S winds of Lincoln in the winter)?  
```{r cache=TRUE}
...
Knowing the month and the wind direction are roughly equally as helpful, with each driving accuracy to 68%.  Assuming that the model already has access to temperature, dew point, and wind direction (accuracy 68%), how helpful is it to add wind speed?  
```{r cache=TRUE}
...
Adding wind speed has only a minor impact, increasing accuracy to 70%.  If instead month is added:  
```{r cache=TRUE}
...
* Wind and Dew Point - 60% accuracy  
* Wind and Dew Point and Wind Speed - 60% accuracy  
* Wind and Dew Point and Wind Direction - 68% accuracy  
* Wind and Dew Point and Month - 68% accuracy  
* Wind and Dew Point and Wind Direction and Wind Speed - 70% accuracy  
* Wind and Dew Point and Wind Direction and Month - 70% accuracy  
* Wind and Dew Point and Wind Direction and Wind Speed and Month - 78% accuracy  
