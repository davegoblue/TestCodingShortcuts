...
extractPrecipData <- function(processedFile, pType="RA") {
    ...
    procMET <- procMET %>% 
        mutate(precipData=str_extract(origMETAR, pattern=keyPattern), 
               isPrecip=grepl(paste0(pType, "[B|E]"), origMETAR), 
...
# Data for Chicago Rain
# testIntervals <- createPrecipInterval(testAllEnds, testAllBegins, endExclude=c("2016-03-01 0414"))
# testIntervals <- createPrecipInterval(testAllEnds, testAllBegins, 
...
# testIntervals <- createPrecipInterval(testAllEnds, testAllBegins, 
#                                       beginExclude=c("2016-07-13 2347"),
#                                       endExclude=c("2016-03-01 0414", "2016-03-24 2214", "2016-08-29 2048"), 
#                                       maxProb=1500
#                                       )

# Data for Chicago Snow
# testIntervals <- createPrecipInterval(testAllEnds, testAllBegins, endExclude=c("2016-02-10 0216"))
# testIntervals <- createPrecipInterval(testAllEnds, testAllBegins, 
...
createPrecipIntervalPlots <- function(intervalData, beginsData, titleText, yAxisText, beginExclude=c()) {

    # Exclude any data from begins as needed
    beginsData <- beginsData[!(beginsData %in% lubridate::ymd_hm(beginExclude))]
    
    # Plot the precipitation durations in minutes
    p1 <- data.frame(minutesPrecip=as.numeric(intervalData), month=lubridate::month(beginsData)) %>%
        ggplot(aes(x=minutesPrecip/60)) +
        geom_histogram() + 
        labs(title=titleText, x=yAxisText)
    print(p1)

    # Plot the rain totals (in hours) by month
    p2 <- data.frame(minutesPrecip=as.numeric(intervalData), month=lubridate::month(beginsData)) %>%
        group_by(month) %>%
        summarize(minutesPrecip=sum(minutesPrecip), nPrecip=n()) %>%
        ggplot(aes(x=factor(month.abb[month], levels=month.abb[1:12]), y=minutesPrecip/60)) +
        geom_col() + 
        labs(title=titleText, y=yAxisText, x="") + 
        geom_text(aes(y=2+minutesPrecip/60, label=round(minutesPrecip/60, 1)))
    print(p2)
...
# Plots for Chicago Rain
# createPrecipIntervalPlots(testIntervals, testAllBegins, beginExclude=c("2016-07-13 2347"),
#                           titleText="Chicago, IL Rainfall (hours) in 2016", yAxisText="Hours of Rain"
#                           )
