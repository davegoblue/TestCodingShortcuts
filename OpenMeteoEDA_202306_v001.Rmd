...
runKMeans <- function(df, 
    ...
    # plotByCluster: boolean, plot variable means by cluster?
    ...
    if(isTRUE(plotByCluster)) {
        p1 <- km$centers %>%
            ...
            scale_color_discrete("Cluster") + 
            facet_wrap(~factor(cluster)) +
            labs(title=paste0("Cluster means (kmeans, centers=", centers, ")"), 
                 x="Metric", 
                 y="Cluster mean"
                 ) + 
...
km003 <- runKMeans(tmpTempTrain, vars=varsTrain, centers=3, nStart=25, seed=23082113, plotByCluster=TRUE)
str(km003)
