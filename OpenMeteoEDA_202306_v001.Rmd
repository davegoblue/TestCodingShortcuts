...
simpleOneVarConfusionReport <- function(df, 
    ...
    if(isTRUE(plotConf)) {
        p1 <- df %>%
            ...
            labs(x="Actual", 
                 y="Predicted", 
                 title=paste0("Training data - Actual vs. predicted ", tgtOrig), 
...
simpleOneVarChain <- function(df,
    ...
    # rankType: method for breaking ties of same n, passed to base::rank as ties.method=
    # naMethod: method for handling NA in ranks, passed to base::rank as na.last=    
    # createReport: boolean, should the confusion report be created?
    ...
    if(isTRUE(createReport)) {
        simpleOneVarConfusionReport(df=dfConfusion, 
                                    tgtOrig=tgt, 
                                    tgtPred="predicted", 
    ...
    # Return data if requested
    if(isTRUE(returnData)) list(dfFit=dfFit, mapper=mapper, dfApplied=dfApplied, dfConfusion=dfConfusion)
...
# Data only
tmpChain_v2 <- simpleOneVarChain(tmpTemp, tgt="month", prd="pct_temperature_2m", createReport=FALSE)
