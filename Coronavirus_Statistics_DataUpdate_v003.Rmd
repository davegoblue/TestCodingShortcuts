...
Year-end 2020 data are downloaded from COVID Tracking Project, with existing segments applied:  
```{r cache=TRUE}

# Use existing segments with updated data
locDownload <- "./RInputFiles/Coronavirus/CV_downloaded_210101.csv"
old_hier5_210101 <- readRunCOVIDTrackingProject(thruLabel="Dec 31, 2020", 
                                                downloadTo=if(file.exists(locDownload)) NULL else locDownload,
                                                readFrom=locDownload, 
                                                compareFile=readFromRDS("test_hier5_201025")$dfRaw,
                                                useClusters=readFromRDS("test_hier5_201130")$useClusters
                                                )
saveToRDS(old_hier5_210101, ovrWriteError=FALSE)

```

Year-end 2020 data are downloaded from USA Facts, with existing segments applied:  
```{r cache=TRUE}

# Locations for the population, case, and death file
popLoc <- "./RInputFiles/Coronavirus/covid_county_population_usafacts.csv"
caseLoc <- "./RInputFiles/Coronavirus/covid_confirmed_usafacts_downloaded_20210102.csv"
deathLoc <- "./RInputFiles/Coronavirus/covid_deaths_usafacts_downloaded_20210102.csv"

# Run old segments against new data
cty_old_20210102 <- readRunUSAFacts(maxDate="2020-12-29", 
                                    popLoc=popLoc, 
                                    caseLoc=caseLoc, 
                                    deathLoc=deathLoc, 
                                    dlCaseDeath=!(file.exists(caseLoc) & file.exists(deathLoc)),
...
saveToRDS(cty_old_20210102, ovrWriteError=FALSE)

```

There appears to be a continuity and/or consistency issue with the very recent data, which merits further exploration and/or a future data download.

Data showing a negative daily total for either cases or deaths are pulled:  
```{r}

negData <- cty_old_20210102$helperACC_county %>% 
    mutate(state=stringr::str_pad(state, width=5, side="left", pad="0")) %>%
    filter(deaths < 0 | cases < 0) %>% 
    left_join(select(usmap::countypop, -pop_2015), by=c("state"="fips"))

negSum <- negData %>%
    mutate(mon=lubridate::month(date)) %>%
    group_by(abbr, mon) %>%
    summarize(cases=sum(pmin(cases, 0)), deaths=sum(pmin(deaths, 0))) %>%
    pivot_longer(-c(abbr, mon)) %>%
    filter(value < 0)

negSum %>%
    ggplot(aes(x=fct_reorder(abbr, -value), y=value)) + 
    geom_col(aes(fill=month.abb[mon]), position="stack") + 
    coord_flip() + 
    labs(y="Sum of negaive values") +
    facet_wrap(~name, scales="free_x")

```
  
While some restatements are normal and expected, there appears to be a significant increase in this phenomenon for a few key states in December.  Restatements are plotted by date:  
```{r}

negData %>%
    select(date, cases, deaths) %>%
    mutate(cases=pmin(cases, 0), deaths=pmin(deaths, 0)) %>%
    group_by(date) %>%
...
There appears to be a potentially anomalous reporting in late December.  A later version of the data file is downloaded to see if the anomalies have cleared:  
```{r cache=TRUE}

# Locations for the population, case, and death file
popLoc <- "./RInputFiles/Coronavirus/covid_county_population_usafacts.csv"
caseLoc <- "./RInputFiles/Coronavirus/covid_confirmed_usafacts_downloaded_20210104.csv"
deathLoc <- "./RInputFiles/Coronavirus/covid_deaths_usafacts_downloaded_20210104.csv"

# Run old segments against new data
cty_old_20210104 <- readRunUSAFacts(maxDate="2020-12-31", 
                                    popLoc=popLoc, 
                                    caseLoc=caseLoc, 
                                    deathLoc=deathLoc, 
                                    dlCaseDeath=!(file.exists(caseLoc) & file.exists(deathLoc)),
...
saveToRDS(cty_old_20210104, ovrWriteError=FALSE)

```

While there are still some anomalies, the issues in SD where increases were reported as cumulative has been cleared.  The largest remaining restatements are in MA and MO, potentially year-end data clean up.  The restatements no longer drive pronounced trend breaks for the segment-level totals.

Updated CDC data are also downloaded:  
```{r cache=TRUE}

# Download new data
cdcLoc <- "Weekly_counts_of_deaths_by_jurisdiction_and_age_group_downloaded_20210105.csv"
cdcList_20210105 <- readRunCDCAllCause(loc=cdcLoc, 
                                       startYear=2015, 
                                       curYear=2020,
                                       weekThru=48, 
                                       startWeek=9, 
                                       lst=old_hier5_210101, 
                                       epiMap=readFromRDS("epiMonth"), 
                                       agePopData=readFromRDS("usPopBucket2020"), 
                                       cvDeathThru="2020-11-28", 
                                       cdcPlotStartWeek=10, 
                                       dlData=!file.exists(paste0("./RInputFiles/Coronavirus/", cdcLoc)), 
                                       stateNoCheck=c("NC")
                                       )
saveToRDS(cdcList_20210105, ovrWriteError=FALSE)

```

There continues to be reasonable alignment among locales / segments with heavy coronavirus burden and those with high excess all-cause deaths per million.  Generally, excess all-cause deaths exceed reported coronavirus deaths.