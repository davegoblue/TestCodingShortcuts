...
cdcNoNA <- cdcFourFiles %>%
    group_by(weekEnding) %>%
    summarize(numNA=sum(is.na(reportedDeaths)), .groups="drop") %>%
    filter(numNA==0) %>%
    select(weekEnding) %>%
    inner_join(cdcFourFiles, by="weekEnding")
cdcNoNA

cdcRatio <- cdcNoNA %>%
    arrange(weekEnding, source) %>%
    group_by(weekEnding) %>%
    mutate(firstDeaths=first(reportedDeaths), pctFirst=reportedDeaths/firstDeaths) %>%
    ungroup()
cdcRatio %>%
    filter(lubridate::year(weekEnding)==2020) %>%
    ggplot(aes(x=weekEnding, y=pctFirst)) + 
    geom_line(aes(group=source, color=factor(source))) + 
    labs(x="Data reported for week", 
         y="Ratio vs. reported in 2020-09-23 file", 
         title="Ratio of CDC all-cause deaths reported by week in 2020", 
         subtitle="Compared to number by week reported in 2020-09-23 file"
         ) + 
    scale_color_discrete("Download Date")
cdcRatio %>%
    filter(lubridate::year(weekEnding)==2020) %>%
    mutate(weeksPrior=(lubridate::yday(max(weekEnding))-lubridate::yday(weekEnding))/7) %>%
    group_by(weekEnding) %>%
    filter(row_number()==n()) %>%
    ggplot(aes(x=weeksPrior, y=pctFirst)) + 
    geom_text(aes(label=paste0(round(100*pctFirst), "%")), size=3) + 
    labs(x="Weeks prior to latest week reported in 2020-09-23 file", 
         y="Ratio vs. reported in 2020-09-23 file", 
         title="Ratio of CDC all-cause deaths reported by week in 2020", 
         subtitle="Compared to number by week reported in 2020-09-23 file"
         )
