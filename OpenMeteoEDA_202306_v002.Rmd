...
The function is updated to allow for an automated estimate for rounding parameter:  
```{r}
...
runPartialImportanceRF <- function(dfTrain, 
    ...
    # Make an estimate for rounding if parameter passed as -1L
    if(!is.null(rndTo)) {
        if(isTRUE(all.equal(-1L, rndTo))) {
            predVals <- dfTest %>% group_by(across(all_of(yVar))) %>% summarize(n=n()) %>% pull(yVar)
            if(length(predVals) <= 1.25*rndBucketsAuto) rndTo <- NULL
            else rndTo <- diff(range(predVals)) / rndBucketsAuto
        }
    }
...
The rounding routine is written as a function:  
```{r}
...
# No rounding
autoRound(exRands, rndTo=NULL)

# Designated rounding
autoRound(exRands, rndTo=0.1) %>% table()

# Automated rounding (10 buckets, no significant digits hurdle)
autoRound(exRands, rndBucketsAuto=10) %>% table()

# Automated rounding (10 buckets, 1 significant digits hurdle)
autoRound(exRands, rndBucketsAuto=10, nSig=1) %>% table()

# Automated rounding (40 buckets, 2 significant digits hurdle)
autoRound(exRands, rndBucketsAuto=40, nSig=2) %>% table()

```

Function plotConfusion() is updated to allow for auto-rounding:  
```{r, fig.height=9, fig.width=9}
...
# Default (no rounding)
plotConfusion(dfTestCont[1:5000,], trueCol="trueVal", predCol="predVal", plotCont=TRUE, refXY=TRUE)

# User-specified rounding criteria
plotConfusion(dfTestCont, trueCol="trueVal", predCol="predVal", plotCont=TRUE, rndTo=0.05, refXY=TRUE)

# Automated rounding
plotConfusion(dfTestCont, 
              trueCol="trueVal", 
              predCol="predVal", 
              plotCont=TRUE, 
              rndTo=-1L, 
...
Function runFullRF() is updated for auto-rounding:  
```{r, fig.height=9, fig.width=9}
...
# Test updated function with auto-rounding
runFullRF(dfTrain=dfTrain %>% mutate(doy=yday(date)), 
          yVar="soil_temperature_100_to_255cm", 
          xVars=c("doy", "month"), 
          dfTest=dfTest %>% mutate(doy=yday(date)), 
...
Function runPartialImportanceRF() is updated for auto-rounding:  
```{r, fig.height=9, fig.width=9}
