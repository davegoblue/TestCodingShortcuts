...
runFullRF(dfTrain=NULL,
          ...
          useExistingRF=rfTestMaxT_v002$rf,
          yVar="temperature_2m_max", 
          isContVar=TRUE, 
          rndTo=-1L,
          returnData=FALSE
...
The approach is modified by assuming the "most related" city for the new data (Atlanta, GA):  
```{r, fig.height=9, fig.width=9, cache=TRUE}

runFullRF(dfTrain=NULL,
          dfTest=dfDailyPHX %>% 
              mutate(fct_city=factor("Atlanta GA", 
                                     levels=dfMultiCity_v2 %>% pull(cityName) %>% unique()
                                     ), 
                     fct_mon=factor(month.abb[month(date)], levels=month.abb)
                     ),
...
The random forest is re-run to predict city including the new data, with numerics and factors as predictors:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

omCityMap_v3 <- c("atl"="Atlanta GA", 
                  "ord"="Chicago IL", 
                  "dtw"="Detroit MI", 
                  "msp"="Minneapolis MN",
                  "msy"="New Orleans LA", 
...
dfMultiCity_v3 <- map_dfr(.x=names(omCityMap_v3), 
                          .f=function(x) omRunAllSteps(dlData=FALSE, runStats=FALSE, abbCity=x, returnDF=TRUE), 
                          .id="src"
                          ) %>%
    mutate(cityName=unname(omCityMap_v3)[as.integer(src)])

dfMultiCity_v3 %>% 
    count(cityName)

set.seed(25080313)
idxTrain_v3 <- sample(1:nrow(dfMultiCity_v3), round(0.75*nrow(dfMultiCity_v3)), replace=FALSE)

rfTestCity_v003 <- runFullRF(dfTrain=dfMultiCity_v3[idxTrain_v3, ] %>% 
                                 mutate(fct_city=factor(cityName), 
                                        fct_mon=factor(month.abb[month(date)], levels=month.abb)
                                        ),
                             dfTest=dfMultiCity_v3[-idxTrain_v3, ] %>% 
                                 mutate(fct_city=factor(cityName), 
                                        fct_mon=factor(month.abb[month(date)], levels=month.abb)
                                        ),
                             yVar="fct_city", 
                             xVars=dfMultiCity_v3 %>% 
                                 select(where(is.numeric), where(is.factor), -winddirection_10m_dominant) %>% 
                                 names %>%
                                 c("fct_mon"),
                             isContVar=FALSE, 
...
The random forest is re-run to predict month including the new data, with numerics and factors as predictors:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

set.seed(25080412)
idxTrain_v3 <- sample(1:nrow(dfMultiCity_v3), round(0.75*nrow(dfMultiCity_v3)), replace=FALSE)

rfTestMon_v003 <- runFullRF(dfTrain=dfMultiCity_v3[idxTrain_v3, ] %>% 
                                mutate(fct_city=factor(cityName), 
                                       fct_mon=factor(month.abb[month(date)], levels=month.abb)
                                       ),
                            dfTest=dfMultiCity_v3[-idxTrain_v3, ] %>% 
...
cat("\n\nAccuracy of month predictions by city (test data):\n")
rfTestMon_v003$tstPred %>%
    group_by(trueCity=fct_city) %>%
    summarize(n=n(), acc=mean(fct_mon==pred)) %>%
    arrange(desc(acc))
...
rfTestMaxT_v003 <- runFullRF(dfTrain=dfMultiCity_v3[idxTrain_v3, ] %>% 
                                 mutate(fct_city=factor(cityName), 
                                        fct_mon=factor(month.abb[month(date)], levels=month.abb)
                                        ),
                             dfTest=dfMultiCity_v3[-idxTrain_v3, ] %>% 
...
dfDailyBNA <- omRunAllSteps(dlData=FALSE, 
                            runStats=TRUE, 
                            abbCity="bna", 
                            useNameCity="Nashville TN", 
                            returnDF=TRUE
                            ) %>%
    mutate(cityName="Nashville TN")

dfDailyBNA

```

An existing random forest is applied for predicting Nashville:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

runFullRF(dfTrain=NULL,
          dfTest=dfDailyBNA %>% 
              mutate(fct_city=factor(cityName, 
                                     levels=dfMultiCity_v3 %>% pull(cityName) %>% unique()
                                     ), 
                     fct_mon=factor(month.abb[month(date)], levels=month.abb)
                     ),
          useExistingRF=rfTestCity_v003$rf,
          yVar="fct_city", 
          xVars=dfMultiCity_v3 %>% 
              select(where(is.numeric), where(is.factor), -winddirection_10m_dominant) %>% 
              names %>%
...
runFullRF(dfTrain=NULL,
          ...
          useExistingRF=rfTestMaxT_v003$rf,
          yVar="temperature_2m_max", 
          isContVar=TRUE, 
          rndTo=-1L,
          returnData=FALSE
...
Adding "most similar" city does not change model performance

The random forest is re-run to predict city including the new data, with numerics and factors as predictors:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

omCityMap_v4 <- c("atl"="Atlanta GA", 
                  "ord"="Chicago IL", 
                  "dtw"="Detroit MI", 
                  "msp"="Minneapolis MN",
                  "msy"="New Orleans LA", 
...
dfMultiCity_v4 <- map_dfr(.x=names(omCityMap_v4), 
                          .f=function(x) omRunAllSteps(dlData=FALSE, runStats=FALSE, abbCity=x, returnDF=TRUE), 
                          .id="src"
                          ) %>%
    mutate(cityName=unname(omCityMap_v4)[as.integer(src)])

dfMultiCity_v4 %>% 
    count(cityName)

set.seed(25081012)
idxTrain_v4 <- sample(1:nrow(dfMultiCity_v4), round(0.75*nrow(dfMultiCity_v4)), replace=FALSE)

rfTestCity_v004 <- runFullRF(dfTrain=dfMultiCity_v4[idxTrain_v4, ] %>% 
                                 mutate(fct_city=factor(cityName), 
                                        fct_mon=factor(month.abb[month(date)], levels=month.abb)
                                        ),
                             dfTest=dfMultiCity_v4[-idxTrain_v4, ] %>% 
                                 mutate(fct_city=factor(cityName), 
                                        fct_mon=factor(month.abb[month(date)], levels=month.abb)
                                        ),
                             yVar="fct_city", 
                             xVars=dfMultiCity_v4 %>% 
                                 select(where(is.numeric), where(is.factor), -winddirection_10m_dominant) %>% 
                                 names %>%
                                 c("fct_mon"),
                             isContVar=FALSE, 
...
rfTestMon_v004 <- runFullRF(dfTrain=dfMultiCity_v4[idxTrain_v4, ] %>% 
                                mutate(fct_city=factor(cityName), 
                                       fct_mon=factor(month.abb[month(date)], levels=month.abb)
                                       ),
                            dfTest=dfMultiCity_v4[-idxTrain_v4, ] %>% 
...
rfTestMaxT_v004 <- runFullRF(dfTrain=dfMultiCity_v4[idxTrain_v4, ] %>% 
                                 mutate(fct_city=factor(cityName), 
                                        fct_mon=factor(month.abb[month(date)], levels=month.abb)
                                        ),
                             dfTest=dfMultiCity_v4[-idxTrain_v4, ] %>% 
...
dfDailyMCI <- omRunAllSteps(dlData=FALSE, 
                            runStats=TRUE, 
                            abbCity="mci", 
                            useNameCity="Kansas City MO", 
                            returnDF=TRUE
                            ) %>%
    mutate(cityName="Kansas City MO")

dfDailyMCI

```

An existing random forest is applied for predicting Kansas City:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

runFullRF(dfTrain=NULL,
          dfTest=dfDailyMCI %>% 
              mutate(fct_city=factor(cityName, 
                                     levels=dfMultiCity_v4 %>% pull(cityName) %>% unique()
                                     ), 
                     fct_mon=factor(month.abb[month(date)], levels=month.abb)
                     ),
          useExistingRF=rfTestCity_v004$rf,
          yVar="fct_city", 
          xVars=dfMultiCity_v4 %>% 
              select(where(is.numeric), where(is.factor), -winddirection_10m_dominant) %>% 
              names %>%
...
runFullRF(dfTrain=NULL,
          ...
          useExistingRF=rfTestMaxT_v004$rf,
          yVar="temperature_2m_max", 
          isContVar=TRUE, 
          rndTo=-1L,
          returnData=FALSE
...
The approach is modified by assuming the "most related" city for the new data (Nashville, TN):  
```{r, fig.height=9, fig.width=9, cache=TRUE}

runFullRF(dfTrain=NULL,
          dfTest=dfDailyMCI %>% 
              mutate(fct_city=factor("Nashville TN", 
                                     levels=dfMultiCity_v4 %>% pull(cityName) %>% unique()
                                     ), 
                     fct_mon=factor(month.abb[month(date)], levels=month.abb)
                     ),
...
dfMultiCity_v5 <- map_dfr(.x=names(omCityMap_v5), 
                          .f=function(x) omRunAllSteps(dlData=FALSE, runStats=FALSE, abbCity=x, returnDF=TRUE), 
                          .id="src"
                          ) %>%
    mutate(cityName=unname(omCityMap_v5)[as.integer(src)])

dfMultiCity_v5 %>% 
    count(cityName)

set.seed(25081712)
idxTrain_v5 <- sample(1:nrow(dfMultiCity_v5), round(0.75*nrow(dfMultiCity_v5)), replace=FALSE)

rfTestCity_v005 <- runFullRF(dfTrain=dfMultiCity_v5[idxTrain_v5, ] %>% 
                                 mutate(fct_city=factor(cityName), 
                                        fct_mon=factor(month.abb[month(date)], levels=month.abb)
                                        ),
                             dfTest=dfMultiCity_v5[-idxTrain_v5, ] %>% 
                                 mutate(fct_city=factor(cityName), 
                                        fct_mon=factor(month.abb[month(date)], levels=month.abb)
                                        ),
                             yVar="fct_city", 
                             xVars=dfMultiCity_v5 %>% 
                                 select(where(is.numeric), where(is.factor), -winddirection_10m_dominant) %>% 
                                 names %>%
                                 c("fct_mon"),
                             isContVar=FALSE, 
...
rfTestMon_v005 <- runFullRF(dfTrain=dfMultiCity_v5[idxTrain_v5, ] %>% 
                                mutate(fct_city=factor(cityName), 
                                       fct_mon=factor(month.abb[month(date)], levels=month.abb)
                                       ),
                            dfTest=dfMultiCity_v5[-idxTrain_v5, ] %>% 
...
rfTestMaxT_v005 <- runFullRF(dfTrain=dfMultiCity_v5[idxTrain_v5, ] %>% 
                                 mutate(fct_city=factor(cityName), 
                                        fct_mon=factor(month.abb[month(date)], levels=month.abb)
                                        ),
                             dfTest=dfMultiCity_v5[-idxTrain_v5, ] %>% 
...
cat("\n\nR-squared of month predictions by city (test data):\n")
rfTestMaxT_v005$tstPred %>%
    group_by(trueCity=fct_city) %>%
    summarize(n=n(), 
              muTemp=mean(temperature_2m_max), 
              muTemp2=mean(temperature_2m_max**2), 
              rmseNull=sqrt(muTemp2-muTemp**2), 
              rmsePred=sqrt(mean((temperature_2m_max-pred)**2)), 
              r2=1-rmsePred**2/rmseNull**2
              ) %>%
    arrange(desc(r2))
