...
makePeakValley <- function(df, 
    ...
    if(is.null(names(useTitle))) useTitle <- rep(useTitle, times=length(numVar)) %>% purrr::set_names(all_of(numVar))
    if(is.null(names(yLab))) yLab <- rep(yLab, times=length(numVar)) %>% purrr::set_names(all_of(numVar))
    ...
    for(keyVar in numVar) {
        ...
        p1 <- newDF %>%
            ggplot(aes(x=get(uqBy), y=fnNumVar(get(keyVar)))) + 
            geom_line() + 
            ...
            geom_text(data=~filter(., get(paste0(keyVar, "_isPeak"))), 
                      aes(y=fnPeak(fnNumVar(get(keyVar))), 
                          label=paste0(get(uqBy), "\n", round(fnNumVar(get(keyVar))))
                          ), 
                      color="red", 
                      size=3
                      ) + 
            geom_text(data=~filter(., get(paste0(keyVar, "_isValley"))), 
                      aes(y=fnValley(fnNumVar(get(keyVar))), 
                          label=paste0(get(uqBy), "\n", round(fnNumVar(get(keyVar))))
                          ), 
                      color="black", 
                      size=3
