...
dec2022Data %>%
    ...
    labs(title="Relationship between cases and deaths (each point is a state)", 
...
* Correlations are high among total vaccinations and vaccinations by sub-group  
* In December 2021, there is little correlation between case rate and death rate  
* In December 2021, more highly vaccinated states tend to have higher case rates  
* In December 2021, more highly vaccinated states tend to have lower case fatality rates, such that deaths per state are similar whether vaccination levels are high or low  
  
The process is converted to functional form:  
```{r, fig.height=9, fig.width=9}

stateAssociationMapper <- list("cpm"=list("name"="New cases per million", "shortName"="cases", "lim"=c(0, NA)), 
                               "dpm"=list("name"="New deaths per million", "shortName"="deaths", "lim"=c(0, NA)), 
                               "vxcpoppct"=list("name"="% population fully vaccinated", 
                                                "shortName"="vaccinated", 
                                                "lim"=c(0, 100)
                                                ), 
                               "vxcgte65pct"=list("name"="% 65+ population fully vaccinated", 
                                                  "shortName"="vaccinated (65+)", 
                                                  "lim"=c(0, 100)
                                                  ), 
                               "cfr"=list("name"="Crude case fatlatity rate (deaths/cases)", 
                                          "shortName"="CFR", 
                                          "lim"=c(0, NA)
                                          ), 
                               "hpm"=list("name"="Hospitalized per million (avg.)", 
                                          "shortName"="hospitalized", 
                                          "lim"=c(0, NA)
                                          )
                               )

stateAssociationPlots <- function(df, 
                                  xVar, 
                                  yVar,
                                  xAxisName=NA, 
                                  yAxisName=NA,
                                  plotTime="December 2021",
                                  varMapper=stateAssociationMapper,
                                  plotTitle=NA, 
                                  plotSubtitle=NA, 
                                  plotXLim=NA, 
                                  plotYLim=NA, 
    ...
    # FUNCTION ARGUMENTS:
    # df: processed data frame with key metrics, unique by level
    # xVar: x-variable
    # yVar: y-variable
    # xAxisName: name for x-axis (NA means create from xVar; NULL means exclude)
    # yAxisName: name for y-axis (NA means create from yVar; NULL means exclude)
    # plotTime: time period for the plot
    # varMapper: mapping file for converting NA to title and axis names for plot
    # plotTitle: title for the plot (NA means create from xVar and yVar; NULL means exclude)
    # plotSubtitle: subtitle for the plot (NA means create from date; NULL means exclude)
    # plotXLim: x-limits for the plot (NA means create automatically)
    # plotYLim: y-limites for the plot (NA means create automatically)
    # printPlot: boolean, should the plot be printed?
    # returnPlot: boolean, should the plot be returned?
    # showReg: boolean, show the regression line and r-squared?

    # Create plotTitle, plotSubtitle, xAxisName, and yAxisName, plotXLim, and plotYLim if passed as NA
    if(isTRUE(is.na(plotTitle))) 
        plotTitle <- paste0("Relationship between ", 
                            varMapper[[xVar]]$shortName, 
                            " and ", 
                            varMapper[[yVar]]$shortName, 
                            " (each point is a state)"
                            )
    if(isTRUE(is.na(plotSubtitle))) plotSubtitle <- paste0("Data from ", plotTime)
    if(isTRUE(is.na(xAxisName))) xAxisName <- paste0(varMapper[[xVar]]$name, " in ", plotTime)
    if(isTRUE(is.na(yAxisName))) yAxisName <- paste0(varMapper[[yVar]]$name, " in ", plotTime)
    if(isTRUE(is.na(plotXLim))) plotXLim <- varMapper[[xVar]]$lim
    if(isTRUE(is.na(plotYLim))) plotYLim <- varMapper[[yVar]]$lim
    ...
    p1 <- df %>%
        ...
        lims(y=plotYLim, x=plotXLim)
    
    # Add the regression line if requested
    if(isTRUE(showReg)) {
        lmReg <- lm(get(yVar) ~ get(xVar), data=df)
        lmText <- paste0(yVar, " = ", signif(coef(lmReg)[2], 3), " * ", xVar)
        lmText <- paste0(lmText, ifelse(coef(lmReg)[1] < 0, " - ", " + "), signif(abs(coef(lmReg)[1]), 3))
        lmText <- paste0(lmText, "\nR-squared: ", round(summary(lmReg)$r.squared, 3))
        p1 <- p1 + annotate("text", x=-Inf, y=Inf, hjust=0, vjust=1, label=lmText)
    }
...
# Examples for 1) cases and deaths, 2) cases and vaccines, 3) deaths and vaccines, 4) CFR and vaccines
p1 <- stateAssociationPlots(dec2022Data, xVar="cpm", yVar="dpm", plotTime="December 2021", printPlot=FALSE)
p2 <- stateAssociationPlots(dec2022Data, xVar="vxcpoppct", yVar="cpm", plotTime="December 2021", printPlot=FALSE)
p3 <- stateAssociationPlots(dec2022Data, xVar="vxcpoppct", yVar="dpm", plotTime="December 2021", printPlot=FALSE)
p4 <- dec2022Data %>% 
    mutate(cfr=dpm/cpm) %>% 
    stateAssociationPlots(xVar="vxcpoppct", yVar="cfr", plotTime="December 2021", printPlot=FALSE)
...
Plots can then be created for various time periods:  
```{r, fig.height=9, fig.width=9}

exploreStateAssociations <- function(lst, 
                                     uqVar="state",
                                     sumVars=c("cpm", "dpm"),
                                     meanVars=c(),
                                     firstVars=c(), 
                                     lastVars=c("vxcpoppct", "vxcgte18pct", "vxcgte65pct"),
                                     plotLabel="",
                                     minDate=NULL, 
                                     maxDate=NULL, 
                                     xVars=c("cpm", "vxcpoppct", "vxcpoppct", "vxcpoppct"), 
                                     yVars=c("dpm", "cpm", "dpm", "cfr"), 
                                     showReg=TRUE
                                     ) {
    
    # FUNCTION ARGUMENTS:
    # lst: data frame containing processed state-level data, or list with sub-element "dfPerCapita"
    # uqVar: variable that will be the final level after grouping and summarizing
    # sumVars: variables to be summed over the relevant time range
    # meanVars: variables to be averaged over the relevant time range
    # firstVars: variables to keep the first non-NA value
    # lastVars: variables to keep the last non-NA value
    # plotLabel: text that should be included to describe the time period
    # minDate: minimum date to be used for summing cases and deaths (NULL means find earliest in data)
    # maxDate: maximum date to be used for summing cases and deaths (also used as date for % vaccinated)
    #          NULL means find latest date in data
    # xVars: the x-variables for the four plots created
    # yVars: the y-variables for the four plots created
    # showReg: boolean, show the regression equation on the plots?
    
    # Extract dfPerCapita is a list has been passed
    if("list" %in% class(lst)) lst <- lst[["dfPerCapita"]]
    
    # Find minDate and maxDate if passed as NULL
    if(is.null(minDate)) minDate <- lst %>% pull(date) %>% min()
    if(is.null(maxDate)) maxDate <- lst %>% pull(date) %>% max()
    
    # Function to find the last non-NA value
    lastNonNA <- function(x) last(x[!is.na(x)])
    firstNonNA <- function(x) first(x[!is.na(x)])
    
    # Create a relevant analysis data frame
    df <- lst %>%
        select(c(all_of(uqVar), "date", all_of(sumVars), all_of(meanVars), all_of(firstVars), all_of(lastVars))) %>%
        filter(date <= maxDate, date >= minDate) %>%
        arrange(across(c(all_of(uqVar), "date"))) %>%
        group_by(across(all_of(uqVar))) %>%
        summarize(across(all_of(sumVars), .fn=sum, na.rm=TRUE),
                  across(all_of(meanVars), .fn=mean, na.rm=TRUE), 
                  across(all_of(firstVars), .fn=firstNonNA),
                  across(all_of(lastVars), .fn=lastNonNA), 
                  .groups="drop"
                  )    
    
    # Create the plot series
    p1 <- stateAssociationPlots(df, xVar=xVars[1], yVar=yVars[1], plotTime=plotLabel, printPlot=FALSE, showReg=showReg)
    p2 <- stateAssociationPlots(df, xVar=xVars[2], yVar=yVars[2], plotTime=plotLabel, printPlot=FALSE, showReg=showReg)
    p3 <- stateAssociationPlots(df, xVar=xVars[3], yVar=yVars[3], plotTime=plotLabel, printPlot=FALSE, showReg=showReg)
    p4 <- df %>% 
        mutate(cfr=dpm/cpm) %>% 
        stateAssociationPlots(xVar=xVars[4], yVar=yVars[4], plotTime=plotLabel, printPlot=FALSE, showReg=showReg)
...
# Plots created using defaults
exploreStateAssociations(cdc_daily_220103, plotLabel="December 2021", minDate="2021-12-01", maxDate="2021-12-30")
exploreStateAssociations(cdc_daily_220103, plotLabel="September 2021", minDate="2021-09-01", maxDate="2021-09-30")
exploreStateAssociations(cdc_daily_220103, plotLabel="March 2021", minDate="2021-03-01", maxDate="2021-03-31")
exploreStateAssociations(cdc_daily_220103, plotLabel="T2 2021", minDate="2021-05-01", maxDate="2021-08-31")

# Plots created using vxcgte65pct
exploreStateAssociations(cdc_daily_220103, 
                         plotLabel="December 2021", 
                         minDate="2021-12-01", 
                         maxDate="2021-12-30", 
                         xVars=rep(c("vxcgte65pct"), 4), 
                         yVars=c("vxcpoppct", "cpm", "dpm", "cfr")
                         )
exploreStateAssociations(cdc_daily_220103, 
                         plotLabel="September 2021", 
                         minDate="2021-09-01", 
                         maxDate="2021-09-30", 
                         xVars=rep(c("vxcgte65pct"), 4), 
                         yVars=c("vxcpoppct", "cpm", "dpm", "cfr")
...
# Plots including hospitalized
exploreStateAssociations(cdc_daily_220103, 
                         meanVars="hpm",
                         plotLabel="December 2021", 
                         minDate="2021-12-01", 
                         maxDate="2021-12-30", 
                         xVars=rep(c("vxcpoppct"), 4), 
                         yVars=c("cpm", "hpm", "dpm", "cfr")
                         )

# Plots using vaccinated from earliest date
exploreStateAssociations(cdc_daily_220103, 
                         meanVars="hpm",
                         firstVars=c("vxcpoppct", "vxcgte18pct", "vxcgte65pct"), 
                         lastVars=c(),
                         plotLabel="Q4 2021", 
                         minDate="2021-09-01", 
                         maxDate="2021-12-30", 
...
readList <- list("cdcDaily"="./RInputFiles/Coronavirus/CDC_dc_downloaded_220206.csv", 
                 "cdcHosp"="./RInputFiles/Coronavirus/CDC_h_downloaded_220206.csv", 
                 "vax"="./RInputFiles/Coronavirus/vaxData_downloaded_220206.csv"
                 )
compareList <- list("cdcDaily"=readFromRDS("cdc_daily_220103")$dfRaw$cdcDaily, 
                    "cdcHosp"=readFromRDS("cdc_daily_220103")$dfRaw$cdcHosp, 
                    "vax"=readFromRDS("cdc_daily_220103")$dfRaw$vax
                    )

cdc_daily_220206 <- readRunCDCDaily(thruLabel="Feb 04, 2022", 
                                    downloadTo=lapply(readList, FUN=function(x) if(file.exists(x)) NA else x), 
                                    readFrom=readList,
                                    compareFile=compareList, 
                                    writeLog=NULL, 
...
saveToRDS(cdc_daily_220206, ovrWriteError=FALSE)
...
# Run for latest data, save as RDS
indivHosp_20220207 <- downloadReadHospitalData(loc="./RInputFiles/Coronavirus/HHS_Hospital_20220207.csv")
saveToRDS(indivHosp_20220207, ovrWriteError=FALSE)
...
stateHosp_20220207 <- readFromRDS("indivHosp_20220207") %>%
    filter(state %in% c(state.abb, "DC")) %>%
    colSelector(c("state", "collection_week", names(hhsMapper))) %>%
    colRenamer(hhsMapper) %>%
    mutate(across(where(is.numeric), .fns=function(x) ifelse(is.na(x), NA, ifelse(x==-999999, NA, x)))) %>%
...
# ICU summary
createGeoMap(stateHosp_20220207, 
             yVars=list("pctCovidICU"=c("label"="Covid", "color"="red"), 
                        "pctICU"=c("label"="Total", "color"="black")
                        ), 
             fullList=list("pctICU"=expression(icu_beds_occupied/icu_beds), 
...
# Cases per million summary
createGeoMap(cdc_daily_220206$dfPerCapita, 
             yVars=list("cpm7"=c("label"="Rolling\n7-day\ncases", "color"="red")), 
             xVar="date", 
             plotTitle="Cases per-million per day (rolling 7-day average)", 
             plotSubtitle="January 2020 to January 2022", 
             plotScaleLabel="", 
             hLine=NULL,
             returnData=FALSE
             )
...
State association plots are also updated:  
```{r, fig.height=9, fig.width=9}

# Plots using population vaccination vs cases and deaths
exploreStateAssociations(cdc_daily_220206, plotLabel="January 2022", minDate="2022-01-01", maxDate="2022-01-31")
exploreStateAssociations(cdc_daily_220206, plotLabel="Nov '21 to Jan '22", minDate="2021-11-01", maxDate="2022-01-31")

# Plots including hospitalized
exploreStateAssociations(cdc_daily_220206, 
                         meanVars="hpm",
                         firstVars=c("vxcpoppct", "vxcgte18pct", "vxcgte65pct"), 
                         plotLabel="January 2022", 
                         minDate="2022-01-01", 
                         maxDate="2022-01-31", 
                         xVars=rep(c("vxcpoppct"), 4), 
                         yVars=c("cpm", "hpm", "dpm", "cfr")
...
Hospitalization data are also summarized by segment:  
```{r, fig.height=9, fig.width=9}

purrr::walk(.x=sort(unique(cdc_daily_220206$useClusters)), 
            .f=function(segNum) {
                indivHosp_20220207 %>% 
                    filter(state %in% c(names(cdc_daily_220206$useClusters[cdc_daily_220206$useClusters==segNum])), 
                           collection_week==max(collection_week)
                           ) %>% 
                    pull(hospital_pk) %>%
                    plotHospitalUtilization(df=indivHosp_20220207, 
                                            keyHosp=., 
                                            plotTitle=paste0(paste0("State Segment ", segNum, " Hospitals Summed"))
                                            )
                }
...
indivHosp_20220207 %>% 
    ...
    pull(hospital_pk) %>%
    plotHospitalUtilization(df=indivHosp_20220207, keyHosp=., plotTitle="US Hospitals Summed")

```

Hospitals with significant changes in beds are explored:  
```{r, fig.height=9, fig.width=9}

indivHosp_20220207 %>% 
    select(!where(is.numeric), adult_beds=all_adult_hospital_beds_7_day_avg) %>% 
    group_by(collection_week) %>% 
    summarize(nNA=sum(is.na(adult_beds)), 
              n999999=sum(adult_beds==-999999, na.rm=TRUE), 
              nReport=n()-nNA-n999999, 
              adult_beds=sum(ifelse(adult_beds==-999999, 0, adult_beds), na.rm=TRUE)
              ) %>%
    arrange(desc(collection_week))

```

A significant number of hospitals are reporting NA capacity in the most recent time periods. An algorithm should be created to assume the last valid data in these cases:
```{r, fig.height=9, fig.width=9}

indivHosp_20220207 %>%
    select(!where(is.numeric), names(hhsMapper)) %>%
    colRenamer(vecRename = hhsMapper) %>%
    pivot_longer(where(is.numeric)) %>%
    group_by(collection_week, name) %>%
    summarize(n=n(), n999999=sum(value==-999999, na.rm=TRUE), nNA=sum(is.na(value)), .groups="drop") %>%
    ggplot(aes(x=collection_week)) + 
    geom_line(aes(y=n, color="Total records")) + 
    geom_line(aes(y=n999999, color="Records: -999999")) + 
    geom_line(aes(y=nNA, color="Records: NA")) +
    facet_wrap(~name) + 
    lims(y=c(0, NA)) + 
    labs(title="Data reporting by metric in February 7, 2022 hospital file", x=NULL, y="Number of records") + 
    scale_color_manual("Record Type", 
                       values=c("Total records"="black", "Records: NA"="red", "Records: -999999"="blue")
                       )

```
  
The issue seems limited to 'adult_beds' and 'total_beds'. An assumption is made to carry forward the last non-NA value when NA is reported:  
```{r, fig.height=9, fig.width=9}

naMod <- indivHosp_20220207 %>%
    select(!where(is.numeric), names(hhsMapper)) %>%
    colRenamer(vecRename = hhsMapper) %>%
    select(hospital_pk, collection_week, adult_beds, total_beds, inpatient_beds) %>%
    arrange(hospital_pk, collection_week) %>%
    group_by(hospital_pk) %>%
    mutate(across(c(total_beds, adult_beds), 
                  .fns=function(x) zoo::na.locf(x, na.rm=FALSE)
                  )
           ) 
naMod %>%
    pivot_longer(where(is.numeric)) %>%
...
naMod %>% 
    ...
    pivot_wider(collection_week) %>%
    arrange(desc(collection_week))

```

While there is still a reported decline in beds (greater percentage for adult and total than for inpatient), there is a much smaller trend break. A function is written for imputation, including a third column for ratios:  
```{r, fig.height=9, fig.width=9}
...
# Basic function
testImputeNA(x=c(1, 2, 3, 4, NA, NA, 7, -999999, NA, 10), naValues=c(-999999))

# Including y vector
testImputeNA(x=c(1, 2, 3, 4, NA, NA, 7, -999999, NA, 10), y=1:10, naValues=c(-999999))

# Applied to hospital data
naTestImpute <- indivHosp_20220207 %>%
    select(!where(is.numeric), names(hhsMapper)) %>%
    colRenamer(vecRename = hhsMapper) %>%
    select(hospital_pk, collection_week, adult_beds, total_beds, inpatient_beds) %>%
    arrange(hospital_pk, collection_week) %>%
    group_by(hospital_pk) %>%
    mutate(across(c(total_beds, adult_beds), 
                  .fns=function(x) testImputeNA(x=x, y=inpatient_beds, naValues=-999999)
                  )
           )
...
Further exploration of the decline in reported beds is merited:  
```{r, fig.height=9, fig.width=9}

metricDelta <- naTestImpute %>%
    filter(collection_week %in% as.Date(c("2022-01-21", "2021-12-31"))) %>%
    arrange(hospital_pk, collection_week) %>%
    summarize(across(where(is.numeric), 
                     .fns=function(x) ifelse(x[2]==-999999, 0, x[2])-ifelse(x[1]==-999999, 0, x[1])
                     )
              )

metricDelta %>%
    pivot_longer(-c(hospital_pk)) %>%
    group_by(hospital_pk) %>%
    mutate(maxAll=max(abs(value))) %>%
    ungroup() %>%
    filter(maxAll > 100) %>%
    pivot_wider() %>%
    arrange(desc(maxAll)) %>%
    head(20) %>%
    left_join(select(indivHosp_20220207[indivHosp_20220207$collection_week=="2022-01-21", ], 
                     hospital_pk, 
                     hospital_name, 
                     city, 
                     state
...
There appears to be a significant reporting change among some large hospitals. Hospital data are compared to the CDC daily files:  
```{r, fig.height=9, fig.width=9}

df1 <- indivHosp_20220207 %>%
    select(!where(is.numeric), names(hhsMapper)) %>%
    colRenamer(vecRename = hhsMapper) %>%
    pivot_longer(where(is.numeric)) %>%
    group_by(collection_week, name) %>%
    summarize(value=sum(ifelse(is.na(value), 0, ifelse(value==-999999, 0, value))), .groups="drop")
df2 <- cdc_daily_220206$dfPerCapita %>%
    group_by(date) %>%
    summarize(across(c(inp, hosp_adult, hosp_ped), .fns=function(x) sum(ifelse(is.na(x), 0, x))), .groups="drop")

df1 %>%
    filter(name %in% c("adult_beds_covid", "adult_beds_occupied", "icu_beds_occupied", "adult_icu_covid")) %>%
    select(date=collection_week, everything()) %>%
    ggplot(aes(x=date)) + 
    geom_line(aes(y=value/1000)) + 
    geom_line(data=df2, aes(y=hosp_adult/1000), color="red") +
    facet_wrap(~name) + 
    labs(title="Hospital occupancy summary - 50 states plus DC", 
         subtitle="Red line is variable ''hosp_adult' from CDC daily; black lines are metrics from hospital file",
         y="Beds occupied (000)", 
         x=NULL
         )
...
The 'adult_beds_covid' field in the hospital data and the 'hosp_adult' field in the CDC daily data track closely together.

Updated maps with imputed capacity are created:  
```{r, fig.height=9, fig.width=9}

modStateHosp_20220207 <- indivHosp_20220207 %>%
    filter(state %in% c(state.abb, "DC")) %>%
    colSelector(c("state", "collection_week", "hospital_pk", names(hhsMapper))) %>%
    colRenamer(hhsMapper) %>%
    mutate(across(where(is.numeric), .fns=function(x) ifelse(is.na(x), NA, ifelse(x==-999999, NA, x)))) %>%
    arrange(hospital_pk, collection_week) %>%
    group_by(hospital_pk) %>%
...
# ICU summary
createGeoMap(modStateHosp_20220207, 
             yVars=list("pctCovidICU"=c("label"="Covid", "color"="red"), 
                        "pctICU"=c("label"="Total", "color"="black")
                        ), 
             fullList=list("pctICU"=expression(icu_beds_occupied/icu_beds), 
...
Imputation leads to more reasonable plots of hospital capacity. CT appears to still be an outlier on percent capacity in the most recent weeks.

The latest hospital data are downloaded:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

# Run for latest data, save as RDS
indivHosp_20220218 <- downloadReadHospitalData(loc="./RInputFiles/Coronavirus/HHS_Hospital_20220218.csv")
saveToRDS(indivHosp_20220218, ovrWriteError=FALSE)

```
  
An updated summary is created for all US hospitals:  
```{r, fig.height=9, fig.width=9}
...
indivHosp_20220218 %>% 
    ...
    pull(hospital_pk) %>%
    plotHospitalUtilization(df=indivHosp_20220218, keyHosp=., plotTitle="US Hospitals Summed")

```

There continues to be missing data in the fields for adult_beds and total_beds. Moving forward, a new metric should be selected for capacity calculations.