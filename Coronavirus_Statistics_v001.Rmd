...
stateCountySummary <- function(states, 
    ...
    p3 <- filteredStateData %>%
        ...
        coord_flip() + 
        facet_wrap(~c("cumDeaths"="1. Total deaths", "dpm"="2. Deaths per million")[name], scales="free_x") +
        labs(x="Cluster", y="", title="Deaths (total and per million) by cluster")

    # Plot 4: Evolution of deaths by time
    p4 <- filteredStateData %>%
        filter(!is.na(dpm7)) %>%
    ...
    # Create a 2x2 summary of the state
    gridExtra::grid.arrange(p1, p2, p3, p4, nrow=2, top=paste0("States: ", paste0(states, collapse=", ")))
    
    # Return the state data file if requested
    if (returnData) filteredStateData
