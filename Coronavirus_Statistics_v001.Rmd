...
A few summaries are created without using functional form:  
```{r}
...
clusterStates <- function(df, 
    ...
    # method: the method for hierarchical clustering (e.g., 'complete' or 'single')
    ...
    print(arrange(dfCluster, desc(state)))
    
    # Create hierarchical segments or kmeans segments
    if (hierarchical) {
        distData <- dfCluster %>%
            column_to_rownames("state")
        objCluster <-  hclust(dist(distData), method=method)
        plot(objCluster)
    } else {
        cat("\nK-means is not yet implemented\n")
        objCluster <- NULL
    }
...
assessClusters(clustVec)

```
  
At a glance, the segments appear reasonable:  
  
* Segments 4 and 5 had early disease, but with less pronounced spikes in segment 5 than in segment 4, but with a greater case rebound in segment 5 than in segment 4    
* Segments 2 had late disease, with little spike early  
* Segments 1 and 3 have generally had few deaths, though with spiky cases that are similarly shaped if much lower than segment 2  
