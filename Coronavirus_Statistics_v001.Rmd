...
Next steps are to explore states that fall in the late pandemic segment and the consistent pandemic segment.  A few late pandemic states are explored (FL, TX, AZ):  
```{r}
...
Correlations between cases and deaths are generally a bit lower, with the highest correlations coming at around 3 weeks lag.  Deaths as a proportion of cases tend to be in the 2% range, with significant declines from early in the pandemic to late in the pandemic.  There are obvious anomalies of death reporting where some 7-day periods are close to zero, in between periods with high deaths.  Further smoothing of the data may be required.

Lastly, a state with fairy consistent disease (LA) is assessed:  
```{r}
...
The individual counties of Louisiana appear to be a mix of those hit earlier, those hit later, and those hit consistently.  In many of the counties, correlations between cases and deaths are rather low at every value of lag from 0-30 days.  Death rates range from ~1.5% (Lafayette, which looks like a "late hit" segment) to ~5% (Orleans, which looks like an "early hit" segment).

Next steps are to explore in more detail the "excess deaths" available from the CDC, and to see how that relates to the cases and deaths data.
...
The file has been downloaded and has data through the week ending 12-SEP-2020.  Fields include:  
  
* Jurisdiction (state)  
* Week Ending  
* State Abbreviation  
* Year  
* Week (week of year, starts at 1)  
* Age Group (bucketed)  
* Number of Deaths  
* Time Period (2015-2019 or 2020)  
* Type (Predicted (weighted) or Unweighted)  
* Suppress (whether data are suppressed and not reported)  
* Note  
  
The data are read in, with names modified and data types wrangled as appropriate:  
```{r}

fName <- "./RInputFiles/Coronavirus/Weekly_counts_of_deaths_by_jurisdiction_and_age_group_downloaded_20200923.csv"

cdcRaw <- readr::read_csv(fName, col_types="ccciicdcccc")
glimpse(cdcRaw)

cdcRenamed <- cdcRaw %>%
    rename(weekEnding=`Week Ending Date`, 
...
Next, key fields are examined for which values they contain:  
```{r}
...
helperCDCFields("age")
helperCDCFields(c("period", "year", "type"))
helperCDCFields(c("period", "Suppress"))
helperCDCFields(c("period", "Note"))
helperCDCFields(c("state", "Jurisdiction")) %>% 
    arrange(-deaths) %>% 
    as.data.frame()
helperCDCFields(c("year", "week")) %>% 
...
A number of follow-up steps are identified:  
  
* Age should be converted to a factor, with levels of ascending age  
* Year should be converted to a factor  
* Period should be converted to a factor  
* Data should be limited to type "Predicted (weighted)"  
* Data should be limited to 2015-2019 and 2020 through week 30 (potential incompleteness afterwards)  
* The records that are listed as suppressed should be checked  
* The records for full US and Puerto Rico should be deleted, and the record for YC (New York City) should be investigated to see if it is part of NY (New York) or a standalone  
* Remaining data should be double-checked against an external reference for total death counts  
  
First, variables are converted to factors:  
```{r}
...
Next, data are limited to "Predicted (weighted)" and up through 2020 week 30:  
```{r}

cdcFiltered <- cdcFactored %>%
    filter(type=="Predicted (weighted)", period=="2015-2019" | week <= 30)
glimpse(cdcFiltered)
...
The data suppressions all fell in weeks that have been excluded.  The remaining issue is PR, YC, NY, and US:  
```{r}

cdcFiltered %>%
    mutate(newState=ifelse(state %in% c("US", "PR", "NY", "YC"), state, "Other")) %>%
    group_by(newState) %>%
...
Adding the deaths of all sub-elements (including PR, NY, YC) prodices 15.931 million deaths while US reports 15.815 million deaths, for a small discrepancy.  Is this 1% gap constant by year?  
```{r}

cdcFiltered %>%
    mutate(newState=ifelse(state=="US", "US", "Other")) %>%
    group_by(year, newState) %>%
    summarize(deaths=sum(deaths)) %>%
    pivot_wider(year, names_from="newState", values_from="deaths") %>%
    mutate(ratOthUS=Other/US)

```
  
Consistently, the sum of the parts is about 1% greater than the whole.  All of the potentially questionable parts (PR, NY, YC) make up more than 1% of the total and are not obvious explanatory components.  This analysis will continue by 1) excluding US and PR, and 2) adding YC to NY for comparisons to NY in other data:  
```{r}
...
cdcProcessed <- cdcFiltered %>%
    ...
    group_by(Jurisdiction, weekEnding, state, year, week, age, period, type, Suppress, Note) %>%
    summarize(n=n(), deaths=sum(deaths)) %>%
    ungroup()
...
The file has been made unique and in theory contains all deaths from US states plus DC for 2015 through week 30 of 2020.  Total deaths counts by year are:  
```{r}
...
cdcProcessed %>%
    ...
    labs(title="CDC Deaths for 50 states plus DC", 
         x="", 
         y="Deaths (000s)", 
         subtitle="Through week 30 of 2020"
         )

```
  
Published data suggest the US had 2.712 million deaths in 2015 and 2.813 million deaths in 2017.  The data for this analysis are about 1% under, at least partly driven by the exclusion of Puerto Rico.  Given the many other uncertainties in the analysis, the data appear sufficiently accurate for exploration.

As a starting point, deaths across all jurisdictions are explored by year, in total and then facetted by age:  
```{r}
...
The first wave of the virus is clear on the plots, with the early part of the second wave showing and more data needed to see the extent to which deaths above historical norms are associated with the same timing as this wave.

Of interest, there is a visible impact of increased deaths around the same time as the first wave in all age groups with the exception of "Under 25 years".  Consistent with published report, the bulk of the absolute death increase is in the elderly population, especially the 75-84 year and 85+ year populations.

The same analysis can be run using the segments created previously for states:  
```{r}
...
The data are broadly consistent with previous observations for the clusters:  
  
* An "early" cluster that has very high deaths in the early pandemic that declines back to baseline  
* An "early" cluster that has moderate deaths increases in the early pandemic, declining to a plateau above basline  
* A "late" cluster that has moderate deaths in the late pandemic (CDC all-cause data not yet able to show a peak or decline, if any)  
* Two clusters that have had relatively mild impact from virus, though deaths above baseline are observed in these segments also  
  
To a first order, the deaths from coronavirus appear associated with increases in all-cause deaths from the same geographies.

Suppose that deaths by cluster in 2020 are assessed by age group:  
```{r}
...
Withouth the 2015-2019 baseline, three of the clusters show an obvious pandemic surge, while the increase is more subtle (slight summer increase when historically there is a slight summer decrease) in the remaining two segments.

Next steps are to further explore the "excess deaths" by component and compare to the published deaths due to coronavirus.

Given a set of death data from 2015-2019, an estimate can be made for expected deaths in 2020.  Broadly, there are two components:  
  
1.  Deaths per year may be tending to increase or decrease over time  
2.  Deaths by year generally occur in a seasonal manner, with higher deaths in the winter than in the summer  
  
Deaths in the winter are also generally subject to greater fluctuations, potentially driven among other things by severity in that season's influenza strain.

As an example, suppose that the full US data is explored for the 65-74 year old cohort:  
```{r}

# All deaths by week and year for the 65-74 cohort
allDeath_6574 <- cdcProcessed %>%
...
# Plot of all deaths for the 65-74 cohort by week and year
allDeath_6574 %>%
    ggplot(aes(x=week, y=deaths, color=year, group=year)) + 
    geom_line() + 
    ylim(c(0, NA)) + 
    labs(x="Week", y="Deaths", title="All-cause deaths for 65-74 year old cohort")

```
  
Suppose that only the 2015-2019 data were used, with a goal of understanding the mean and spread of the historcal data:  
```{r}
...
p1 <- allDeath_6574 %>%
    ...
    labs(x="Week", 
         y="Deaths", 
         title="2015-2019 all-cause deaths (65-74 years)", 
         subtitle="Line is 2015-2019 annual mean deaths, ribbon is +/- 1 standard deviation"
         ) + 
    ylim(c(0, NA))
p1
...
Suppose that a simple linear regression is used to estimate the deaths by month for 2020.  The assumption is that the shape follows the mean shape while the height is a linear model of height changes from 2015-2019:  
  
Deaths = f(Week as factor, Year as integer) with no intercept since the week component will drive this  
  
```{r}

# Very basic linear model for deaths vs. week and year
lm6574 <- allDeath_6574 %>%
    filter(year != 2020) %>%
    lm(deaths ~ weekfct + I(yearint-2015) + 0, data=.)

# Prediction for 2020
allDeath_6574 %>%
    mutate(pred=predict(lm6574, newdata=.)) %>%
    ggplot(aes(x=week)) + 
    geom_line(data=~filter(., year != 2020), aes(y=deaths, color=year, group=year)) + 
    geom_line(data=~filter(., year == 2020), aes(y=pred), lty=2, size=1) +
    ylim(c(0, NA)) + 
    labs(x="Week", 
         y="Deaths", 
         title="All-cause deaths for 65-74 year old cohort", 
         subtitle="Solid lines are actual 2015-2019, dashed line is predicted 2020"
         )

```
  
Of note, 2018 had an unusually high death rate for 65-74 year olds in the first five weeks of the year.  For the remainder of the year, 2018 broadly had more deaths than 2015 and fewer than 2019.  As such, the baseline prediction is for fewer deaths in 2020 early in the year than occurred in 2018, though with significant uncertainty given that the flu season (or other factors) could be severe or mild or anywhere in between.

Predictions are available for each year, and it is known and expected that the predictions will be inaccurate, particularly in the winter.  Facetted plots are produced to see the impact:  
```{r}

# Predictions and deltas for 2015-2020
allPred_6574 <- allDeath_6574 %>%
    mutate(pred=predict(lm6574, newdata=.), delta=(deaths-pred)) %>%
    group_by(year) %>%
    arrange(week) %>%
...
# Plot of predictions and actuals facetted by year
allPred_6574 %>%
    ...
    labs(x="Week", 
         y="Deaths", 
         title="All-cause deaths for 65-74 year old cohort", 
         subtitle="Predictions are based on a simple linear model for 2015-2019 data"
         ) + 
    scale_color_discrete("Deaths")

```
  
The predictions are generally close to the actuals for 2015-2019, with some misses early in the year for 2018 (higher deaths) and 2016 (lower deaths).  The excess deaths in 2020 are obvious by comparison, and occur around the same time as the coronavirus outbreak.  This is suggestive that coronavirus (including disease and response to disease) may be a significant driver of these excess deaths in the 65-74 year old cohort.

A look at the percentage of deaths vs. predictions can also be taken:  
```{r}
...
allPred_6574 %>%
    ...
    labs(x="Week", 
         ...
         title="Actual vs. predicted deaths for 65-74 year old cohort", 
         subtitle="Predictions are based on a simple linear model for 2015-2019 data"
         )

```
  
Using 2015-2019 as the baseline, it appears normal to have swings of 5%-10% in deaths vs. predicted during the very early and very late weeks of the year (winter).  During the remainder of the year, predicted and actual deaths are usually fairly tightly aligned.  This suggests that area under the curve starting at around week 10 may be a good gauge for excess deaths in 2020, excluding any impact of the early 2020 flu season (or other wintry factors).

The cumulative proportion of death vs. prediction can also be plotted:  
```{r}

# Plot of cumulative predictions and actuals facetted by year
allPred_6574 %>%
    mutate(rat=cumDelta/cumPred) %>%
    ggplot(aes(x=week)) + 
    geom_line(aes(y=rat)) + 
    facet_wrap(~year) + 
    geom_hline(yintercept=0, lty=2) +
    labs(x="Week", 
         y="Excess deaths vs. prediction (cumulative, as %)", 
         title="Cumulative excess deaths as percentage of cumulative prediction for 65-74 year old cohort", 
         subtitle="Predictions are based on a simple linear model for 2015-2019 data"
         )

```
  
The impact of winter can again be seen.  The 2020 data stand out as moving away from the 0% line as the year proceeds, in contrast to 2015-2019 where the trend is for predicted and actual deaths to fairly closely align as the year proceeds.  Among the 65-74 year old cohort, there appear to be ~10% more deaths than expected through week 30 of 2020, with a possible uptick (need more data) occurring between weeks 25-30.

As a very basic heuristic, the change in cumulative deaths from week 9 onwards can be defined as the amount of excess deaths absent the impact of winter.  How does this look for the 65-74 year old cohort?  
```{r}
...
allPred_6574 %>%
    ...
    geom_hline(yintercept=0, lty=2) +
    labs(x="Week", 
         y="Excess deaths vs. prediction (cumulative after week 8)", 
         title="Cumulative excess deaths after week 8 for 65-74 year old cohort", 
         subtitle="Predictions are based on a simple linear model for 2015-2019 data"
         )
...
In previous years, predictions have generally been accurate to within +/- 5000 deaths.  In 2020 so far, the prediction is low by about 40,000 deaths.  An estimate of 40,000 +/- 5000 excess deaths among the 65-74 year old cohort through week 30 of 2020 seems like a plausible initial stake in the ground.

This process can be converted to functional form and repeated to make estimates for the other age cohorts, which is the next step.

A function is written to prepare subsetted data:  
```{r}
...
allDeath_65plus <- subsetCDC(cdcProcessed, 
                             critFilter=list("age"=c("65-74 years", "75-84 years", "85 years and older")), 
...
Next, functions are written to run the regression and attach the predictions to the data frame:  
```{r}
...
lm_65plus <- cdcRegression(allDeath_65plus)
...
# Check that the function works
allPred_65plus <- cdcPrediction(allDeath_65plus, lmReg=lm_65plus)
allPred_65plus

```
  
Next, functions are written to plot the differences in deaths and expectations:  
```{r}
...
# Test that the function works
cdcPredictedvsActual(allPred_65plus, cohortName="65+ years old")

```
  
Next steps are to combine the functions so that all of the steps can be run with a single call:  
```{r}
...
# Test the function for the 65+ years old cohort
...
# Test the function for the under 25 years old cohort
list_under25 <- cdcCohortAnalysis(cohortName="Under 25 years old", 
                                  critFilter=list("age"=c("Under 25 years")), 
                                  plotTitle="All-cause deaths for under 25 years old cohort"
                                  )

# Test the function for the 25-64 years old cohort
list_25to64 <- cdcCohortAnalysis(cohortName="25-64 years old", 
                                 critFilter=list("age"=c("25-44 years", "45-64 years")), 
                                 plotTitle="All-cause deaths for 25-64 years old cohort"
                                 )

```
  
Findings are broadly as would be expected based on published data about coronavirus (note that due to the cutoff of incomplete CDC data, these findings are broadly through the end of July 2020):  
  
* Around 1,000 excess deaths in the 0-24 years old cohort, and with a large standard error on this estimate since deaths in this cohort tend to be small in number and highly variable in percentage each year  
* Around 50,000 excess deaths in the 25-64 years old cohort, in contrast to a typical year that sees under 10,000 deaths difference between predictions and actuals  
* Around 150,000 excess deaths in the 65+ years old cohort, in contrast to a typical year that sees under 10,000 deaths difference between predictions and actuals  
  
Combining the cohorts, these data suggest that roughly 200,000 more Americans died through July 2020 than would have been expected given a linear projection (including seasonality) of deaths from 2015-2019.  

This excess is greater than the number of reported coronavirus deaths through July 2020.  Among the publicized hypotheses are 1) some coronavirus deaths may have been missed due to no diagnosis, and 2) some deaths may be due to reactions to coronavirus (e.g., deferred medical care for other conditions, deaths of despair, etc.)  
  
For comparison, the analysis is run using the full nation as the cohort.  It is known and expected that an analysis like this will not have the sum of the parts equal the whole, though findings should be broadly consistent:  
```{r}

# Test the function for the full US
...
As hoped for, the findings of the all-US analysis are that there are roughly 200,000 excess deaths through July 2020, and where a typical year would be within 10,000 deaths of predicted through July.  The impact of the pandemic is clear visually, as are the two waves (arguably, there is a first wave at different times in norther and southern geographies).

The analysis is also extended to look at a handful of the early-pandemic states and late pandemic states:  
```{r}

# Test the function for a few states that were hit early and hard
...
# Test the function for a few states that were hit late and hard
list_late <- cdcCohortAnalysis(cohortName="all ages, AZ/TX/MS/FL", 
                               critFilter=list("state"=c("AZ", "TX", "MS", "FL")),
                               plotTitle="All-cause total deaths for AZ/TX/MS/FL"
                               )

```
  
* Four early-pandemic states (NY, NJ, CT, MA) have roughly 70,000 excess deaths as of July 2020, and curves that suggest death rates have returned to predictions (historical norms) by the end of July 2020.  This is broadly consistent with the shape of the reported coronavirus deaths in these states  
  
* Four late-pandemic states (AZ, TX, MS, FL) have roughly 30,000 excess deaths as of July 2020, and curves that suggest death rates remain elevated as of the end of July 2020.  There is evidence of impact in the early pandemic as the normal post-winter decline in deaths was not observed in 2020.  There is strong evidence of the late pandemic impact on excess deaths starting around week 25 (late June).  More recent data will be needed to see if the all-cause deaths for these states mirror the reported coronavirus deaths  
  
Next steps are to pull the data for additional cohorts by state, and to compare the all-causes excess deaths data to the reported coronavirus deaths and cases for those same states.
  
The death estimation methodology requires a certain level of deaths such that signal dominates over noise.  For example, how well does the methodology work for Wyoming?  
```{r}

# Test the function for the smallest state (WY)
list_wy <- cdcCohortAnalysis(cohortName="all ages, WY", 
                             critFilter=list("state"=c("WY")),
                             plotTitle="All-cause total deaths for WY"
                             )

```
  
There are not many expected deaths in a typical week in Wyoming, thus it is common for some high percentage fluctuations to occur even in the absence of pandemic.  Despite that, there are signs that there has been very limited impact of virus in Wyoming so far in 2020.

Suppose instead that the methodology is applied to Nebraska (3x the size of Wyoming)?
```{r}

# Test the function for a small-mid sized state (NE)
list_ne <- cdcCohortAnalysis(cohortName="all ages, NE", 
                             critFilter=list("state"=c("NE")),
                             plotTitle="All-cause total deaths for NE"
                             )

```
  
There is still a fair bit of noise, and also evidence that there has been limited impact of coronavirus on excess deaths through July 2020.

Suppose that a state of similar size that was hit early is explored?
```{r}

# Test the function for a small-mid sized state (NM)
list_nm <- cdcCohortAnalysis(cohortName="all ages, NM", 
                             critFilter=list("state"=c("NM")),
                             plotTitle="All-cause total deaths for NM"
                             )

```
  
While the noise issue remains the same, there is a clear signal for pandemic in 2020, with roughly 1000 excess deaths by the end of 2020 where the random fluctuations in a typical year would produce +/- 200 excess deaths.

Suppose then that the methodology is run by segment, using the segments produced previously by state:  
```{r}

listClusters <- vector("list", 5)

for (cluster in 1:5) {
    clusterStates <- names(clustVec[clustVec==cluster])
    listClusters[[cluster]] <- cdcCohortAnalysis(cohortName=paste0("all ages, cluster ", cluster), 
                                                 critFilter=list("state"=clusterStates),
                                                 plotTitle=paste0("All-cause total deaths for cluster ",
                                                                  cluster
                                                                  ), 
...
Predictions by cluster are reasonably smooth, and there is reason for optimism that predictions are plausibly in range for deaths by cluster that might have occurred in 2020 had it stayed on trend with 2015-2019.

The "excess deaths" by cluster can then be compared with the "coronavirus deaths" by cluster to see the relationships.  First, the "excess deaths" data files are consolidated:  
```{r}

# Create the excess cluster file
excessCluster <- map_dfr(.x=listClusters, .f=function(x) { x[["preds"]] }, .id="cluster") %>%
    mutate(cluster=as.integer(cluster))
excessCluster

# Attach population by cluster
excessCluster <- stateData %>%
    mutate(cluster=clustVec[state]) %>%
    group_by(cluster) %>%
    summarize(pop=sum(pop)) %>%
    right_join(excessCluster, by="cluster") %>%
    mutate(cluster=as.character(cluster))
excessCluster

# Plot of excess deaths by week by cluster
excessCluster %>%
    filter(year==2020) %>%
    ggplot(aes(x=week, y=delta)) + 
    geom_line(aes(group=cluster, color=cluster)) + 
    labs(title="Actual minus predicted deaths by cluster and week (2020)", 
         x="Week", 
...
The coronavirus death data by cluster can be transformed to week and compared against the excess all-cause deaths by week:  
```{r}

# Explore the conversion of week by week ending data in the raw CDC data
cdcRaw %>% 
    filter(Year==2020) %>% 
    count(Week, `Week Ending Date`) %>%
    as.data.frame()

# Pull the cluster-level data and create a CDC week number
fullDeathData <- consolidatedPlotData_20200830 %>%
    filter(name=="deaths", state=="cluster") %>%
    ungroup() %>%
    ...
    ungroup() %>%
    right_join(excessCluster, by=c("cluster", "week")) %>%
    mutate(cvDeaths=ifelse(is.na(cvDeaths), 0, cvDeaths))
fullDeathData

```
  
The death totals can then be compared:  
```{r}

# Plot for total excess deaths and reported coronavirus deaths by cluster by week
fullDeathData %>%
    filter(year==2020) %>%
    select(cluster, week, cvDeaths, pop, delta) %>%
    pivot_longer(-c(cluster, week, pop), names_to="source", values_to="deaths") %>%
    ggplot(aes(x=week, y=deaths)) + 
    geom_line(aes(group=source, color=c("cvDeaths"="Reported COVID", "delta"="Excess All-Cause")[source])) + 
    facet_wrap(~cluster) + 
    scale_color_discrete("Deaths")

```
  
Among the three clusters with spikes, it appears that excess all-cause deaths start earlier and peak higher than reported COVID deaths.  The cause for this delta is not discernible from this simple look at the data.  Shapes of the all-cause excess deaths curve and the COVID death curves are generally similar, with each cluster looking much more like itself than like the other clusters.  This is suggestive, though not dispositive, that coronavirus deaths are a meaningful component of excess all-cause deaths.
  
Next steps are to explore further by state to see if the trend (earlier state, higher peak) is common across states or driven by a subset of states.

The methodology is now run by state:  
```{r}

listStates_v001 <- vector("list", 51)

for (abbState in names(clustVec)) {
    listStates_v001[[abbState]] <- cdcCohortAnalysis(cohortName=paste0("all ages, state ", abbState), 
                                                     critFilter=list("state"=abbState),
                                                     plotTitle=paste0("All-cause total deaths for state ",
                                                                      abbState
                                                                      ), 
                                                     showSubsetPlots=FALSE,
                                                     showPredActualPlots=FALSE
                                                     )
}

```
  
The deaths data are consolidated to a single file:  
```{r}

# Create the excess cluster file
excessState_v001 <- map_dfr(.x=listStates_v001, .f=function(x) { x[["preds"]] }, .id="state")
excessState_v001

```
  
Excess deaths by state in 2020 can be plotted:  
```{r}

excessState_v001 %>%
    filter(year==2020) %>%
    mutate(quarter=factor(paste0("Q", ceiling(week/13), "-20"))) %>%
    ...
    labs(x="State", 
         ...
         subtitle="CDC data through July 2020 (Q3 incomplete)"
         ) +
    scale_fill_discrete("Quarter")

```
  
Many states appear to have been running slightly under on excess deaths in the first quarter, a trend which reversed in many locations around the same time as coronavirus deaths began to peak (Q2-20).

The same plot can be created on a per-capita basis:  
```{r}
...
excessState_v001 %>%
    ...
    summarize(excess=sum(delta)) %>%
    ungroup() %>%
    left_join(select(usmap::statepop, abbr, pop_2015), by=c("state"="abbr")) %>%
    mutate(excesspm=excess*1000000/pop_2015) %>%
...
The data can also be plotted per month, starting with March:  
```{r}

pes_001 <- excessState_v001 %>%
    filter(year==2020, week>=10) %>%
    mutate(quarter=factor(paste0("Q", ceiling(week/13), "-20")), 
           month=factor(case_when(week <= 13 ~ "Mar", 
                                  week <= 18 ~ "Apr", 
                                  week <= 22 ~ "May", 
                                  week <= 26 ~ "Jun", 
                                  week <= 31 ~ "Jul", 
                                  TRUE ~ "Error"), 
                        levels=c("Mar", "Apr", "May", "Jun", "Jul", "Error")
                        )
           ) %>%
    group_by(state, month) %>%
    summarize(excess=sum(delta)) %>%
    ...
    labs(x="State", 
         y="Excess Deaths (per million people)", 
         title="All-cause excess deaths per million people in 2020", 
         subtitle="CDC data through July 2020", 
         caption="Epi Week to Month: Mar (10-13), Apr (14-18), May (19-22), Jun (23-26), Jul (27-31)"
         ) +
...
Further, coronavirus deaths by state (through July and through) can be added to the plot:  
```{r}

cumDeathState_001 <- consolidatedPlotData_20200830 %>% 
    filter(name=="deaths", state != "cluster") %>% 
    group_by(state) %>% 
    mutate(cumdpm=cumsum(vpm)) %>%
    filter(date %in% c(as.Date("2020-07-31"), max(date)))

pes_001 + 
    geom_point(data=filter(cumDeathState_001, date==as.Date("2020-07-31")), aes(x=state, y=cumdpm), size=2) + 
    labs(title="Deaths per million people using data through July 2020", 
         subtitle="Bars based on CDC all-cause deaths, points are reported coronavirus deaths", 
         y="Deaths (per million people) through July 2020"
         )

```
  
Of interest, reported coronavirus deaths through July 2020 are a very different percentage of all-cause excess deaths as estimated using CDC data for some states.  There is a clear correlation where most states with a high number of excess all-cause deaths also report a high number of coronavirus deaths per capita.

Suppose that the ratio is of interest:  
```{r}
...
excessState_v001 %>%
    ...
    left_join(select(usmap::statepop, abbr, pop_2015), by=c("state"="abbr")) %>%
    mutate(deltapm=delta*1000000/pop_2015) %>%
    inner_join(select(filter(cumDeathState_001, date==as.Date("2020-07-31")), state, cumdpm)) %>%
    mutate(rat=deltapm/cumdpm) %>%
    ...
    labs(x="State", 
         ...
         subtitle="All data through July 2020"
         ) + 
    ylim(c(0, NA)) + 
...
The states with the most extreme ratios tend to be small, raising the question of whether there is just a methodology error for smaller locales.  The state of Hawaii has had negative estimated excess deaths and its ratio of nearly -200% is not plotted.

A few states of interest are assessed using plots:  
```{r}

for (abbState in c("HI", "VT", "WY", "WV", "OR", "TX")) {
    cdcCohortAnalysis(cohortName=paste0("all ages, state ", abbState), 
                      critFilter=list("state"=abbState),
                      plotTitle=paste0("All-cause total deaths for state ", abbState), 
                      showSubsetPlots=FALSE,
                      showPredActualPlots=TRUE,
                      predActualPlotsOnePage=TRUE
                      )
...
* Texas appears to have meaningful excess deaths starting around week 15 and then spiking around week 25; the methodology for estimating excess all-cause deaths seems reasonable for Texas; there may be more coronavirus impact in Texas through July 2020 than is captured in coronavirus deaths reported in Texas through July 2020  
  
* Oregon all-cause excess deaths have historically been noisy, particularly in 2018 and 2019; there appears to be an increase in 2020, but the signal to noise ratio is low and these could just be normal fluctuations  
  
* West Virginia appears to be showing an increase in 2020 all-cause deaths that is above the level of fluctuations that have been observed in previous years  
  
* Wyoming all-cause excess deaths have historically been noisy and there is no obvious pattern in 2020 that stands out as aberrant in comparison to previous years; the general direction, as almost everywhere, is up, but any standard error estimate would likely be at least as large as the increase  
  
* Vermont all-cause death data is historically noisy, but there is a clear 2020 signal that dominates the noise and is fairly consistent throughout the year; while Vermont deaths by week in 2015-2019 tend to be 100% +/- 10% of expected, in 2020 they appear to be 115% +/- 10% of expected; there may be more coronavirus impact in Vermont through July 2020 than is reflected solely in the coronavirus deaths data  
  
* Hawaii all-cause death data is historically noisy and the 2020 all-cause deaths are in line with patterns seen in 2015-2019; the null hypothesis of no trend change for all-cause deaths in Hawaii would likely survive any statistical tests based solely on these data  
  
Next steps are to look at the age demographics that make up the excess deaths in the clusters and some of the more interesting (off trend) states.
  
First, excess deaths by age group are explored:  
```{r}

listAges_v001 <- vector("list", 6)

n <- 1
for (age in levels(cdcProcessed$age)) {
    listAges_v001[[n]] <- cdcCohortAnalysis(cohortName="", 
                                            critFilter=list("age"=age),
                                            plotTitle="", 
                                            showSubsetPlots=FALSE,
                                            showPredActualPlots=FALSE
                                            )
...
names(listAges_v001) <- levels(cdcProcessed$age)
...
# Create the excess cluster file
excessAge_v001 <- map_dfr(.x=listAges_v001, .f=function(x) { x[["preds"]] }, .id="age")
excessAge_v001
...
The bulk of the excess deaths occur in the elderly population, with over 50,000 excess deaths in the 85+ bucket, over 40,000 excess deaths in the 75-84 year bucket, and roughly 40,000 excess deaths in the 45-64 year and 65-74 year buckets.

Estimates for US population by age group are pulled from the dataset survival::uspop2 (should updated to census data at some point):  
```{r}
...
usPopBucket2020 <- usPopAge2020 %>%
    mutate(age=factor(case_when(ageActual <= 24 ~ "Under 25 years", 
                                ...
                                ), levels=levels(cdcProcessed$age)
                      )
           ) %>%
    group_by(age) %>%
    summarize(pop_2020=sum(pop_2020))
usPopBucket2020
...
pea_001 <- excessAge_v001 %>%
    ...
    mutate(quarter=factor(paste0("Q", ceiling(week/13), "-20")), 
           ...
           age=factor(age, levels=levels(cdcProcessed$age))
           ) %>%
    group_by(age, month) %>%
    summarize(excess=sum(delta)) %>%
    ungroup() %>%
    inner_join(usPopBucket2020, by="age") %>%
    mutate(excesspm=1000000*excess/pop_2020) %>%
...
The disparity by age is much more evident, with over 8,000 excess deaths per million (0.8%) in the 85+ year old bucket, roughly 3,000 excess deaths per million (0.3%) in the 75-84 year bucket, while persons under age 25 have an excess death rate of 11 per million (well under 0.01%).

For reference, the age group projections are plotted using the 1-page summary:  
```{r}

for (age in levels(cdcProcessed$age)) {
    cdcCohortAnalysis(cohortName=paste0("ages ", age), 
                      critFilter=list("age"=age),
...
There is interesting variation in the shape of the excess death curves by age cohort:  
  
* The 85+, 75-84, and 65-74 cohorts have significant peaks early but (as of July 2020) not much of a second peak  
* The 45-64 cohort has a somewhat more sustained rate of excess deaths starting in weeks 10-15  
* The 25-44 cohort has a sustained rate of excess deaths starting in weeks 10-15  
* The under 25 cohort has a modest rate of excess deaths that is similar in shape to 2016 and 2017  
  
Suppose that state excess deaths are broken down in to those for people 65+ and those for people under 65:  
```{r}

ageOld <- c("65-74 years", "75-84 years", "85 years and older")
cdcTwoAge <- cdcProcessed %>%
...
listAgeState_v001 <- vector("list", 102)

n <- 1
for (age in levels(cdcTwoAge$ageSplit)) {
    for (state in state.abb) {
        listAgeState_v001[[n]] <- cdcCohortAnalysis(df=cdcTwoAge, 
                                                    cohortName="", 
                                                    critFilter=list("ageSplit"=age, "state"=state),
                                                    plotTitle="", 
                                                    showSubsetPlots=FALSE,
                                                    showPredActualPlots=FALSE
                                                    )
...
names(listAgeState_v001) <- paste(rep(levels(cdcTwoAge$ageSplit), each=51), state.abb, sep="_")

# Create the excess cluster file
excessAgeState_v001 <- map_dfr(.x=listAgeState_v001, .f=function(x) { x[["preds"]] }, .id="ageState") %>%
    tidyr::separate(ageState, into=c("age", "state"), sep="_")
excessAgeState_v001

```
  
The proportion of excess deaths by state can then be assessed:  
```{r}
...
Many of the heaviest hit states have roughly 75% of their excess deaths through July 2020 in the 65+ age cohort.  States with smaller amounts of excess deaths through July 2020 have more variability in this ratio, with a general trend that a less percentage of their excess deaths fall in the 65+ age cohort.

A look at the excess death national trends for the two groups:  
```{r}
...
Patterns are consistent with those noticed previously.

Next steps are to update the analysis with data through the end of September, and to plot the evolution of the state-level segments given that updated data.