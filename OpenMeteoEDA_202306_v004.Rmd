...
A correlation heatmap is produced, borrowing from the recipe provided by [STHDA](http://www.sthda.com/english/wiki/ggplot2-quick-correlation-matrix-heatmap-r-software-and-data-visualization):
```{r, fig.height=9, fig.width=9}
...
dfSmallR2TODS <- as.data.frame(mtxSmallTODS) %>% 
    purrr::set_names(c("idx1", "idx2", "r2")) %>% 
    tibble::as_tibble() %>% 
    mutate(var1=useTODS[idx1], var2=useTODS[idx2], rn=row_number()) 
...
dfSmallR2TODS %>% 
    arrange(desc(r2)) %>% 
    filter(!str_detect(var2, "hour|month|doy"), !str_detect(var1, "hour|month|doy")) %>% 
    select(var1, var2, r2) %>% 
    print(n=20)
...
dfSmallR2TODS %>% 
    ...
    geom_hline(yintercept=1, lty=2, color="red") +
    labs(title="Accuracy in every 2-predictor model including self and one other", 
         subtitle="Predicting todSeason (excluding hour, month, and doy)", 
         y="Range of Accuracy (min-mean-max)", 
         x=NULL
    )

# Null accuracy would pick the most frequent observation
allCity %>% count(todSeason, sort=TRUE) %>% mutate(pct=n/sum(n))
...
for(idx1 in 1:(length(possLargeTODS)-1)) {
    for(idx2 in (idx1+1):length(possLargeTODS)) {
        r2LargeTODS <- runFullRF(dfTrain=dfTrainCloud[,] %>% mutate(weathercode=factor(weathercode)), 
                                 yVar="todSeason", 
                                 xVars=possLargeTODS[c(idx1, idx2)], 
                                 dfTest=dfTestCloud %>% mutate(weathercode=factor(weathercode)),
                                 useLabel=keyLabel, 
                                 useSub=stringr::str_to_sentence(keyLabel), 
                                 isContVar=FALSE,
...
dfLargeR2TODS <- as.data.frame(mtxLargeTODS) %>% 
    purrr::set_names(c("idx1", "idx2", "r2")) %>% 
    tibble::as_tibble() %>% 
    mutate(var1=possLargeTODS[idx1], var2=possLargeTODS[idx2], rn=row_number()) 
dfLargeR2TODS %>% arrange(desc(r2)) %>% select(var1, var2, r2) %>% print(n=20)

```

A function is written for running select variables on the full training data:  
```{r}
...
The function is tested for a categorical variable using the function's default full training data:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

comboVarRF(possXVars=c("month", "hour", "shortwave_radiation"), yVar="todSeason", isContVar=FALSE)

```

The function is tested for a continuous variable using the function's default full training data:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

comboVarRF(possXVars=c("precipitation", "rain", "temperature_2m"), yVar="snowfall", isContVar=TRUE)

```

The function is tested for a categorical variable using the select smaller training data:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

# idxSmallTODS created previously - 5,000 indices to use in smaller training
# possLargeTODS created previously as vector of 6 interesting potential predictors
comboVarRF(possXVars=possLargeTODS, 
           yVar="todSeason", 
           isContVar=FALSE, 
           idxTrain=idxSmallTODS
           ) %>%
    arrange(desc(r2))

```

The function is tested for a continuous variable using the select smaller training data:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

# idxSmallTODS created previously - 5,000 indices to use in smaller training
comboVarRF(possXVars=c("precipitation", "rain", "temperature_2m", "weathercode", "cloudcover_mid"), 
           yVar="snowfall", 
           isContVar=TRUE, 
           idxTrain=idxSmallTODS
           ) %>%
...
The process for converting an R2/accuracy matrix to is also converted to functional form:  
```{r}
...
The function is tested on a pre-existing matrix:  
```{r}

# Using defaults
tstDF <- mtxToDFComboVar(mtxSmallTODS, vecVarNames=useTODS)
tstDF

# No printing, no data returned
mtxToDFComboVar(mtxSmallTODS, vecVarNames=useTODS, reportData=FALSE, returnData=FALSE)

# Print 10 items, no data returned
mtxToDFComboVar(mtxSmallTODS, vecVarNames=useTODS, reportData=10, returnData=FALSE)

```

A function is written to explore accuracy by pairs of metrics:  
```{r, fig.height=9, fig.width=9}
...
accComboVarRF(tstDF, 
              yVarName="todSeason", 
              exclVars=c("month", "doy"), 
              isR2=FALSE
              )

accComboVarRF(tstDF, 
              yVarName="todSeason", 
              exclVars=c("month", "doy", "hour"), 
              isR2=FALSE, 
              plotData=FALSE, 
              nPrint=14
              )
...
The process is tested to replicate the todSeason exploration:  
```{r, fig.height=9, fig.width=9, cache=TRUE}
...
# Create the comboVarRF() data
tstComboVar <- comboVarRF(possXVars=useTODS, 
                          yVar="todSeason", 
                          isContVar=FALSE, 
                          idxTrain=idxSmallTODS
                          )
...
The process is further replicated to explore additional variables in more depth:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

# Plot the accuracies excluding month, doy, and hour
accComboVarRF(tstComboVar, 
              yVarName="todSeason", 
              exclVars=c("month", "doy", "hour"),
              isR2=FALSE
...
# Create the comboVarRF() data
tstLargeCombo <- comboVarRF(possXVars=possLargeTODS, 
                            yVar="todSeason", 
                            isContVar=FALSE
                            )

# Plot the accuracies
accComboVarRF(tstLargeCombo, 
              yVarName="todSeason", 
              isR2=FALSE
              )
...
Data for Boston are downloaded, cached to avoid multiple hits to the server:  
```{r cache=TRUE}

# Hourly data download for Boston, MA
testURLHourly <- helperOpenMeteoURL(cityName="Boston MA", 
                                    hourlyIndices=1:nrow(tblMetricsHourly),
                                    startDate="2010-01-01", 
                                    endDate="2023-12-31", 
                                    tz="US/Eastern"
...
# Download file
if(!file.exists("testOM_hourly_bos.json")) {
    fileDownload(fileName="testOM_hourly_bos.json", url=testURLHourly)
} else {
    cat("\nFile testOM_hourly_bos.json already exists, skipping download\n")
}


# Daily data download for Boston, MA
testURLDaily <- helperOpenMeteoURL(cityName="Boston MA", 
                                   dailyIndices=1:nrow(tblMetricsDaily),
                                   startDate="2010-01-01", 
                                   endDate="2023-12-31", 
                                   tz="US/Eastern"
...
# Download file
if(!file.exists("testOM_daily_bos.json")) {
    fileDownload(fileName="testOM_daily_bos.json", url=testURLDaily)
} else {
    cat("\nFile testOM_daily_bos.json already exists, skipping download\n")
}
...
# Read daily JSON file
bosOMDaily <- formatOpenMeteoJSON("testOM_daily_bos.json")

# Read hourly JSON file
bosTemp <- formatOpenMeteoJSON("testOM_hourly_bos.json", addVars=TRUE)
...
At a glance, Boston seems similar to Chicago and NYC on several metrics. The scatter of temperature and dewpoint is also explored:  
```{r, fig.height=9, fig.width=9}
...
Boston exhibits strong correlation between temperature and dewpoint, most similar in magnitude to NYC (slightly less strong than Chicago)

A basic rpart model is run excluding Boston, with predictions made on holdout data including Boston:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

tstRP3 <- allCity %>% 
    filter(tt=="train", src!="Boston") %>% 
    select(fct_src, all_of(varsTrain)) %>% 
    rpart::rpart(fct_src ~ ., data=., method="class")

# Variable importances
tstRP3$variable.importance %>% 
    as.data.frame() %>% 
    purrr::set_names("varImp") %>% 
    rownames_to_column("predictor")
...
# Predictions
allCity %>% 
    mutate(pred=predict(tstRP3, newdata=., type="class")) %>% 
    count(tt, src, pred) %>% 
    ggplot(aes(x=src, y=pred)) + 
    geom_tile(aes(fill=n)) + 
    scale_fill_continuous(low="white", high="green") + 
...
A basic tree model works well to split the data by city, and almost always predicts Boston as NYC (expected) or Miami (unexpected) when trained without Boston data

The single-city GLM function is run for each city, with results collected in a list:  
```{r}
...
The single-city models perform best on Vegas, Miami, Boston, and Houston
...
Predictions based on maximum probability by singleCityGLM() are highly accurate. Boston is predicted on a standalone basis, using only the non-Boston models:  
```{r, fig.height=9, fig.width=9}

glmPreds %>% 
    filter(src=="Boston", glm!="Boston") %>%
    group_by(src, rn) %>% 
    filter(pred==max(pred)) %>%
    mutate(wt=1/n()) %>%
    group_by(src, glm) %>%
...
GLM trained without Boston predict Boston mainly as NYC (~55%) and Chicago (~35%), with a bit of LA (~10%)
