...
* Standard Jacks or Better 9/5 pay table  
* Cash back component of 2.0% (in other words, if you assume a permanent 2.0% loss, how many units WORSE than that are you likely to be at your very lowest point)  
...
The start time is calculated to understand overall performance.  The 9/5 Jacks or Better file is read in, using raw data from <http://wizardofodds.com/games/video-poker/strategy/jacks-or-better/>, with the flush payout manually reduced from 6 to 5.  Further, the cash back component is set to be 2.0%:  
```{r}
startTime <- proc.time()

## Step 1: Read in the outcomes file
baseOutcomes <- getBaseOutcomes(myFileName="JB_95_Probs.csv", forceEQ=TRUE)
...
####_Conclusions_  
The results suggest that assuming a 2.0% loss rate in a 9/5 JB game, it is unlikely that actual results will ever fall more than 8k-12k units behind that benchmark (notably, the number of units is high since the margin for error is low; EXP is -1.6%, not much different from cb of +2.0%.  The simulations match up reasonably well with the theory, suggesting that the "fake" cash back as solved by the polynomial is a plausible representation of the real world.  
