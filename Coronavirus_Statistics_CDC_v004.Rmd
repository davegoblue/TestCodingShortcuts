...
readProcessCDC <- function(fName, 
    ...
    if ((nrow(suppressData) <= maxSuppressAllowed) & (nProblem==0)) {
        ...
        if (nrow(filter(suppressData, state %in% stateNoCheck))==nrow(suppressData)) {
    ...
    cdcProcessed <- cdcFiltered %>%
        rowFilter(lstExclude=list("state"=c("US", "PR"))) %>%
...
readProcessCDC(cdcLoc, weekThru=17, stateNoCheck=c("NC"))

```
  
There are still significant edits to make, but the routine creates a usable CDC deaths database.