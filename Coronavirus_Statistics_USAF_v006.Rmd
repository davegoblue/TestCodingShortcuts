...
cty_vaxdata_20220607 <- processCountyVaccines(loc="./RInputFiles/Coronavirus/county_vaccine_20220607.csv", 
                                              ctyList=cty_newdata_20220606, 
                                              minDateCD=c("2022-02-01", "2022-02-01"),
                                              maxDateCD="2022-05-31"
                                              )

# Save the refreshed file
saveToRDS(cty_vaxdata_20220607, ovrWriteError=FALSE)
...
cty_postdata_20220606 <- postProcessCountyData(lstCtyBurden=cty_newdata_20220606$dfPerCapita, 
                                               lstCtyVax=cty_vaxdata_20220607$vaxFix, 
                                               lstState=readFromRDS("cdc_daily_220602")$dfPerCapita
                                               )

# Save the refreshed file
saveToRDS(cty_postdata_20220606, ovrWriteError=FALSE)
...
# Step 1a: Burden comparisons for aggregated states
additionalCountyPostProcess(cty_postdata_20220606, p1CompareStates=c(state.abb, "DC"), p1AggData=TRUE)

# Step 1: Burden aggregation for key states
# Step 2: vaccine comparisons
# Step 3: Scoring updates (and errors)
# Step 4: New rolling data (28-day default with ceilings 50000 CPM, 500 DPM)
additionalCountyPostProcess(cty_postdata_20220606, 
                            p1CompareStates=c("GA", "FL", "NE"), 
                            p2VaxStates=c("MA", "HI", "TX", "VA", "VT", "GA", "CO", "SD"), 
                            p3VaxTimes=sort(c("2021-06-01", "2022-05-31")),
                            p4DF=cty_newdata_20220606$dfPerCapita
                            )

```
  
Additional plots of relationships between vaccines and burden are generated:  
```{r, fig.height=9, fig.width=9}

# Multiple vaccine variables for counties 25k-500k
purrr::walk(.x=c("mean_vxcpoppct", "mean_vxcgte18pct", "mean_vxcgte65pct"), 
            .f=~integrateCountyBurdenVax(cty_newdata_20220606, cty_vaxdata_20220607, keyDates=c(4, 369)) %>%
                makeIntegratedCountyPlotData() %>%
                plotIntegratedCounty(popRange=c(25000, 500000), vaxVar = .x) %>%
                regIntegratedCounty(vaxVar=.x)
            )
...
# 6-month time period
integrateCountyBurdenVax(cty_newdata_20220606, cty_vaxdata_20220607, keyDates=c(4, 186)) %>%
    makeIntegratedCountyPlotData() %>%
    plotIntegratedCounty(popRange=c(25000, 500000), 
                         vaxVar = "mean_vxcgte18pct", 
                         yLabel = "Growth rate (most recent 6 months)", 
...
A select group of counties can have evolution of burden and vaccines plotted:  
```{r, fig.height=9, fig.width=9}

# Extract county population
ctyPop <- cty_vaxdata_20220607$vaxFix %>% 
    filter(date==max(date)) %>% 
    select(countyFIPS=FIPS, countyName, state, pop) %>%
    mutate(useName=paste0(stringr::str_remove_all(countyName, " County"), ", ", state))

# Pivot vaccines data
vaxPivot <- cty_vaxdata_20220607$vaxFix %>%
    select(countyFIPS=FIPS, date, starts_with("vxc")) %>%
    pivot_longer(-c(countyFIPS, date)) %>%
    filter(!is.na(value))

# Pivot burdens data
burdenPivot <- cty_newdata_20220606$dfPerCapita %>%
    select(countyFIPS, date, tcpm7, cpm7, tdpm7, dpm7) %>%
    pivot_longer(-c(countyFIPS, date)) %>%
    filter(!is.na(value))

# Integrated data
vaxBurden <- bind_rows(vaxPivot, burdenPivot)

# Plot subset of counties (those around 250k population)
tmpCountySelect <- c("29047", "53077", "48245", "42041")
tmpMapper <- c("tdpm7"="Total DPM (7-day avg)", 
               "tcpm7"="Total CPM (7-day avg)", 
               "vxcgte65pct"="% Vax (65+)", 
               "vxcpoppct"="% Vax (all)"
               )
vaxBurden %>%
    filter(countyFIPS %in% c(tmpCountySelect), 
           name %in% c("vxcpoppct", "vxcgte65pct", "tdpm7", "tcpm7")
           ) %>%
    left_join(select(ctyPop, countyFIPS, useName), by="countyFIPS") %>%
    ggplot(aes(x=date, y=value)) + 
    geom_line(aes(color=useName, group=useName)) + 
    facet_wrap(~tmpMapper[name], scales="free_y") + 
    labs(x=NULL, y="Metric (% vaccinated or burden per million", title="Select counties around 250k population")

```

Data can also be aggregated by population, excluding states that have significant discontinuities in vaccines or deaths or cases:  
```{r, fig.height=9, fig.width=9}

# States with problems
problemStates <- c("VA", "TX", "SD", "HI", "GA", "CO", "GA", "FL", "NE", "VA")

# Create aggregated lists of counties by vaccine status (not problem states, population 125k-250k)
tmpCountyList <- ctyPop %>%
    filter(pop >= 125000, pop <= 250000, !(state %in% problemStates)) %>%
    left_join(filter(vaxPivot, name=="vxcpoppct", date==max(date)), by="countyFIPS") %>%
...
tmpCountyList %>%
    bind_rows() %>%
    select(countyFIPS, vaxBucket, pop) %>%
    left_join(vaxBurden, by=c("countyFIPS")) %>%
    group_by(vaxBucket, date, name) %>%
    summarize(value=sum(value*pop)/sum(pop), pop=sum(pop), n=n(), .groups="drop") %>%
    filter(name %in% c("vxcpoppct", "vxcgte65pct", "tdpm7", "tcpm7")) %>%
    ggplot(aes(x=date, y=value)) + 
    geom_line(aes(color=vaxBucket, group=vaxBucket)) + 
    facet_wrap(~tmpMapper[name], scales="free_y") + 
    labs(x=NULL, 
         y="Metric (% vaccinated or burden per million)", 
         title="Counties 125k-250k population in states with continuous county data"
         )
...
createVaxBurdenData <- function(lstVax, 
                                lstBurden
                                ) {

    # FUNCTION ARGUMENTS:
    # lstVax: list containing vaccines and population data
    # lstBurden: list containing burden data
    ...
    # Integrated data
    dfVaxBurden <- bind_rows(dfVax, dfBurden)
    
    # Return data
    list(ctyPop=dfPop, dfVaxBurden=dfVaxBurden)
    
}


plotVaxBurdenData <- function(lst, 
                              ctyPlot, 
                              varMapper=c("tdpm7"="Total DPM (7-day avg)", 
                                          "tcpm7"="Total CPM (7-day avg)", 
                                          "vxcgte65pct"="% Vax (65+)", 
                                          "vxcpoppct"="% Vax (all)"
                                          ), 
                              plotVars=names(varMapper), 
                              plotTitle="Select counties", 
                              scaleToDate=NULL, 
                              scaleToVars=c("tdpm7", "tcpm7")
                              ) {

    # FUNCTION ARGUMENTS:
    # lst: list of processed population and vaccines/burden data
    # ctyPlot: OPTION 1: character vector of countyFIPS to plot (each plotted individually)
    #          OPTION 2: list of form list("name1"=df1, "name2"=df2, ...) - each aggregate plotted
    # varMapper: named vector to map variables to descriptive labels
    # plotVars: variables to be plotted
    # plotTitle: title for the plot
    # scaleToDate: date to be used as baseline (level=100) for burden - NULL means keep as-is
    # scaleToVars: variables to have scaling to 100 applied

    # Create the dataset
    if("list" %in% class(ctyPlot)) {
        dfPlot <- ctyPlot %>%
            bind_rows() %>%
            select(countyFIPS, bucket, pop) %>%
            left_join(lst[["dfVaxBurden"]], by="countyFIPS") %>%
            group_by(useName=bucket, date, name) %>%
            summarize(value=sum(value*pop)/sum(pop), pop=sum(pop), n=n(), .groups="drop") %>%
            filter(name %in% all_of(plotVars))
    } else {
        dfPlot <- lst[["dfVaxBurden"]] %>%
            filter(countyFIPS %in% all_of(ctyPlot), 
                   name %in% all_of(plotVars)
                   ) %>%
            left_join(select(lst[["ctyPop"]], countyFIPS, useName), by="countyFIPS")
    }
    
    # Scale if appropriate
    if(!is.null(scaleToDate)) {
        dfPlot <- dfPlot %>%
            group_by(useName, name) %>%
            mutate(value=ifelse(name %in% all_of(scaleToVars), 
                                100*value/sum(value*ifelse(date==scaleToDate, 1, 0)), 
                                value
                                )
                   ) %>%
            ungroup()
    ...
    # Create y-axis label
    if(is.null(scaleToDate)) yVar <- "Metric (% vaccinated or burden per million)"
    else yVar <- paste0("Metric (% vaccinated or burden relative to date ", scaleToDate, " (index=100))")
    p1 <- dfPlot %>%
        ggplot(aes(x=date, y=value)) + 
        geom_line(aes(color=useName, group=useName)) + 
    ...
    if(!is.null(scaleToDate)) p1 <- p1 + geom_vline(xintercept=as.Date(scaleToDate), lty=2)
    print(p1)

}

# Example code
tmpVaxBurden <- createVaxBurdenData(lstVax=cty_vaxdata_20220607, lstBurden=cty_newdata_20220606)
identical(tmpVaxBurden$ctyPop, ctyPop)
identical(tmpVaxBurden$dfVaxBurden, vaxBurden)

# Example plot - individual counties
plotVaxBurdenData(tmpVaxBurden, 
                  ctyPlot=c("29047", "53077", "48245", "42041"), 
                  plotTitle="Select counties ~250k population"
                  )
# Example plot - aggregated counties
plotVaxBurdenData(tmpVaxBurden, 
                  ctyPlot=lapply(tmpCountyList, FUN=function(x) x %>% rename(bucket=vaxBucket)), 
                  plotTitle="Counties 125k-250k population in states with continuous county data"
                  )

# Example of other aggregations
problemStates <- c("VA", "TX", "SD", "HI", "GA", "CO", "GA", "FL", "NE", "VA")
southStates <- state.abb[state.region=="South"]
tmpCountyList <- tmpVaxBurden$ctyPop %>%
    filter(state %in% southStates, !(state %in% problemStates)) %>%
...
plotVaxBurdenData(tmpVaxBurden, 
                  ctyPlot=lapply(tmpCountyList, FUN=function(x) x %>% rename(bucket=vaxBucket)), 
                  plotTitle="Counties in South states with continuous county data"
                  )
plotVaxBurdenData(tmpVaxBurden, 
                  ctyPlot=lapply(tmpCountyList, FUN=function(x) x %>% rename(bucket=vaxBucket)), 
                  plotTitle="Counties in South states with continuous county data", 
...
readList <- list("usafCase"="./RInputFiles/Coronavirus/covid_confirmed_usafacts_downloaded_20220708.csv", 
                 "usafDeath"="./RInputFiles/Coronavirus/covid_deaths_usafacts_downloaded_20220708.csv"
                 )
compareList <- list("usafCase"=readFromRDS("cty_newdata_20220606")$dfRaw$usafCase, 
                    "usafDeath"=readFromRDS("cty_newdata_20220606")$dfRaw$usafDeath
                    )

# Use existing clusters
cty_newdata_20220708 <- readRunUSAFacts(maxDate="2022-07-06", 
                                        downloadTo=lapply(readList, 
                                                          FUN=function(x) if(file.exists(x)) NA else x
                                                          ),
                                        readFrom=readList, 
                                        compareFile=compareList, 
                                        writeLog="./RInputFiles/Coronavirus/USAF_NewData_20220708_chk_v005.log", 
                                        ovrwriteLog=TRUE,
                                        useClusters=readFromRDS("cty_newdata_20210813")$useClusters,
                                        skipAssessmentPlots=FALSE,
                                        brewPalette="Paired"
...
# Plot all counties based on closest cluster
sparseCountyClusterMap(cty_newdata_20220708$useClusters, 
                       caption="Includes only counties with 25k+ population",
                       brewPalette="viridis"
                       )

# Save the refreshed file
saveToRDS(cty_newdata_20220708, ovrWriteError=FALSE)
...
cty_vaxdata_20220709 <- processCountyVaccines(loc="./RInputFiles/Coronavirus/county_vaccine_20220709.csv", 
                                              ctyList=cty_newdata_20220708, 
                                              minDateCD=c("2022-02-01", "2022-02-01"),
                                              maxDateCD="2022-06-30"
                                              )

# Save the refreshed file
saveToRDS(cty_vaxdata_20220709, ovrWriteError=FALSE)
...
cty_postdata_20220708 <- postProcessCountyData(lstCtyBurden=cty_newdata_20220708$dfPerCapita, 
                                               lstCtyVax=cty_vaxdata_20220709$vaxFix, 
                                               lstState=readFromRDS("cdc_daily_220704")$dfPerCapita
                                               )

# Save the refreshed file
saveToRDS(cty_postdata_20220708, ovrWriteError=FALSE)
...
# Step 1a: Burden comparisons for aggregated states
additionalCountyPostProcess(cty_postdata_20220708, p1CompareStates=c(state.abb, "DC"), p1AggData=TRUE)

# Step 1: Burden aggregation for key states
# Step 2: vaccine comparisons
# Step 3: Scoring updates (and errors)
# Step 4: New rolling data (28-day default with ceilings 50000 CPM, 500 DPM)
additionalCountyPostProcess(cty_postdata_20220708, 
                            p1CompareStates=c("GA", "FL", "NE"), 
                            p2VaxStates=c("MA", "HI", "TX", "VA", "VT", "GA", "CO", "SD"), 
                            p3VaxTimes=sort(c("2021-07-01", "2022-06-30")),
                            p4DF=cty_newdata_20220708$dfPerCapita
                            )

```
  
Additional plots are updated:  
```{r, fig.height=9, fig.width=9}

# Creating the vaccine and burden data
tmpVaxBurden <- createVaxBurdenData(lstVax=cty_vaxdata_20220709, lstBurden=cty_newdata_20220708)
