...
compareBurdenVaxCounty <- function(lstBurden, 
                                   lstVax, 
                                   ctyIDs, 
                                   burdenVars=c("cpm7", "dpm7", "tcpm7", "tdpm7"), 
    ...
    # ctyIDs: vector of county-FIPS to process
    ...
    # Convert ctyIDs to 5-string characters (zero-padded as needed)
    ctyIDs <- zeroPad5(ctyIDs)
    
    # Extract name and population data
    dfNamePop <- lstBurden[["countyData"]] %>%
        filter(countyFIPS %in% ctyIDs) %>%
        mutate(fullName=paste0(countyName, ", ", state, " (pop: ", round(pop/1000), "k)"))
    ...
    if(isTRUE(printPlot)) {
        p1 <- dfCombine %>%
            ...
            ggplot(aes(x=date, y=value)) + 
            geom_line(aes(color=namePopVec[countyFIPS], group=countyFIPS)) + 
