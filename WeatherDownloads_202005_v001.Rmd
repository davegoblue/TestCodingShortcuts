...
validateClouds <- function(tbl, 
    ...
    htChecker <- function(y) {
        if (all(is.na(y))) {
            # This is OK, if there are no clouds then they are all NA
        } else if (all(is.na(y[2:length(y)]))) {
            # This is OK, single cloud value is by definition ascending
        } else {
            ...
            if (any(is.na(y) != cummax(is.na(y)))) {
                ...
                nonNA <- y[!is.na(y)]
                if (min(diff(nonNA)) <= 0) {
                    cat("\nIssue with clouds at same or lower height:", y, "\n")
                }
            }
        }
    }
    
    apply(cHeights, 1, FUN=htChecker)
    ...
    ifelse(obscureTypes==0, "", cloudStates[obscureTypes])
...
extractAndWrangleClouds <- function(met, 
    ...
    # Allow for up to 6 cloud types (hard coded)
    if (dim(numRecords)[2] > 6) {
        cat("Too many cloud records relative to hard-coded maximum of 6: Dimensions -", dim(numRecords))
        stop("Investigate and fix")
    }
    
    # Create columns for cType1-cType6 and cLevel1-cLevel6
    for (intCtr in 1:6) {
    ...
    # Validate clouds are sensible
    met$wType <- validateClouds(met, cloudStates=numStates)
...
# Variables in both fullGRB2 but not in fullGRB
...
for (colName in namesBoth) {
    cat("100% Match for Variable:", colName, all.equal(fullGRB[, colName], fullGRB2[, colName]), "\n")
}
...
There is a need to update the wType variable to be a factor and to reflect that SKC/CLR are an acceptable state.  Further checks should be run to ensure that there is exactly 1 of SKC, CLR, or wType!="".

Month is also a valuable variable for analysis, though it is contained in 'dtime' already.

Further checks should be run to confirm that 'dtime' is aligned to origMETAR.