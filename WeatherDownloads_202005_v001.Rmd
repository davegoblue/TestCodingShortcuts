...
#### _Example #6: Integrate Processing Steps_  
The processing steps can be integrated, and the results can be saved as a file for use later.
...
* checkTimePropoerties() - confirm that times are aligned between 'dtime' and 'origMETAR' and add year, month (as integer and as factored mmm abbreviation), and day  
...
integrateProcessingMETAR <- function(locRawMETAR, 
    ...
    keyZulu <- findHourlyReportingTimes(metFile=locRawMETAR, threshError=threshError)
    ...
    initFile <- initialParseMETAR(readFile, val=valMet, labs=labsMET, glimpseFinal=TRUE)
    ...
    # This process is quite hard-coded to a specific format (which is fine for now)
    fullFile <- extractAndWrangleClouds(fixFile, metVar="origMETAR")
...
The process can then be run on the Green Bay data to verify consistency of results:  
```{r cache=TRUE}

# The generally expected METAR parsing string
genMET <- ".*?(VRB|\\d{3})(\\d{2})(G\\d{2})?KT(.*?)(\\d{1,2}SM).*?\\s(M?\\d{2})/(M?\\d{2}).*?(A\\d{4}).*?RMK.*?(SLP\\d{3}).*?(T\\d{8})"

# The generally expected METAR parsing column labels
labsMet <- c("METAR", "WindDir", "WindSpeed", "WindGust", "Dummy", "Visibility", 
             "TempC", "DewC", "Altimeter", "SLP", "FahrC"
             )

# Test the process on the Green Bay data
testGRB <- integrateProcessingMETAR("./RInputFiles/metar_kgrb_2016.txt", 
                         startDay="2015-12-31", 
                         endDay="2017-01-01", 
                         colTypes=metType, 
                         genMET=genMET, 
...
# Test for equality across the files
identical(testGRB, fullGRB3)

# Test the process on the Green Bay data, including saving as RDS (allow for over-writing)
integrateProcessingMETAR("./RInputFiles/metar_kgrb_2016.txt", 
                         startDay="2015-12-31", 
                         endDay="2017-01-01", 
                         colTypes=metType, 
                         genMET=genMET, 
                         labsMET=labsMet, 
                         saveLoc="./RInputFiles/ProcessedMETAR/_TESTONLY_metar_kgrb_2016,rds", 
                         ovrWrite=TRUE
                         )

# Test for equality across the files when read in
identical(readRDS("./RInputFiles/ProcessedMETAR/_TESTONLY_metar_kgrb_2016,rds"), fullGRB3)

```
  
The processed Green Bay data matches what was produced by running all functions sequentially.  
  
The process can then be run for all of the processed METAR files:  
```{r cache=TRUE}

# Run for each of the 6 airports and save the processed file
for (airport in c("kmke", "kmsn", "kgrb", "ktvc", "kgrr", "kind")) {
    ...
    cat("\nFinished Processing Airport:", airport, "\n")

}

```
  
An issue was identified where sometimes clouds are described in the remarks.  The METAR string has been modified to only pick up clouds that come prior to RMK.
  
There is a single issue with non-ascending cloud heights in Madison, WI.  Perhaps this can be managed as a one-off or perhaps there is a need to update htChecker().  
  
There are 100+ mis-formatted METAR for several of the cities.  These appear to be data capture errors, and it is probably OK to exlude these from further analysis.  The output in these situations is perhaps overly verbose (printing all is more sensible for ~10 errors rather than ~100 errors).
  
#### _Example #7: Updated Core Functions and Integration_  
Several updates can help with the core functions and the data integration process:  
  
* Allow for output to be cat(..., file=) to minimize the size of the .html file while maintaining the ability to see diagnostic data  
* Allow for the SLP graph to be suppressed  
* The core functions should return NA for anything that is an incomplete METAR (e.g., do not pull out the visibility or the wind gust if there is a fragment for that in an otherwise incomplete METAR)  

  
The updated integration function is as follows:  
```{r}
...
The base process is run for Green Bay to ensure that data outputs remain unchanges:  
```{r}

# Run for Green Bay only yo confirm results are as expected
for (airport in c("kgrb")) {
    
    year <- 2016
    coreString <- paste0("metar_", airport, "_", as.character(year))
    
    cat("\nProcessing Airport:", airport)
    rerunGRB <- integrateProcessingMETAR(paste0("./RInputFiles/", coreString, ".txt"), 
                                         startDay=paste0(as.character(year-1), "-12-31"), 
                                         endDay=paste0(as.character(year+1), "-01-01"), 
                                         colTypes=metType, 
                                         genMET=genMET, 
...
# Test for equality across the files
identical(rerunGRB, fullGRB3)

```
  
The process is then run for Green Bay only using the updated parameters (cat to a log file, do not show SLP graph, use NA for all values associated to a mal-formatted METAR):  
```{r}
...
for (airport in c("kgrb")) {
    ...
    cat("\nProcessing Airport:", airport)
    newerGRB <- integrateProcessingMETAR(paste0("./RInputFiles/", coreString, ".txt"), 
                                         startDay=paste0(as.character(year-1), "-12-31"), 
                                         endDay=paste0(as.character(year+1), "-01-01"), 
                                         colTypes=metType, 
                                         genMET=genMET, 
...
# Check that file dimensions and variable names are the same
print(all.equal(dim(newerGRB), dim(rerunGRB)))
print(all.equal(names(newerGRB), names(rerunGRB)))

# Check that the only differences for NA are in clouds, Wind Gust, and Visibility
print(colSums(is.na(newerGRB) != is.na(rerunGRB)) %>% sort(decreasing=TRUE))

# Check that the only differences for NA are in wType and clouds
print(colSums(!is.na(newerGRB) & !is.na(rerunGRB) & newerGRB != rerunGRB) %>% sort(decreasing=TRUE))

```
  
And the process can then be run for the Chicago 2015-2016-2017 data:  
```{r cache=TRUE}
...
for (year in 2015:2017) {
    ...
    cat("\nProcessing Airport:", airport, "Year:", year)
    integrateProcessingMETAR(paste0("./RInputFiles/", coreString, ".txt"), 
                             startDay=paste0(as.character(year-1), "-12-31"), 
                             endDay=paste0(as.character(year+1), "-01-01"), 
                             colTypes=metType, 
    ...
    cat("\nFinished Processing Airport:", airport, "Year:", year, "\n")
