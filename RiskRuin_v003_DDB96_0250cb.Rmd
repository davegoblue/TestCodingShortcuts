...
* Cash back component of 2.5% (in other words, if you assume a permanent 2.5% loss, how much WORSE than that are you likely to be at your very lowest point)  
...
The start time is calculated to understand overall performance.  The 9/6 Double Double Bonus file is read in, using raw data from <http://wizardofodds.com/games/video-poker/tables/double-double-bonus/>.  Further, the cash back component is set to be 2.5%:  
```{r}
startTime <- proc.time()
...
nAddOnePer <- 40 ## Cash back is 1/nAddOnePer
...
####_Conclusions_  
The results suggest that assuming a 2.5% loss rate in a 9/6 DDB game, it is unlikely that actual results will ever fall more than 6k-10k units behind that benchmark.  The simulations match up reasonably well with the theory, suggesting that the "fake" cash back as solved by the polynomial is a plausible representation of the real world.  
