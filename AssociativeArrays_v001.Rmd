...
oedTrim <- sapply(oedLines, FUN=trimws)
...
As a first pass, only the entries flagged as "n.", "adj.", "v.", and "adv." will be used.  These will be single words that are flagged as noun, adjective, verb, and adverb.  
```{r}
# fooThree <- function(x) { (strsplit(x, "\\s+")[[1]][2] %in% c("n.", "adj.", "v.", "adv.") ) }
fooThree <- function(x) { length(grepRaw("[vn][\\.]|ad[jv]\\.", strsplit(x, "\\s+")[[1]][2])) > 0 }
...
for (eachLine in oedSubset) {
    ...
    oedDef <- substr(eachLine, grepRaw("\\S*[vn][\\.]|\\S*ad[jv]\\.", eachLine), nchar(eachLine))
...
Next, the words in the sayer dictionary are assessed, to see how many of them have been included:  
```{r}
...
This initial pass matches ~40% of the words in Tom Sawyer, but misses many of the common words.  It appears types like "conj.", "prep.", "pron.", and the like may need to be added to the search.