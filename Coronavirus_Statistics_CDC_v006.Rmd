...
df_u45 <- calculateRestatementFromRaw("cdcList_arch_2022w17", 
    ...
    filter(lubridate::year(weekEnding)==2022, 
           age %in% c("Under 25 years", "25-44 years"), 
           !is.na(deaths_220425)
           )

# Get the list of states in every week
u45States <- df_u45 %>% count(state) %>% filter(n==max(n)) %>% pull(state)
u45States
...
There are significant differences in the amount of negative restatement by state and week
