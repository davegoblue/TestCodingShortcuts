...
df_u45 <- calculateRestatementFromRaw("cdcList_arch_2022w17", 
    ...
    filter(weekEnding >= as.Date("2021-12-01"),
           age %in% c("Under 25 years", "25-44 years"), 
           !is.na(deaths_220425)
           )
...
df_u45 %>%
    ...
    ggplot(aes(x=fct_reorder(state, chg), y=weekEnding)) + 
    ...
    labs(y="Week", 
         ...
         subtitle="States reporting all weeks for Under 25 and 25-44 years in each data set"
         ) + 
    facet_wrap(~age)

```
  
There are significant differences in the amount of negative restatement by state and week in 2022. Restatements are generally very modest to nonexistent in the reported 2021 data for the same files
