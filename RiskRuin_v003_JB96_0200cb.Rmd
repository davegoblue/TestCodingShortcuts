...
The start time is calculated to understand overall performance.  The 9/6 Jacks or Better file is read in, using raw data from <http://wizardofodds.com/games/video-poker/tables/jacks-or-better/>.  Further, the cash back component is set to be 2.0%:  
```{r}
startTime <- proc.time()

## Step 1: Read in the outcomes file
baseOutcomes <- getBaseOutcomes(myFileName="JB_96_Probs.csv", forceEQ=TRUE)
...
####_Conclusions_  
The results suggest that assuming a 2.0% loss rate in a 9/6 JB game, it is unlikely that actual results will ever fall more than 1.5k-2.5k units behind that benchmark.  The simulations match up reasonably well with the theory, suggesting that the "fake" cash back as solved by the polynomial is a plausible representation of the real world.  
