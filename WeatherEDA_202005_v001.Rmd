...
The function can then be run for all the 2016 data:  
```{r}

# Extracted high-low temperatures
all_2016_hilo <- map_dfr(.x=fileNames, .f=getDailyHighLow, .id="source") %>%
...
The high-low temperatures appear to be calculated almost always at 6Z in 8 Central time zone locales; captured almost always at 5Z in 5 East time zone locales; and captured almost always at 8Z in 2 West time zone locales.

Precipitation data can also be extracted by function:  
```{r}
...
all_2016_pin <- map_dfr(.x=fileNames, .f=getPrecipAmount, .id="source") %>%
    mutate(source=fileNames[as.integer(source)], locale=cityNameMapper[source]) %>%
    select(source, locale, everything())
all_2016_pin
...
for (plotVar in c("p1Inches", "p36Inches", "p24Inches")) {
    ...
    p1 <- all_2016_pin %>%
        ...
        ggplot(aes(x=hour, fill=notNA)) + 
        geom_bar(position="stack") + 
        scale_fill_discrete(paste0(mapChartVar[plotVar], " Data")) + 
        labs(y="Number of Days with Observations at Hour", x="METAR Hour", 
...
There are some minor differences when summing the precipitation by different units.  The biggest outlier is that Houston 1-hour total being ~10 inches greater than the Houston 6-hour totals or 24-hour totals:  
```{r}
...
kiah_2016 %>%
    filter(lubridate::date(dtime) %in% as.Date(c("2016-04-18", "2016-04-19"))) %>%
    pull(origMETAR)

```

Houston experienced near-record flooding on April 18, 2016 due to ~10 inches of rain.  The 1-hour totals appear to capture this, while the 24-hour total is absent on April 18 and 7//// (data bad) on April 19.

The precipitation function is updated to allow for sending its detailed results to a log file, creating only a summary of the results, and plots if requested.

To begin, the base functions are updated to allow for sending data to a log file.  The actual approach will use capture.output(), so these are back-compatible and available in case direct printing is ever needed:  
```{r}

# Function to extract precipitation state and range data from METAR
fnPrecip1 <- function(df, 
                      pType, 
                      showRegex=TRUE, 
                      logFile="", 
                      append=TRUE) {
...
# Function to extract issues - continuity, distance from METAR
fnPrecip3 <- function(df, logFile="", append=TRUE) {
    ...
    # Flag that there are continuity or consistency errors
    cat("\nContinuity or consistency error - record(s) will be deleted\n", file=logFile, append=append)
    
    # Write the errors to a logFile if provided, otherwiset to stdout
    if (logFile != "") { sink(logFile, append=append) }
    issueCheck %>%
        filter(issueCons) %>%
        print()
    if (logFile != "") { sink() }
...
# Function to find wrong sequence of times
fnPrecip4 <- function(dfTimes, dfOrig, logFile="", append=TRUE) {
...
# Function to work through each bad date-time and suggest new ranges
fnPrecip5 <- function(df, issueTimes, lagCurFrame, logFile="", append=TRUE) {
    ...
    # Print to logFile if provided, stdout otherwise
    if (logFile != "") { sink(logFile, append=append) }
    dfIssues %>%
        select(-issueCons, -issue) %>%
        print()
...
fnPrecip6 <- function(lst, df, logFile="", append=TRUE) {
    ...
    if (length(mism) == 0) {
        ...
        if (logFile != "") {
            # This message should be duplicated to stdout if sent to a log
            cat("\nFull matches between METAR observations and intervals\n")
        }
    } else {
        for (x in mism) {
            ...
            if (logFile != "") { 
                # Send a message to stdout that there were mismatches
                cat("\nMismatches detected - see log file", logFile, "\n")
...
The overall process can then be consolidated in to a big function:  
```{r}
...
# Function to run EDA, send output to logs if requested, and return list
...
The functions can then be run for rain on a subset of the 2016 data:  
```{r}

subFileNames <- ls(pattern="^km[a-z]{2}_2016$")
cat("\nProcesses will be run for files:\n\n", paste0(subFileNames, collapse="\n"), "\n", sep="")

# Run as normal for the subset of files
subRainList01 <- wrapPrecipTimes(subFileNames, 
                                 pType="(?<!FZ)RA", 
                                 pExt="_RA", 
                                 pTypeName="rain"
                                 )

# Confirm that data are the same as previous
all.equal(rainLength %>% filter(str_detect(source, pattern="km.*2016")), subRainList01$precipLength)
for (listNames in names(subRainList01$precipList)) {
    print(identical(subRainList01$precipList[listNames], rainList[listNames]))
}


# Show the path where files will be written
cat("\nFiles will be written to:", filePath, "\n")

# Redirect to log files for the same subset
subRainList02 <- wrapPrecipTimes(subFileNames, 
                                 pType="(?<!FZ)RA", 
                                 pExt="_RA", 
                                 pTypeName="rain", 
                                 writeLogFile="rainIntervalTimes.log",
                                 writeLogPDF="rainIntervalTimes.pdf",
                                 writeLogPath=filePath,
                                 appendWriteFile=FALSE
                                 )

# Confirm that data are the same as previous
all.equal(rainLength %>% filter(str_detect(source, pattern="km.*2016")), subRainList02$precipLength)
for (listNames in names(subRainList02$precipList)) {
    print(identical(subRainList02$precipList[listNames], rainList[listNames]))
}

```
  
Logs and graphs are available in stand-alone files for deep-dives, leaving the main log file less cluttered.

Next steps are to consolidate/save all final functions in to a single script, run on all locales, and save processed EDA for later use (see _v002).