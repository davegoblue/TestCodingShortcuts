...
combinedEDA <- function(filename, 
    ...
    if (is.null(desc)) {
        for (potMatch in names(cityNameMapper)) {
            if (str_detect(string=filename, pattern=potMatch)) {
                desc <- cityNameMapper[potMatch]
                break
            }
        }
        if (is.null(desc)) {
            desc <- paste0("UNMAPPED_", filename)
            cat("\nUnable to find a description, will use ", desc, "\n", sep="")
        } else {
            cat("\nWill use ", desc, " as the description for ", filename, "\n", sep="")
        }
    }
    ...
    for (ys in lmVars) {
        lmMETAR(tbl, y=ys[1], x=ys[2], yName=mapper[ys[1]], subT=desc)
    }
