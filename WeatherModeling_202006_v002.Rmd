...
Each sub-section is written as its own function, particularly to allow #4 to be run as a stand-alone.  A wrapper function calls then all together.  Example code includes:  
```{r}

# Function #1 Variable Importance (this will replace the existing helperPlotVarImp)
...
# Function #2: Plot R-squared and MSE by number of trees
...
# Run the new function #2
plotMSERSQ(rfRegrData, returnData=TRUE)


# Function #3: RMSE and R-squared by key variable
...
# Function #4: Actual vs. predicted
