...
exploreReportedChange <- function(dfNew, 
                                  ...
                                  metricMapper=hhsMapper,
                                  naMetric=-999999,
                                  checkTimes=c("2021-Q2", "2022-Q2"),
                                  useLabels=c("1. Older", "2. Newer"),
                                  lstFilter=list(), 
                                  lstExclude=list()
                                  ) {
    ...
    # metricMapper: mapping file for 'use_metric' to descriptive name
    # naMetric: vector of values to treat as NA
    # checkTimes: vector of lenght two with time periods of interest for measuring change in 'use_metric'
    # useLabels: the labels for the data sources in the plot, as length-two character vector
    ...
    # Create summary
    testTable <- helperGroupMetrics(testNew) %>%
    ...
    # Plot summary of testTable
    p1 <- testTable %>%
        pivot_longer(-c(customTime)) %>%
        mutate(metric=stringr::str_extract(name, pattern="^[a-zA-Z]+"), 
               timePeriod=stringr::str_extract(name, pattern="[a-zA-Z]+$"), 
               useValue=ifelse(is.na(value), 0, value), 
               keyLabel=useLabels[ifelse(timePeriod=="older", 1, 2)]
               ) %>%
        ggplot(aes(x=fct_rev(customTime), y=useValue)) + 
        geom_point(aes(color=keyLabel)) + 
        geom_text(data=~filter(., timePeriod=="newer"), 
                  aes(label=round(useValue, ifelse(useValue > 1, 0, 3))), 
                  color=ifelse(useLabels[1] < useLabels[2], "darkgreen", "red"), 
                  nudge_x=0.2
                  ) +
        geom_text(data=~filter(., timePeriod=="older"), 
                  aes(label=round(useValue, ifelse(useValue > 1, 0, 3))), 
                  color=ifelse(useLabels[1] < useLabels[2], "red", "darkgreen"), 
                  nudge_x=-0.2
                  ) +
        ...
        labs(x=NULL, 
             y=NULL, 
             title=paste0("Summaries by data source for metric: ", metricMapper[useMetric]), 
             subtitle="Sum and percent of hospital-week records NA"
             ) +
        scale_color_discrete("Source:")
    print(p1)

    # Create the summary data frame of declines by hospital
    testFull <- testOld %>%
        group_by(hospital_pk, customTime) %>%
        summarize(nNA=mean(is.na(keyMetric)), 
                  keyMetric=mean(ifelse(is.na(keyMetric), 0, keyMetric)), 
        ...
        filter(customTime %in% all_of(checkTimes)) %>%
        mutate(name=ifelse(customTime==min(customTime), "older", "newer")) %>%
        pivot_wider(hospital_pk, names_from="name", values_from="keyMetric", values_fill=0) %>%
        mutate(delta=newer-older, pctDelta=delta/older) %>%
        arrange(delta) %>%
        left_join(unique(select(testOld, hospital_pk, hospital_name, city, hospital_subtype)), by="hospital_pk")
...
exploreReportedChange(dfNew=indivHosp_20220805, 
                      ...
                      useLabels=c("2022-07-04 data", "2022-08-05 data")
                      )

```
  
An example hospital is studied:  
```{r, fig.height=9, fig.width=9}

readFromRDS("indivHosp_20220704") %>%
    filter(hospital_pk=="360180") %>%
    select(c("hospital_pk", "collection_week", "hospital_name", "city", names(hhsMapper))) %>%
    pivot_longer(-c(hospital_pk, collection_week, hospital_name, city)) %>%
    filter(!is.na(value), value != -999999) %>%
    ggplot(aes(x=collection_week, y=value)) + 
    geom_line() + 
    facet_wrap(~name, scales="free_y") + 
    lims(y=c(0, NA)) + 
    labs(title="Select metrics by week for Cleveland Clinic", x=NULL, y=NULL)

```
  
It appears that Cleveland Clinic (and many other entities) still report some, but not all, capacity metrics. Further review of the latest dataset documentation may be needed. Additional metrics are explored:  
```{r, fig.height=9, fig.width=9}

readFromRDS("indivHosp_20220704") %>%
    filter(hospital_pk=="360180") %>%
    select(c("hospital_pk", "collection_week", "hospital_name", "city", 
             contains("beds") & contains("avg") &!contains("used"))
           ) %>%
    pivot_longer(-c(hospital_pk, collection_week, hospital_name, city)) %>%
    filter(!is.na(value), value != -999999) %>%
    ggplot(aes(x=collection_week, y=value)) + 
...
Reporting appears to still be consistent for 'inpatient_beds', even as it fell off for 'all_beds'. Additional metrics are explored for values vs. one year ago:  
```{r, fig.height=9, fig.width=9}

readFromRDS("indivHosp_20220704") %>%
    filter(hospital_pk=="360180", collection_week %in% (max(collection_week)-c(0, 7*52))) %>%
    select(hospital_pk, collection_week, hospital_name, city, where(is.numeric)) %>%
    pivot_longer(-c(hospital_pk, collection_week, hospital_name, city)) %>%
    mutate(valueNA=ifelse(is.na(value), 1, ifelse(value==-999999, 1, 0)), 
           metType=stringr::str_sub(name, start=-3L, end=-1L)
           ) %>%
    filter(metType == "age") %>%
    ggplot(aes(x=name, 
               y=ifelse(valueNA==1, 0, value), 
               color=ifelse((collection_week==max(collection_week)), "2022-06", "2021-06")
               )
           ) + 
    geom_point() + 
    geom_text(aes(label=ifelse(valueNA==1, 0, value), 
                  hjust=ifelse(collection_week==max(collection_week), -1, 1)
                  )
              ) +
    coord_flip() + 
    labs(x=NULL, y=NULL, title="Coverage metrics for 2022-06 vs 2021-06") +
...
readFromRDS("indivHosp_20220704") %>%
    ...
    coord_flip() + 
    labs(x=NULL, y=NULL, title="Average metrics for 2022-06 vs 2021-06") +
    scale_color_discrete("Month")
    
```
  
Reported coverage has declined for select metrics. A new measure of hospital capacity should be created from the raw data. The process is converted to functional form:  
```{r, fig.height=9, fig.width=9}

tempPlotHospitalMetrics <- function(df, 
                                    keyHospitalPk, 
                                    weekOffset=c(0, 52),
                                    createPlotDF=TRUE,
                                    keyMetType="age", 
                                    keyMetMap=c("age"="Coverage", "avg"="Average", "sum"="Sum"), 
                                    returnData=isTRUE(createPlotDF)
                                    ) {

    # FUNCTION ARGUMENTS:
    # df: data frame for analysis
    # keyHospitalPk: hospital_pk value to be selected
    # weekOffset: weeks of the data to compare (default is most recent vs. one year prior)
    # createPlotDF: boolean, should plot data be created? (if FALSE, plotData assumed to be passed)
    # keyMetType: metric type to evaluate ("age", "avg", "sum")
    # returnData: boolean, should data be returned?
    
    # Create plot data if requested
    if(isTRUE(createPlotDF)) {
        df <- df %>%
            filter(hospital_pk==keyHospitalPk, 
                   collection_week %in% (max(collection_week)-7*weekOffset)
                   ) %>%
            select(hospital_pk, collection_week, hospital_name, city, where(is.numeric)) %>%
            pivot_longer(-c(hospital_pk, collection_week, hospital_name, city)) %>%
            mutate(valueNA=ifelse(is.na(value), 1, ifelse(value==-999999, 1, 0)), 
    ...
    # Get key dates (assumes weekOffset passed ascending)
    keyDates <- df %>% 
        count(collection_week) %>% 
        mutate(useWeek=paste0(lubridate::year(collection_week), "-", zeroPad2(lubridate::month(collection_week)))) %>%
        pull(useWeek) %>%
        sort(decreasing=TRUE)
    
    # Get key hospital name
    keyHospName <- df %>% 
        mutate(hospName=paste0(hospital_name, ", ", city, " (", hospital_pk, ")")) %>%
        slice(1) %>%
        pull(hospName)
    
    # Create plot (assumes only two options for weekOffset)
    p1 <- df %>%
        filter(metType %in% all_of(keyMetType)) %>%
        ggplot(aes(x=name, 
                   y=ifelse(valueNA==1, 0, value), 
                   color=ifelse((collection_week==max(collection_week)), keyDates[1], keyDates[2])
                   )
               ) + 
        geom_point() + 
        geom_text(aes(label=ifelse(valueNA==1, 0, value), 
        ...
        labs(x=NULL, 
             y=NULL, 
             title=paste0(keyMetMap[keyMetType], " metrics for ", paste0(keyDates, collapse=" and ")), 
             subtitle=keyHospName
             ) +
...
tmpCleveland <- tempPlotHospitalMetrics(readFromRDS("indivHosp_20220704"), 
                                        keyHospitalPk="360180", 
                                        weekOffset=c(0, 52),
                                        keyMetType="age"
                                        )
tempPlotHospitalMetrics(tmpCleveland, weekOffset=c(0, 52), keyMetType="avg", createPlotDF=FALSE)
tempPlotHospitalMetrics(tmpCleveland, weekOffset=c(0, 52), keyMetType="sum", createPlotDF=FALSE)

```
  
Capacitybased on 'inpatient_beds_7_day_avg', 'inpatient_beds_used_7_day_avg', and 'inpatient_beds_used_covid_7_day_avg' is calculated:  
```{r, fig.height=9, fig.width=9}

# Plot individual hospitals
readFromRDS("indivHosp_20220704") %>%
    filter(hospital_pk %in% c("360180", "100007", "390164")) %>%
    select(hospital_name, 
           collection_week, 
           beds=inpatient_beds_7_day_avg, 
           bedsUsed=inpatient_beds_used_7_day_avg, 
           bedsCovid=inpatient_beds_used_covid_7_day_avg
           ) %>%
    pivot_longer(-c(hospital_name, collection_week, beds)) %>%
    ggplot(aes(x=collection_week, y=value/beds)) + 
    geom_line(aes(group=name, color=name)) + 
    facet_wrap(~hospital_name) +
    labs(x=NULL, y="% Inpatient Beds Occupied", title="Select Hospitals - inpatient bed capacity") + 
    geom_hline(yintercept=1, lty=2) + 
    lims(y=c(0, NA)) + 
    scale_color_discrete("Metric")

# Plot aggregates
dfTemp <- readFromRDS("indivHosp_20220704") %>%
    select(hospital_name, 
           collection_week, 
           beds=inpatient_beds_7_day_avg, 
           bedsUsed=inpatient_beds_used_7_day_avg, 
    ...
    filter(!is.na(beds), !is.na(bedsUsed), !is.na(bedsCovid)) %>%
    filter(beds >= 0, bedsUsed >= 0, bedsCovid >= 0) %>%
    group_by(collection_week) %>%
    summarize(across(where(is.numeric), sum), .groups="drop") 
dfTemp %>%
    pivot_longer(-c(collection_week)) %>%
    ggplot(aes(x=collection_week, y=value)) + 
    geom_line(aes(group=name, color=name)) + 
    labs(x=NULL, y="% Inpatient Beds Occupied", title="Sum of Fully Reporting Hospitals - inpatient bed capacity") + 
    geom_hline(yintercept=1, lty=2) + 
    lims(y=c(0, NA)) + 
    scale_color_discrete("Metric")
dfTemp %>%
    pivot_longer(-c(collection_week, beds)) %>%
    ggplot(aes(x=collection_week, y=value/beds)) + 
    geom_line(aes(group=name, color=name)) + 
    labs(x=NULL, y="% Inpatient Beds Occupied", title="Sum of Fully Reporting Hospitals - inpatient bed capacity") + 
    geom_hline(yintercept=1, lty=2) + 
...
The metrics appears more stable, with coverage included throughout the dataset time period. There are still some surprising changes in beds reported over time, though ratios of utilization appear more stable

Function hospitalCapacityCDCDaily() is updated:  
```{r, fig.height=9, fig.width=9}

hhsAdd <- c("inpatient_beds_used_7_day_avg"="inpatient_beds_occupied", 
            "inpatient_beds_used_covid_7_day_avg"="inpatient_beds_covid"
            )
hhsMapperUpdated <- c(hhsMapper, hhsAdd)
...
hospitalCapacityCDCDaily <- function(df, 
    ...
    # Convert data if requested
    if(isTRUE(createData)) df <- skinnyHHS(df, varMapper=hhsMapperUpdated) %>% imputeNACapacity() %>% sumImputedHHS()
...
modStateHosp_20220805 <- hospitalCapacityCDCDaily(indivHosp_20220805, plotSub="August 2020 to July 2022")
modStateHosp_20220704 <- hospitalCapacityCDCDaily(readFromRDS("indivHosp_20220704"), plotSub="August 2020 to June 2022")

```

The updated process run on older data appears reasonable. The older data should be spiced in to the newer data, for completeness of 2020 through mid-2021

The latest data are downloaded and processed:  
```{r, fig.height=9, fig.width=9, cache=TRUE}

readList <- list("cdcDaily"="./RInputFiles/Coronavirus/CDC_dc_downloaded_220902.csv", 
                 "cdcHosp"="./RInputFiles/Coronavirus/CDC_h_downloaded_220902.csv", 
                 "vax"="./RInputFiles/Coronavirus/vaxData_downloaded_220902.csv"
                 )
...
cdc_daily_220902 <- readRunCDCDaily(thruLabel="Aug 31, 2022", 
                                    downloadTo=lapply(readList, FUN=function(x) if(file.exists(x)) NA else x), 
                                    readFrom=readList,
                                    compareFile=compareList, 
                                    writeLog=NULL, 
...
saveToRDS(cdc_daily_220902, ovrWriteError=FALSE)

# Run for latest data, save as RDS
indivHosp_20220902 <- downloadReadHospitalData(loc="./RInputFiles/Coronavirus/HHS_Hospital_20220902.csv")
saveToRDS(indivHosp_20220902, ovrWriteError=FALSE)
...
# Create pivoted burden data
burdenPivotList_220902 <- postProcessCDCDaily(cdc_daily_220902, 
...
# Create hospitalized per capita data
hospPerCap_220902 <- hospAgePerCapita(readFromRDS("dfStateAgeBucket2019"), 
                                      lst=burdenPivotList_220902, 
                                      popVar="pop2019", 
                                      excludeState=c(), 
                                      cumStartDate="2020-07-15"
                                      )

burdenPivotList_220902$hospAge %>%
    group_by(adultPed, confSusp, age, name) %>%
    summarize(value=sum(value, na.rm=TRUE), n=n(), .groups="drop")
...
peakValleyCDCDaily(cdc_daily_220902)
...
modStateHosp_20220902 <- hospitalCapacityCDCDaily(indivHosp_20220902, plotSub="August 2020 to August 2022")
...
identical(names(indivHosp_20220902), names(readFromRDS("indivHosp_20220704")))

modHospData <- bind_rows(filter(readFromRDS("indivHosp_20220704"), lubridate::year(collection_week)<2022), 
                         filter(indivHosp_20220902, lubridate::year(collection_week)>=2022), 
                         .id="src"
                         )
updated_modStateHosp_20220902 <- hospitalCapacityCDCDaily(modHospData, 
