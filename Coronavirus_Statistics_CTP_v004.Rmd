...
readCOViDbyState <- function(fileName, 
    ...
    helperSimilarity <- function(newData, refData, label, countOnly=FALSE) {
    ...
    if (is.null(checkFile)) {
        ...
        dateChangeList <- helperSimilarity(newData=df %>% pull(date) %>% unique() %>% format("%Y-%m-%d"), 
        ...
        if (dateMetricPrint) print(deltaDate)
        p1 <- dfByDate %>% 
...
# Reading the file with a comparison to a previous file
...
The added functionality allows for a cleaner log file.  Next steps are to redirect the detailed output to a PDF file if requested.
