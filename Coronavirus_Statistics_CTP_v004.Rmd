...
The functions for creating data alignments can be chained:  
```{r, fig.height=9, fig.width=9}

# Chained cases to hospitalized
caseHospPlots <- makeAlignment(dfAggList, algMetrics=c("cases", "hosp"), useIntercept=TRUE) %>%
    plotAlignment()
str(caseHospPlots, max.level=1)

```

Results are the same as obtained previously, as expected.

The function is then run in reverse, "predicting" cases from total hospitalized:  
```{r, fig.height=9, fig.width=9}

# Chained hospitalized to cases
hospCasePlots <- makeAlignment(dfAggList, 
                               vecLagLead=seq(-36, 36, by=2), 
                               algMetrics=c("hosp", "cases"), 
                               useIntercept=TRUE
                               ) %>%
    plotAlignment()
str(hospCasePlots, max.level=1)

```

Based on lag/lead, these use a slightly different time period.  Curves continue to be well aligned, with the negative lag/lead meaning that cases rise/fall earlier than hospitalizations.

The process is run on more similar time periods:  
```{r, fig.height=9, fig.width=9}

# Chained hospitalized to cases
hospCasePlots <- makeAlignment(dfAggList, 
                               vecLagLead=seq(-36, 10, by=2), 
                               algDates=c("2020-08-10", "2021-02-15"), 
                               algMetrics=c("hosp", "cases"), 
                               useIntercept=TRUE
                               ) %>%
    plotAlignment()
...
If y = ax + b then x = (y-b)/a = y/a - b/a.  So, if the models were perfectly reversed, we would expect:  
  
* New scalar is the inverse of original scalar  
* New intercept is the negative of the original intercept divided by the original scalar  
  
The degree of alignment is assessed:  
```{r}

origLagLead <- caseHospPlots$inputList$bestLagLead %>%
    mutate(expScalar = 1/scalar, expIntercept=-intercept/scalar) %>%
    rename(origLagLead=lagLead, origRSQ=rsq, origRMSE=rmse, origScalar=scalar, origIntercept=intercept)
revLagLead <- hospCasePlots$inputList$bestLagLead

origLagLead %>%
    inner_join(revLagLead, by="cluster") %>%
    select(cluster, origLagLead, lagLead, expScalar, scalar, expIntercept, intercept)

```

While not identical, the reverse model run on data of the same time period produces very similar results.  This is driven by the curves having pronounced peaks that tend to occur at an 8-10 day lag/lead.

The function is then run to predict deaths from hospitalizations:  
```{r, fig.height=9, fig.width=9}

# Chained hospitalized to deaths
hospDeadPlots <- makeAlignment(dfAggList, 
                               vecLagLead=seq(-36, 36, by=2), 
                               algMetrics=c("hosp", "deaths"), 
                               useIntercept=TRUE
                               ) %>%
    plotAlignment()
str(hospDeadPlots, max.level=1)

```

There is strong alignment between change in total hospitalized and change in average daily death rates.  There is roughly a 10-day lag with change in daily deaths being roughly 3% of change in total hospitalizations.

The function is then run in reverse, "predicting" cases from total hospitalized on similar time periods:  
```{r, fig.height=9, fig.width=9}

# Chained deaths to hospitalized
deadHospPlots <- makeAlignment(dfAggList, 
                               vecLagLead=seq(-36, 10, by=2), 
                               algDates=c("2020-08-10", "2021-02-15"), 
                               algMetrics=c("deaths", "hosp"), 
                               useIntercept=TRUE
                               ) %>%
    plotAlignment()
str(deadHospPlots, max.level=1)
...
origLagLead <- hospDeadPlots$inputList$bestLagLead %>%
    mutate(expScalar = 1/scalar, expIntercept=-intercept/scalar) %>%
    rename(origLagLead=lagLead, origRSQ=rsq, origRMSE=rmse, origScalar=scalar, origIntercept=intercept)
revLagLead <- deadHospPlots$inputList$bestLagLead
...
While not identical, the reverse model run on data of the same time period produces very similar results.  This is driven by the curves having pronounced peaks that tend to occur at an 1-2 week lag/lead.

The function for deaths ~ cases has been run previously and is now run in reverse, "predicting" deaths from cases on similar time periods:  
```{r, fig.height=9, fig.width=9}

# Chained deaths to cases
deadCasePlots <- makeAlignment(dfAggList, 
                               vecLagLead=seq(-36, 10, by=2), 
                               algDates=c("2020-08-10", "2021-02-15"), 
                               algMetrics=c("deaths", "cases"), 
                               useIntercept=FALSE
                               ) %>%
    plotAlignment()
str(deadCasePlots, max.level=1)
...
origLagLead <- caseDeathPlots$inputList$bestLagLead %>%
    mutate(expScalar = 1/scalar, expIntercept=-intercept/scalar) %>%
    rename(origLagLead=lagLead, origRSQ=rsq, origRMSE=rmse, origScalar=scalar, origIntercept=intercept)
revLagLead <- deadCasePlots$inputList$bestLagLead
...
While not identical, the reverse model run on data of the same time period produces very similar results.  This is driven by the curves having pronounced peaks that tend to occur at an 2-3 week lag/lead.

Projections can be made using the hospitalized ~ cases data and the deaths ~ hospitalized data.  A function is written to convert data by cluster to a projection using a best lag/lead parameters file:  
```{r}

projectLagLead <- function(df=select(bind_rows(dfAggList, .id="cluster"), cluster, date, metric, value), 
                           bestParams, 
                           keyMetric="cases", 
                           algDates=c("2020-07-15", "2021-01-20")
                           ) {
    ...
    # FUNCTION ARGUMNETS
    # bestParams: a file including cluster-lagLead-scalar-intercept
    # df: a file containing cluster-date-metric-value
    # keyMetric: filter df to only contain records where value == keyMetric (NULL means no filtering)
    # algDates: the date range to include from df (NA means use data as-is)
    
    # Filter df to the relevant metric (only if keyMetric is not NULL)
    if (!is.null(keyMetric)) {
        df <- df %>%
            filter(metric %in% all_of(keyMetric))
    }
    
    # Filter for dates in df, excluding NULL
    if (!is.na(algDates[1])) {
        df <- df %>%
            filter(date >= as.Date(algDates[1]))
    }
    if (!is.na(algDates[2])) {
        df <- df %>%
            filter(date <= as.Date(algDates[2]))
    }
    
    # Get the list of variables to keep
    keepVar <- names(df)
    
    # Merge in the lagLead file (for processing only)
    df <- df %>%
        inner_join(bestParams, by=c("cluster"))
    
    # Create the new date and new value
    df <- df %>%
        mutate(newDate=date + lagLead, newValue = value * scalar + intercept)
    
    # Return the data frame
    df %>%
        select_at(vars(all_of(c(keepVar, "newDate", "newValue"))))
    
}

```

The process is then run on the hospital ~ cases data:  
```{r}

tempData <- projectLagLead(bestParams=caseHospPlots$inputList$bestLagLead, algDates=c("2020-07-15", NA)) %>%
    rename(caseDate=date, caseValue=value, date=newDate, value=newValue) %>%
    projectLagLead(bestParams=hospDeadPlots$inputList$bestLagLead, algDates=c("2020-07-15", NA)) %>%
    rename(hospDate=date, hospValue=value, deadDate=newDate, deadValue=newValue)
tempData

# Projected hospitalizations by cluster
tempData %>%
    ggplot(aes(x=hospDate, y=hospValue, group=cluster, color=cluster)) + 
    geom_line() + 
    labs(x="", 
         y="Per million, 7-day average (projected)", 
         title="Projection for total hospitalized by cluster"
         ) + 
    facet_wrap(~cluster)

# Projected deaths by cluster
tempData %>%
    ggplot(aes(x=deadDate, y=deadValue, group=cluster, color=cluster)) + 
    geom_line() + 
    labs(x="", 
         y="Per million, 7-day average (projected)", 
         title="Projection for total deaths by cluster"
         ) + 
    facet_wrap(~cluster)

```

The projection function appears to produce outputs as desired.

The data can be pivoted longer for easier manipulation:  
```{r}

# Full database of projected and actual
projData <- bind_rows(select(tempData, cluster, date=caseDate, value=caseValue), 
                      select(tempData, cluster, date=hospDate, value=hospValue), 
                      select(tempData, cluster, date=deadDate, value=deadValue), 
                      .id="metric"
                      ) %>%
    mutate(metric=c("1"="cases", "2"="hosp", "3"="deaths")[metric], 
           type="projected"
           ) %>%
    bind_rows(mutate(rename(bind_rows(dfAggList), cluster=state), type="actual"))

# Plot hospitalized and deaths
projData %>%
    filter(metric=="hosp") %>%
    ggplot(aes(x=date, y=value)) + 
    ...
    facet_wrap(~cluster) + 
    labs(x="", 
         y="Total hospitalized per million (7-day average)", 
         title="Projected vs. actual total hospitalized by cluster"
         )
...
projData %>%
    ...
    facet_wrap(~cluster) + 
    labs(x="", 
         y="Deaths per million (7-day average)", 
         title="Projected vs. actual deaths by cluster"
         )

```

The projected and actual data are well aligned, as expected given that most of the actual data were fitted in the projection modeling.

CDC case and death data by state and date are available for download on the [CDC website](https://data.cdc.gov/api/views/9mfq-cb36/rows.csv?accessType=DOWNLOAD).  The data are downloaded and the process cached to avoid repeated hits against the CDC website:  
```{r cache=TRUE}

downloadCOVIDbyState("./RInputFiles/Coronavirus/CDC_dc_downloaded_210410.csv",
...
Control totals are explored by state:  
```{r}

# Read and glimpse
cdcRaw <- readr::read_csv("./RInputFiles/Coronavirus/CDC_dc_downloaded_210410.csv")
glimpse(cdcRaw)
cdcRaw %>% 
    count(consent_cases, consent_deaths)

# Create date field and convert 'consent' to boolean
...
As expected from the documentation, not all jurisdictions split their cases and/or deaths by probable and confirmed.  

Next steps are to explore the mismatches between the sum of 'new' and 'total':
```{r}
...
The reported total field appears to always be greater than or equal to the cumulative sum of the new field.  Discrepancies occur in large chunks at discrete points in time, likely due to reclassifications and other corrections to the data.  It is likely that the 'new' fields are more appropriate for assessing shape of the curve while the 'total' fields are more accurate for assesing total disease impact per capita.  Both will be kept as columns for further analysis.

Next steps are to address the NYC/NYS issue and to compare totals from the CDC to data previously downloaded using CTP.

The cdcProcessed file is converted to a format for easier analysis:  
```{r}

glimpse(cdcProcessed)

# Key fields will be date-state-tot_*-new_*-bool_c_*
cdcPivotLong <- cdcProcessed %>%
    select(date, state, 
           tot_cases, conf_cases, prob_cases, new_cases=new_case, pnew_cases=pnew_case, 
           tot_deaths=tot_death, conf_deaths=conf_death, prob_deaths=prob_death, 
           new_deaths=new_death, pnew_deaths=pnew_death,
           bool_cases=bool_c_cases, bool_deaths=bool_c_deaths
           ) %>%
    pivot_longer(-c(date, state)) %>%
...
cdcPivotLong %>%
    ...
    summarize(n=n(), nna=sum(is.na(value)), sum=sum(value, na.rm=TRUE))

# Function for summing NA so that if all values are NA it is NA, otherwise return sum(na.rm=TRUE)
specNASum <- function(x) if (any(!is.na(x))) sum(x, na.rm=TRUE) else NA

# Consolidate NYC to NY and then keep only state.abb plus DC while dropping the 'bool' data
cdcAnalysis <- cdcPivotLong %>%
    filter(modifier != "bool") %>%
    mutate(state=ifelse(state=="NYC", "NY", state)) %>%
    group_by(state, date, modifier, metric) %>%
    summarize(n=n(), nna=sum(is.na(value)), value=specNASum(value), .groups="drop") %>%
    arrange(date, state, metric, modifier)

glimpse(cdcAnalysis)

# Check that only NY has n > 1
cdcAnalysis %>%
    filter(n > 1) %>%
    count(state, n, nna, name="num")

# Check that specNASum worked
cdcAnalysis %>%
    group_by(n, nna) %>%
...
cdcAnalysis %>%
    ...
    summarize(sumValue=sum(value, na.rm=TRUE), n=n(), nna=sum(is.na(value)))

```

Data appear to be integrated reasonably.  The priority fields with modifiers 'new' and 'tot' do not have any NA remaining.  The data appear appropriate for comparison to previous data from CTP.

Final data from CTP are retrieved, and cumulative totals for cases and deaths as of 2021-03-07 are assessed:  
```{r}
...
stateCompareData <- cdcAnalysis %>%
    ...
    pivot_wider(state, names_from="metric", values_from=c("new", "tot")) %>%
    full_join(ctp_cum_210307, by=c("state")) %>%
    pivot_longer(-c(state))
stateCompareData

# Function to assess variables new_x, tot_x, and x for percentage differences
checkStateCompare <- function(met, 
                              df=stateCompareData
                              ) {
    
    # FUNCTION ARGUMENTS:
    # met: the metric to be used, "cases" or "deaths"
    # df: the data frame containing state-name-value data
    
    # Filter for just the key metrics, and calculate percentage of the "base" metric
    df <- df %>%
        filter(name %in% paste0(c("new_", "tot_", ""), met)) %>%
        group_by(state) %>%
        mutate(pct_of=value/sum(ifelse(name==all_of(met), value, 0))) %>%
        ungroup()
    
    # Plot the percentage differences for both new_cases and tot_cases
    p1 <- df %>%
        filter(name != all_of(met)) %>%
        ...
        coord_flip() + 
        labs(x="", 
             y="CDC as percentage of CTP (totals as of 2021-03-07)", 
             title=paste0("Comparison of CTP and CDC reporting for metric ", met)
             ) + 
        scale_color_discrete("CDC metric") + 
...
caseCompare <- checkStateCompare("cases")
deathCompare <- checkStateCompare("deaths")
...
* CDC reports significantly more cases for Iowa (+20%) and Missouri (+17%)  
* CDC reports modestly less cases for Massachusetts (-4%) and Hawaii (-6%)  
...
* CDC reports significantly less deaths for Indiana (-11% using 'new'), Ohio (-21% using 'new'), and New Jersey (-8% using 'new'), though sums of 'tot' for these states appear well aligned to the CTP data  
...
Next steps are to check the curve evolutions, particularly since states with differences in 'new' and 'tot' have large corrections all reported at once, potentially complicating interpretation of the evolution of disease impact in those jurisdictions.
