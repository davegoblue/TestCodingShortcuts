...
# Includes updates to account for field 'adult_beds' no longer being reliable, and loss of old hospital data
...
# 10. helperSummaryMap() - function for creating summary-level maps (updated for usmap::usmap_transform)
...
# 34. hospitalCapacityCDCDaily() - create plots of the daily hospital capacity
# 35. peakValleyCDCDaily() - plot and mark peaks and valleys in CDC daily data by state
# 36. multiSourceDataCombine() - combine data from two sources covering different time periods
# 37. convertCDCWeeklyDaily() - convert CDC weekly data to daily data
# 38. makeDaily() - Helper function to call weekly to daily

# Function to download/load, process, segment, and analyze data for CDC daily (last updated for weekly data)
readRunCDCDaily <- function(thruLabel, 
                            ...
                            brewPalette=NA,
                            convertWeekly=c("cdcWeeklyBurden", "vax"),
                            ...
                            ) {
    ...
    if (is.null(dfPerCapita)) {
        ...
        for (elem in elemUsed) {
            ...
            if(elem %in% all_of(convertWeekly)) dfProcessList[[elem]] <- makeDaily(dfProcessList[[elem]], type=elem)
...
# Annotate peaks and valleys in CDC daily data (updated for newer peak-valley approach)
makePeakValley <- function(df, 
    ...
    newDF <- df %>% 
        ...
        mutate(if(!is.null(rollMean)) across(all_of(numVar), .fns=zoo::rollmean, k=rollMean, fill=NA),
               across(all_of(numVar), .fns=findPeaks, width=windowWidth, FUN=max, .names="{.col}_isPeak"),
               across(all_of(numVar), .fns=findPeaks, width=windowWidth, FUN=min, .names="{.col}_isValley")
        ) %>% 
        ungroup()
...
# Create a summary of hospital capacity from the data
hospitalCapacityCDCDaily <- function(df, 
    ...
    if(isTRUE(createData)) df <- skinnyHHS(df, varMapper=mapper) %>% imputeNACapacity() %>% sumImputedHHS()
    ...
    pctState <- df %>% 
        mutate(pctAdult=inpatient_beds_occupied/inpatient_beds, pctCovidAdult=inpatient_beds_covid/inpatient_beds)
    exclStates <- pctState %>% 
        filter(inpatient_beds >0 & (pctAdult > maxCapacity | pctCovidAdult > maxCapacity)) %>% 
        count(state) %>% 
        pull(state)
    if(length(exclStates) > 0) plotSub <- paste0(plotSub, "\n(", paste(exclStates, collapse=", "), " data excluded)")
    
    # Create the adult beds summary    
    createGeoMap(df %>% filter(!(state %in% all_of(exclStates)), inpatient_beds > 0), 
                 yVars=list("pctCovidAdult"=c("label"="Covid", "color"="red"), 
                            "pctAdult"=c("label"="Total", "color"="black")
                 ), 
                 fullList=list("pctAdult"=expression(inpatient_beds_occupied/inpatient_beds), 
                               "pctCovidAdult"=expression(inpatient_beds_covid/inpatient_beds)
                 ), 
...
# Plot and mark peaks and valleys in CDC daily data by state
...
# Combine data (hospital by default) from two sources covering different timeframes
...
# Convert CDC weekly data to daily data
convertWeeklyDaily <- function(df, 
    ...
    tibble::tibble(keyTimes) %>%
        ...
        mutate(across(all_of(locfVars), .fns=function(x) zoo::na.locf(x, na.rm=FALSE)), 
...
# Helper function to call weekly-daily conversions
makeDaily <- function(df, type="cdcWeeklyBurden") {
    
    # FUNCTION ARGUMENTS:
    # df: the data frame
    # type: the type of conversion to be run ("cdcWeeklyBurden" or "vax" currently enabled)
    
    # Can run as-is for weekly burden data, convert all numeric variables using zoo::na.locf for vaccines
    if(type=="cdcWeeklyBurden") convertWeeklyDaily(df)
    else if(type=="vax") convertWeeklyDaily(df, locfVars=df %>% select(where(is.numeric)) %>% names, naVars=c())
    else {
        warning("\nFunction makeDaily is not available for specified type, returning dataset as-is\n")
        df
    }
