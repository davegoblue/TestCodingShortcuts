...
set.seed(1711290801)
rndDraw <- vector("list", 10)
rndCards <- vector("list", 10)
rndSuits <- vector("list", 10)
for (intCtr in 1:10) { 
...
set.seed(1712020805)
hndOrder <- sample(1:10, 10, replace=FALSE)
hndOrder
...
for (intCtr in 1:5) {
    ...
    cardD <- rndCards[[nD]]
    suitD <- rndSuits[[nD]]
    cardSuit1 <- cardP[suitP == 1]
    cardSuit2 <- rndCards[[intCtr]][rndSuits[[intCtr]] == 2]
    cardSuit3 <- rndCards[[intCtr]][rndSuits[[intCtr]] == 3]
    cardSuit4 <- rndCards[[intCtr]][rndSuits[[intCtr]] == 4]
    cat("\n\nPLAYER HAND: Suit 1: ", cardP[suitP == 1], "\tSuit 2: ", cardP[suitP == 2], 
        "\tSuit 3: ", cardP[suitP == 3], "\tSuit 4: ", cardP[suitP == 4]
        )
    cat("\nDEALER HAND: Suit 1: ", cardD[suitD == 1], "\tSuit 2: ", cardD[suitD == 2], 
        "\tSuit 3: ", cardD[suitD == 3], "\tSuit 4: ", cardD[suitD == 4]
        )
    cat("\nPLAYER STATS: Max Flush: ", hndDetails[[nP]]$maxFlush, "\tMax SF: ", hndDetails[[nP]]$maxSF, 
        "\tPLAYING HAND: ", hndDetails[[nP]]$playHand
        )
    cat("\nDEALER STATS: Max Flush: ", hndDetails[[nD]]$maxFlush, "\tQUAL: ", hndDetails[[nD]]$qualCheck, 
        "\tPLAYING HAND: ", hndDetails[[nD]]$playHand
        )
    if (hndDetails[[nP]]$maxFlush == 2 | 
        ...
        if (tmp[1] == 2) { hndResult <- -2 } else if (tmp[1] == 1) { hndResult <- 2} else { stop("PROBLEM!") }
    }
