...
Next, the types of hands that are available is assessed, and compared with the baseline:  
```{r}

library(dplyr)

countHandCombos <- function(n1, n2, n3, n4, a1=13, a2=13, a3=13, a4=13, kF=keyFramw) {
    hndCounts <- integer(12)

    ckFrame <- data.frame(nC=c(n1, n2, n3, n4)) %>% 
        group_by(nC) %>% 
        summarize(ct=n()) %>% 
        arrange(desc(ct), nC)
