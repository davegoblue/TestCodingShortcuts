...
sfTrack <- integer(length(playNum))
flTrack <- integer(length(playNum))
resTrack <- integer(length(playNum))
for (thisPlay in playNum) {
    scrD <- storeStat[2*thisPlay - 1, ]
    scrP <- storeStat[2*thisPlay, ]
    flTrack[thisPlay] <- scrP[1]
    sfTrack[thisPlay] <- scrP[2]
    ...
    resTrack[thisPlay] <- tmpRes
}

table(resTrack) / length(playNum)
mean(resTrack)
table(flTrack) / length(playNum)
table(sfTrack) / length(playNum)
